...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self)
    {
        ...
        NSString *path = [NSString stringWithFormat:@"%@/memery.txt", documentsDirectory];
        _writeHandle = [NSFileHandle fileHandleForWritingAtPath:path];
...
- (void) update{
    ...
    self.maxCpu = 100;//self.currCpu > self.maxCpu?self.currCpu:self.maxCpu;
...
- (void)writeFile {
    ...
    NSString *memeryString = [NSString stringWithFormat:@"%@ memery: %4.1fM\n", dateString, (self.currMem/1000000.0f)];
    
    [_writeHandle seekToEndOfFile];
    [_writeHandle writeData:[memeryString dataUsingEncoding:NSUTF8StringEncoding]];
}
