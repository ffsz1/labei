...
typedef NS_ENUM(NSUInteger, JXUIDeviceCurrentMemoryInfo) {
    JXUIDeviceCurrentMemoryInfoUsed = 1,
    JXUIDeviceCurrentMemoryInfoFree,
    JXUIDeviceCurrentMemoryInfoActive,
    JXUIDeviceCurrentMemoryInfoInactive,
    JXUIDeviceCurrentMemoryInfoWired,
    JXUIDeviceCurrentMemoryInfoPurgeable,
};

@implementation UIDevice (JXBase)
...
- (float)jx_cpuUsage {
    float cpu = 0;
    NSArray *cpus = [self jx_cpuUsagePerProcessor];
...
- (int64_t)jx_diskSpaceUsed {
    int64_t total = self.jx_diskSpace;
    int64_t free = self.jx_diskSpaceFree;
...
- (int64_t)jx_memoryTotal {
    int64_t mem = [[NSProcessInfo processInfo] physicalMemory];
    if (mem < -1) mem = -1;
    return mem;
}

static uint64_t jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfo type) {
    ...
    switch (type) {
        case JXUIDeviceCurrentMemoryInfoUsed:
        ...
        case JXUIDeviceCurrentMemoryInfoFree:
            return vm_stat.free_count * page_size;
        case JXUIDeviceCurrentMemoryInfoActive:
            return vm_stat.active_count * page_size;
        case JXUIDeviceCurrentMemoryInfoInactive:
            return vm_stat.inactive_count * page_size;
        case JXUIDeviceCurrentMemoryInfoWired:
            return vm_stat.wire_count * page_size;
        case JXUIDeviceCurrentMemoryInfoPurgeable:
...
- (int64_t)jx_memoryUsed {
    return jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfoUsed);
}

- (int64_t)jx_memoryFree {
    return jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfoFree);
}

- (int64_t)jx_memoryActive {
    return jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfoActive);
}

- (int64_t)jx_memoryInactive {
    return jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfoInactive);
}

- (int64_t)jx_memoryWired {
    return jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfoWired);
}

- (int64_t)jx_memoryPurgeable {
    return jx_current_memory_info_get_by_type(JXUIDeviceCurrentMemoryInfoPurgeable);
}

#pragma mark - Network Info
- (NSString *)_jx_ipAddressWithIfaName:(NSString *)name {
...
- (NSString *)jx_ipAddressWIFI {
    return [self _jx_ipAddressWithIfaName:@"en0"]; // WiFi接入时网卡为en0
}

- (NSString *)jx_ipAddressCell {
    return [self _jx_ipAddressWithIfaName:@"pdp_ip0"]; // // 2G/3G/4G接入时网卡为pdp_ip0
}

- (NSString *)jx_macAddress {
    int                 _mib[6];
    size_t              _length;
    char                *buffer;
    unsigned char       *ptr;
...
typedef struct {
    ...
    uint64_t awdl_out;
} jx_net_interface_counter;

static uint64_t jx_net_counter_add(uint64_t counter, uint64_t bytes) {
    // 进位转换 0xFFFFFFFF = -1
...
static uint64_t jx_net_counter_get_by_type(jx_net_interface_counter *counter, JXUIDeviceNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & JXUIDeviceNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & JXUIDeviceNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & JXUIDeviceNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & JXUIDeviceNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & JXUIDeviceNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & JXUIDeviceNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static jx_net_interface_counter jx_get_net_interface_counter() {
    ...
    jx_net_interface_counter counter = {0};
        ...
        while (cursor) {
            ...
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = jx_net_counter_add(counter_in, data->ifi_ibytes); // ifi_ibytes total number of octets received
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = jx_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)jx_getNetworkTrafficBytes:(JXUIDeviceNetworkTrafficType)types {
    jx_net_interface_counter counter = jx_get_net_interface_counter();
    return jx_net_counter_get_by_type(&counter, types);
}
