...
#ifndef JX_FONT_CLAMP_FONT_VALUES
#define JX_FONT_CLAMP_VALUES(_x_, _low_, _high_)  (((_x_) > (_high_)) ? (_high_) : (((_x_) < (_low_)) ? (_low_) : (_x_)))
#endif

@implementation UIFont (JXBase)

#pragma mark - Base
- (CGFloat)jx_fontWeight {
...
+ (UIFont *)jx_fontWithCGFont:(CGFontRef)CGFont size:(CGFloat)size {
...
- (CTFontRef)jx_CTFontRef CF_RETURNS_RETAINED  { // Name CTFontRef <-> CTFontRef
    CTFontRef font = CTFontCreateWithName((__bridge CFStringRef)self.fontName, self.pointSize, NULL);
    return font;
}

- (CGFontRef)jx_CGFontRef CF_RETURNS_RETAINED {
    CGFontRef font = CGFontCreateWithFontName((__bridge CFStringRef)self.fontName);
    return font;
}

#pragma mark - Create
+ (UIFont *)jx_lightSystemFontOfSize:(CGFloat)fontSize {
    return [UIFont fontWithName:@".SFUIText-Light" size:fontSize];
}

+ (UIFont *)jx_systemFontOfSize:(CGFloat)pointSize weightType:(JXUIFontWeightType)weightType italic:(BOOL)italic {
    BOOL isLight = weightType == JXUIFontWeightTypeLight;
    BOOL isBold = weightType == JXUIFontWeightTypeBold;
...
+ (UIFont *)jx_dynamicSystemFontOfSize:(CGFloat)pointSize weightType:(JXUIFontWeightType)weightType italic:(BOOL)italic {
    return [self jx_dynamicSystemFontOfSize:pointSize minimumPointSize:0 maximumPointSize:pointSize + 5 weightType:weightType italic:italic];
}

+ (UIFont *)jx_dynamicSystemFontOfSize:(CGFloat)pointSize minimumPointSize:(CGFloat)minimumPointSize maximumPointSize:(CGFloat)maximumPointSize weightType:(JXUIFontWeightType)weightType italic:(BOOL)italic {
    ...
    JX_FONT_CLAMP_VALUES(finalPointSize, minimumPointSize, maximumPointSize);
    return [UIFont jx_systemFontOfSize:finalPointSize weightType:weightType italic:NO];
}
...
+ (BOOL)jx_unloadFontFromData:(UIFont *)font {
...
+ (NSData *)jx_dataFromFont:(UIFont *)font {
    CGFontRef cgFont = font.jx_CGFontRef;
    NSData *data = [self jx_dataFromCGFont:cgFont];
    CGFontRelease(cgFont);
    return data;
}

typedef struct JXFontHeader {
    int32_t fVersion;
    uint16_t fNumTables;
...
static uint32_t JXCalcTableCheckSum(const uint32_t *table, uint32_t numberOfBytesInTable)
{
    uint32_t sum = 0;
...
+ (NSData *)jx_dataFromCGFont:(CGFontRef)cgFont {
    if (!cgFont) return nil;
    ...
    size_t totalSize = sizeof(JXFontHeader) + sizeof(JXTableEntry) * tableCount;
    ...
    JXFontHeader *offsetTable = (JXFontHeader *)dataPtr;
    ...
    dataPtr += sizeof(JXFontHeader);
    
    // write tables
    JXTableEntry *entry = (JXTableEntry *)dataPtr;
    dataPtr += sizeof(JXTableEntry) * tableCount;
    ...
    for (int index = 0; index < tableCount; ++index) {
        ...
        entry->fCheckSum = CFSwapInt32HostToBig(JXCalcTableCheckSum((uint32_t *)dataPtr, (uint32_t)tableSize));
