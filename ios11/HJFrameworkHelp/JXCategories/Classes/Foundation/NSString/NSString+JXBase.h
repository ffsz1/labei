...
typedef NS_OPTIONS(NSUInteger, JXNSStringSubstringConversionOptions) { ///< 子字符串转化处理方式
    JXNSStringSubstringConversionByAvoidBreakingUpCharacterSequences = 1, ///< 避免将合成字符序列(A一1️⃣)拆散
    /**
     合成字符序列长度固定(例: 字符串"😊😞", 长度为4，在该模式下，(0, 1)得到的是空字符串，(0, 2)得到的是"😊". 否则, (0, 1)或(0, 2), 得到的都是"😊", JXNSStringSubstringConversionByAvoidBreakingUpCharacterSequences下启用)
     */
    JXNSStringSubstringConversionByCharacterSequencesLengthRequired = 1 << 1,
    /**
     按照ASCII字符为1个字节, 非ASCII字符为2个字节的编码方式来处理(JXNSStringSubstringConversionByAvoidBreakingUpCharacterSequences下启用)
     */
    JXNSStringSubstringConversionByUsingNonASCIICharacterAsTwoEncoding = 1 << 2,
};

@interface NSString (JXBase)
 ...
 */
@property (nonatomic, readonly) NSUInteger jx_lengthOfUsingNonASCIICharacterAsTwoEncoding;
...
/**
 ...
 @return 字符串
 */
+ (NSString *)jx_stringByConcat:(id)obj, ... NS_REQUIRES_NIL_TERMINATION;
...
/**
 ...
 @return 指定范围内的所有子字符串集
 */
- (NSArray<NSString *> *)jx_substringsInRange:(NSRange)range usingBlock:(BOOL (^)(NSString *substring, NSRange substringRange, BOOL *stop))block;
 ...
 */
@property (nonatomic, readonly) NSUInteger jx_lengthOfComposedCharacterSequences;
...
/**
 获取首字母大写的字符串
 */
@property (readonly, copy) NSString *jx_firstCharacterUppercaseString;

/**
 获取首字母小写的字符串
 */
@property (readonly, copy) NSString *jx_firstCharacterLowercaseString;
...
@property (readonly) char jx_charValue;                           ///< 字符串对应的NSNumber对象的char值
@property (readonly) unsigned char jx_unsignedCharValue;          ///< 字符串对应的NSNumber对象的unsignedChar值
@property (readonly) short jx_shortValue;                         ///< 字符串对应的NSNumber对象的short值
@property (readonly) unsigned short jx_unsignedShortValue;        ///< 字符串对应的NSNumber对象的unsignedShort值
@property (readonly) int jx_intValue;                             ///< 字符串对应的NSNumber对象的int值
@property (readonly) unsigned int jx_unsignedIntValue;            ///< 字符串对应的NSNumber对象的unsignedInt值
@property (readonly) long jx_longValue;                           ///< 字符串对应的NSNumber对象的long值
@property (readonly) unsigned long jx_unsignedLongValue;          ///< 字符串对应的NSNumber对象的unsignedLong值
@property (readonly) long long jx_longLongValue;                  ///< 字符串对应的NSNumber对象的longLong值
@property (readonly) unsigned long long jx_unsignedLongLongValue; ///< 字符串对应的NSNumber对象的unsignedLongLong值
@property (readonly) float jx_floatValue;                         ///< 字符串对应的NSNumber对象的float值
@property (readonly) double jx_doubleValue;                       ///< 字符串对应的NSNumber对象的double值
@property (readonly) BOOL jx_boolValue;                           ///< 字符串对应的NSNumber对象的bool值
@property (readonly) NSInteger jx_integerValue;                   ///< 字符串对应的NSNumber对象的integer值
@property (readonly) NSUInteger jx_unsignedIntegerValue;          ///< 字符串对应的NSNumber对象的unsignedInteger值
@property (readonly) CGFloat jx_CGFloatValue;                     ///< 字符串对应的NSNumber对象的CGFloat值
 ...
 */
FOUNDATION_EXPORT NSString *JXNSDocumentsDirectory(void);
 ...
 */
FOUNDATION_EXPORT NSString *JXNSCachesDirectory(void);
 ...
 */
FOUNDATION_EXPORT NSString *JXNSLibraryDirectory(void);
 ...
 */
FOUNDATION_EXPORT NSString *JXNSTemporaryDirectory(void);
...
@interface NSMutableString (JXBase)
