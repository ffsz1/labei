...
#import "NSString+JXChinese.h"
#import "NSString+JXRegularExpression.h"

@implementation NSString (JXChinese)

- (NSString *)_jx_ChineseRegex:(JXNSStringChineseType)type {
    NSMutableString *regex = [NSMutableString stringWithString:@""];
    
    JXNSStringChineseType aType = type ? type : JXNSStringChineseTypeAll;
    if (aType & JXNSStringChineseTypeCharacter) {
        [regex appendString:[self _jx_ChineseCharactersRegex]];
    }
    
    if (aType & JXNSStringChineseTypePunctuation) {
        [regex appendString:[self _jx_ChinesePunctuationRegex]];
    }
    
    if (aType & JXNSStringChineseTypeRadical) {
        [regex appendString:[self _jx_ChineseRadicalRegex]];
    }
    
    if (aType & JXNSStringChineseTypeStroke) {
        [regex appendString:[self _jx_ChineseStrokeRegex]];
    }
    
    if (aType & JXNSStringChineseTypeIdeographicDescription) {
        [regex appendString:[self _jx_ChineseIdeographicDescriptionRegex]];
    }
...
- (NSString *)_jx_ChineseCharactersRegex {
    /*
...
- (NSString *)_jx_ChinesePunctuationRegex {
    /*
...
- (NSString *)_jx_ChineseRadicalRegex {
    /*
...
- (NSString *)_jx_ChineseStrokeRegex {
    /*
...
- (NSString *)_jx_ChineseIdeographicDescriptionRegex {
    /*
...
- (BOOL)jx_isChinese:(JXNSStringChineseType)type {
    NSMutableString *regex = [NSMutableString stringWithString:[self _jx_ChineseRegex:type]];
    if (!regex || !regex.length) return NO;
    
    [regex insertString:@"^" atIndex:0];
    [regex appendString:@"$"];
    return [self jx_matchesRegex:regex.copy options:0];
}

- (BOOL)jx_containsChinese:(JXNSStringChineseType)type {
    __block BOOL isContained = NO;
    [self jx_enumerateChinese:type inRange:NSMakeRange(0, self.length) usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
        isContained = YES;
        *stop = YES;
    }];
    return isContained;
...
- (NSString *)jx_substringWithChinese:(JXNSStringChineseType)type inRange:(NSRange)range {
    ...
    [substring jx_enumerateChinese:type inRange:NSMakeRange(0, substring.length) usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
...
- (void)jx_enumerateChinese:(JXNSStringChineseType)type
    ...
    [self enumerateSubstringsInRange:range options:NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString * _Nullable substring, NSRange substringRange, NSRange enclosingRange, BOOL * _Nonnull stop) {
        if ([substring jx_matchesRegex:[self _jx_ChineseRegex:type] options:0]) {
...
- (NSString *)jx_stringByReplacingChinese:(JXNSStringChineseType)type withString:(NSString *)replacement {
    return [self jx_stringByReplacingRegex:[self _jx_ChineseRegex:type] options:0 withString:replacement];
}

- (NSString *)jx_stringByTrimmingChinese:(JXNSStringChineseType)type {
    return [self jx_stringByReplacingChinese:type withString:@""];
}
