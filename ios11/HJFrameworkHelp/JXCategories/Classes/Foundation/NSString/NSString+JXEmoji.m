...
@implementation NSString (JXEmoji)

NS_INLINE BOOL JXStringIsEmoji(NSString * string) {
...
- (BOOL)_jx_isValidLengthOfEmoji {
    ...
    [self enumerateSubstringsInRange:self.jx_rangeOfAll options:NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
        if (substring) {
            length++;
        }
...
- (BOOL)jx_isEmoji {
    if (![self _jx_isValidLengthOfEmoji]) return NO;
    
    if (self.jx_lengthOfComposedCharacterSequences < 2) return JXStringIsEmoji(self);
    ...
    if (version < 9) {
        ...
        return [self jx_matchesRegex:regex.copy options:NSRegularExpressionCaseInsensitive];
    }
    return NO;
}
