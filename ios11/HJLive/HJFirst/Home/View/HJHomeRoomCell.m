...
-(void)setPeipeiModel:(HJHomePeipeiModel *)peipeiModel{
    ...
    self.animationImageView.image = [UIImage animatedGIFNamed:@"hj_home_people333"];
    self.flagLabel.text = @"";
     self.peopleBgImageView.image = [UIImage imageNamed:@"hj_first_findTa_icon"];
    self.numLabel.text = @"";
    ...
    self.sexImageView.frame = CGRectMake( CGRectGetMaxX(self.avatarImageView.frame)+  width, CGRectGetMinY(self.nameLabel.frame)+3, 12, 12);
       if (peipeiModel.gender ==1 ) {
            self.sexImageView.image= [UIImage imageNamed:@"hj_home_attend_man"];
       }else{
              self.sexImageView.image= [UIImage imageNamed:@"hj_home_attend_woman"];
       }
    
    
}
-(void)setMengxinModel:(HJHomeMengxinModel *)mengxinModel{
...
//       CGSize size = [self.nameLabel.text boundingRectWithSize:CGSizeMake(0, self.nameLabel.height) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.nameLabel.font} context:nil].size;
    ...
    if (mengxinModel.isFirstCharge) {
          self.mengxinBG.image = [UIImage imageNamed:@"hj_mengxin_icon_new"];
    }else{
         self.mengxinBG.image = [UIImage imageNamed:@"hj_xinjing_icon_new"];
    }
    //添加财富等级标签
    ...
    self.animationImageView.image = [UIImage animatedGIFNamed:@"hj_home_people333"];
    self.flagLabel.text = @"";
    self.peopleBgImageView.image = [UIImage imageNamed:@"hj_first_chat_icon"];
    self.numLabel.text = @"";
...
- (void)setModel:(HJHomePageInfo *)model
{
    _model = model;
     self.type = 1;
     self.flagLabel.text = @"";
...
//    self.idLabel.text = model[];
    ...
    self.animationImageView.image = [UIImage animatedGIFNamed:@"hj_home_people"];
