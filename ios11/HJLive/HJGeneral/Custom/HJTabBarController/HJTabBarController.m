...
@property (nonatomic,strong) HJTabBar *tabbar;

@property (nonatomic,assign) NSInteger lastIndex;

@property (nonatomic,strong) HJFloatingView *floatingView;
...
-(void)setStytle
{
    [[UITabBarItem appearance] setTitleTextAttributes:@{ NSForegroundColorAttributeName : [UIColor colorWithHexString:@"#979797"],NSFontAttributeName:[UIFont fontWithName:@"PingFang SC" size: 9]} forState:UIControlStateNormal];
    
    [[UITabBarItem appearance] setTitleTextAttributes:@{ NSForegroundColorAttributeName : [UIColor colorWithHexString:@"#FF81A4"],NSFontAttributeName:[UIFont fontWithName:@"PingFang SC" size: 9]} forState:UIControlStateSelected];
//    [[UITabBarItem appearance] setTitleTextAttributes:@{ NSForegroundColorAttributeName :  [UIColor colorWithPatternImage:[UIImage imageNamed:@"hj_home_peopleBg"]],NSFontAttributeName:[UIFont fontWithName:@"PingFang SC" size: 9]} forState:UIControlStateSelected];
    ...
    UIImageView *image = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"hj_tabBar_bg"]];
    ...
    // 适配iOS13导致的bug
      if (@available(iOS 13.0, *)) {
          // iOS 13以上
//          self.tabBar.tintColor =  [UIColor colorWithPatternImage:[UIImage imageNamed:@"hj_home_peopleBg"]];//#EE47B7   #752FB3
          self.tabBar.tintColor = [UIColor colorWithHexString:@"#FF81A4"];
          self.tabBar.unselectedItemTintColor = [UIColor colorWithHexString:@"#979797"];
          ...
          UITabBarItem *item = [UITabBarItem appearance];
          item.titlePositionAdjustment = UIOffsetMake(0, -2);
          [item setTitleTextAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:iPhoneX? 12:9], NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#979797"]} forState:UIControlStateNormal];
//          [item setTitleTextAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:iPhoneX? 12:9], NSForegroundColorAttributeName:[UIColor colorWithPatternImage:[UIImage imageNamed:@"hj_home_peopleBg"]]} forState:UIControlStateSelected];// #752FB3
...
- (void)addChildViewControllers{
    //图片大小建议32*32
    [self addChildrenViewController:[self controllerFromClassName:@"PagingViewController"] andTitle:@"首页" andImageName:@"hj_tabBar_home" andSelectImage:@"hj_tabBar_home_sel"];
//    [self addChildrenViewController:[self controllerFromClassName:@"HJSpeedMatchVC"] andTitle:@"交友" andImageName:@"hj_tabBar_find" andSelectImage:@"hj_tabBar_find_sel"];
//    //中间这个不设置东西，只占位
//    [self addChildrenViewController:[[UIViewController alloc] init] andTitle:@"" andImageName:@"" andSelectImage:@""];
    [self addChildrenViewController:self.msgVC andTitle:@"消息" andImageName:@"hj_tabBar_msg" andSelectImage:@"hj_tabBar_msg_sel"];
    [self addChildrenViewController:self.mineVC andTitle:@"我的" andImageName:@"hj_tabBar_me" andSelectImage:@"hj_tabBar_me_sel"];
}
...
- (HJBaseNavigationController *)controllerFromClassName:(NSString *)className{
    //2。获取类名
...
- (HJBaseNavigationController *)mineVC{
    UIViewController *vc = HJMeStoryBoard(@"HJMyViewController");
    HJBaseNavigationController *nav = [[HJBaseNavigationController alloc]initWithRootViewController:vc];
    nav.navigationBar.translucent = NO; //message
    return nav;
}

- (HJBaseNavigationController *)msgVC{
    UIViewController *vc = HJMessageBoard(@"HJMessageVC");
    HJBaseNavigationController *nav = [[HJBaseNavigationController alloc]initWithRootViewController:vc];
    nav.navigationBar.translucent = NO; //message
    return nav;
}
...
- (HJFloatingView *)floatingView
{
    if (!_floatingView) {
        _floatingView = [[NSBundle mainBundle] loadNibNamed:@"HJFloatingView" owner:self options:nil][0];
