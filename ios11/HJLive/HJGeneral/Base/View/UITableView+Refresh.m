...
- (void)setupRefreshFunctionWith:(RefreshType)type
{
    ...
    {
        //集成头部刷新
        self.mj_header = [HJRefreshFactory headerRefreshWithTarget:self refreshingAction:@selector(headerRefresh)];
    }
    else if (type==RefreshTypeFooter)
    {
        //集成底部刷新
        self.mj_footer = [HJRefreshFactory footerRefreshWithTarget:self refreshingAction:@selector(footerRefresh)];
...
- (void)setupRefreshFunctionWith:(RefreshType)type andColor:(UIColor *)color {
    ...
    if (type == RefreshTypeHeader)
    {
        //集成头部刷新
//        self.mj_header = [HJRefreshFactory headerRefreshWithTarget:self refreshingAction:@selector(headerRefresh)];
        self.mj_header = [HJRefreshFactory headerRefreshWithTarget:self refreshingAction:@selector(headerRefresh) color:color];
    }
    else if (type==RefreshTypeFooter)
    {
        //集成底部刷新
        self.mj_footer = [HJRefreshFactory footerRefreshWithTarget:self refreshingAction:@selector(footerRefresh) color:color];
        self.mj_footer.automaticallyHidden = YES;//没有数据自动隐藏
        self.footRefreshState =MJFooterRefreshStateNormal;
    }
    else if (type == RefreshTypeHeaderAndFooter)
