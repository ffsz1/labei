...
@interface HJWKWebViewController ()<WKNavigationDelegate,HJShareCoreClient, RedPacketViewDelegate,WKScriptMessageHandler,WKUIDelegate,UIImagePickerControllerDelegate,HJFileCoreClient,UINavigationControllerDelegate>
...
@property (strong, nonatomic) RedPacketView *redPacketView;//界面
...
@implementation HJWKWebViewController
...
- (void)initNav {
    UIBarButtonItem *rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"hj_inviteshare_share"] style:UIBarButtonItemStylePlain target:self action:@selector(showSharePanel)];
    self.navigationItem.rightBarButtonItem = rightBarButtonItem;
}

- (void)initWebView {
    UIBarButtonItem *leftBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"hj_arrow_little_left"] style:UIBarButtonItemStylePlain target:self action:@selector(backButtonClick)];
...
- (void)setUrl:(NSURL *)url{
    _url = url;
    if (self.url == nil) {
        NSString *urlSting = [NSString stringWithFormat:@"%@/front/links/links.html",[HJHttpRequestHelper getHostUrl]];
...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message {
    if ([message.name isEqualToString:@"openPurse"]) {//跳钱包页面
        ...
        NSString *uid = [NSString stringWithFormat:@"%@",message.body];
        if (uid.length > 0) {
            [[HJRoomViewControllerCenter defaultCenter]presentRoomViewWithRoomOwnerUid:uid.userIDValue succ:^(ChatRoomInfo *roomInfo) {
                if (roomInfo !=nil) {
        ...
        @try {
            ...
            if (requestMethod == 1) {
               [HJHttpRequestHelper POST:urlCon params:paramDic success:^(id data) {
                ...
                [HJHttpRequestHelper GET:urlCon params:paramDic success:^(id data) {
                                   [self callBackJSON:urlCon isRequestError:false bodyString:[(NSDictionary *)data yy_modelToJSONString]];
                               } failure:^(NSNumber *resCode, NSString *message) {
                                   [self callBackJSON:urlCon isRequestError:YES bodyString:@""];
                               }];
        ...
        NSString *jsonString = [NSString stringWithFormat:@"%@",message.body];
        HJWebResponseInfo *info = [HJWebResponseInfo yy_modelWithJSON:jsonString];
...
//        NSString *isOnYouthSet = [NSString stringWithFormat:@"isOnYouthSet%@",GetCore(HJAuthCoreHelp).getUid];
...
- (void)setupNavigationBarRightItem:(HJWebResponseInfo *)info {
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (selectedPhoto) {
            ...
            [GetCore(HJFileCore) qiNiuUploadImage:selectedPhoto uploadType:UploadImageTypeIDCard];
        }
    }];
}
...
- (void)showSharePanel {
    if (self.shareJsonData != nil || self.shareJsonData.length > 0) {
        ...
        HJShareInfo *shareInfo = [HJShareInfo yy_modelWithJSON:self.shareJsonData];
