...
@interface YYPhotoListView()<ImPageDelegate, UIPageViewControllerDelegate, UIPageViewControllerDataSource>
{
    __weak YYPhotoViewController* _currentVC;
...
@implementation YYPhotoListView
...
- (UIImage *)currentImage
{
    ...
    {
        if (obj && [obj isKindOfClass:[ImPageImageView class]]) {
            ImPageImageView *imageView = (ImPageImageView*)obj;
...
- (void)setDataProvider:(YYPhotoListDataProvider *)dataProvider
{
    if (dataProvider == nil) {
        return;
    }
...
- (YYPhotoViewController *)photoViewControllerForPageIndex:(NSUInteger)pageIndex
{
    if (pageIndex < self.dataProvider.imageCount)
    {
        YYPhotoViewController* vc = [[YYPhotoViewController alloc] initWithPageIndex:pageIndex];
        vc.view.frame = CGRectMake(0, 0, MRScreenWidth, MRScreenHeight);
        
        NSArray *imageList = self.dataProvider.imageUrlList;
        ImPageImageView *view = [ImPageImageView view];
...
- (UIViewController *)pageViewController:(UIPageViewController *)pvc viewControllerBeforeViewController:(YYPhotoViewController *)vc
{
...
- (UIViewController *)pageViewController:(UIPageViewController *)pvc viewControllerAfterViewController:(YYPhotoViewController *)vc
{
    NSUInteger index = vc.pageIndex;
    return [self photoViewControllerForPageIndex:(index + 1)];
}
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed
{
    ...
    YYPhotoViewController *curVC = [pageViewController.viewControllers firstObject];
    
    if (curVC && [curVC isKindOfClass:[YYPhotoViewController class]]) {
...
- (void)_configImageViewList:(YYPhotoListDataProvider *)dataProvider
{
    BOOL refreshAll = YES;
    ...
    if (refreshAll || (_currentVC == nil)) {
        ...
        YYPhotoViewController *curPage = [self photoViewControllerForPageIndex:self.dataProvider.imageIndex];
        _currentVC = curPage;
