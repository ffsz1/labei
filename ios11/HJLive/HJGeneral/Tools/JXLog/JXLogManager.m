...
@interface JXLogManager ()<HJNotificationCoreClient>
...
+ (instancetype) sharedInstance{
    ...
    static JXLogManager* instance = nil;
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (!instance) {
            instance = [[JXLogManager alloc]init];
        }
    });
...
#pragma mark - 获取参数
    ...
    // [时间]-[模块]-日志内容
    NSString* timeStr = [self.timeFormatter stringFromDate:[JXLogManager getCurrDate]];
    NSString* writeStr = [NSString stringWithFormat:@"[%@]-[%@]-[%@]-%@\n",timeStr,module,[GetCore(HJAuthCoreHelp) getUid],parmaStr];
...
- (void)clearExpiredLog{
    ...
    for (NSString* file in files) {
        ...
        if (date) {
            ...
            NSTimeInterval currTime = [[JXLogManager getCurrDate] timeIntervalSince1970];
...
- (void)uploadLogToServerWithZipFileName:(NSString *)zipFileName {
    ...
    [GetCore(HJFileCore) uploadFileWithPath:zipFilePath fileName:zipFileName success:^(NSString *url) {
        [HJHttpRequestHelper clientLogSaveWithUrl:url success:^{
            @strongify(self);
...
- (void)onRecvCustomP2PNoti:(NIMCustomSystemNotification *)notification {
    ...
    if (attachment != nil && [attachment isKindOfClass:[Attachment class]]) {
