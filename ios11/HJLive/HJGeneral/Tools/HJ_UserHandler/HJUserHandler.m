...
+ (void)reportPost:(NSInteger)uid
        reportType:(NSInteger)reportType
              type:(NSInteger)type
{
    [HJHttpRequestHelper userReportSaveWithUid:uid reportType:reportType type:type phoneNo:@"" success:^{
...
{
    [HJHttpRequestHelper reportAvatar:uid success:^{
        [MBProgressHUD showSuccess:@"举报成功，我们将尽快为你处理" toView:[UIApplication sharedApplication].keyWindow];
    } failure:^(NSNumber *resCode, NSString *message) {
        [MBProgressHUD hideHUD];
    }];
...
{
    [HJHttpRequestHelper reportAlbum:uid success:^{
        [MBProgressHUD showSuccess:@"举报成功，我们将尽快为你处理" toView:[UIApplication sharedApplication].keyWindow];
    } failure:^(NSNumber *resCode, NSString *message) {
        [MBProgressHUD hideHUD];
    }];
...
+ (void)cancelAttendPost:(UserID)tagUID cancelFollowBlock:(void (^)())cancelFollowBlock
{
    ...
    __weak typeof(self)weakSelf = self;
    [HJHttpRequestHelper cancel:mine.userIDValue beCanceledUid:tagUID success:^{
...
+ (void)follow:(UserID)followUID followSucceed:(void (^)())followBlock
{
    ...
    __weak typeof(self)weakSelf = self;
    [HJHttpRequestHelper praise:[GetCore(HJAuthCoreHelp) getUid].userIDValue bePraisedUid:followUID success:^{
        
        [MBProgressHUD hideHUD];
        
        if (tagUID  == GetCore(HJImRoomCoreV2).currentRoomInfo.uid) {
...
+ (void)sendIMMsg_FollowRoomOwner:(UserID)bePraisedUid
{
    ...
    UserInfo *myInfo = [GetCore(HJUserCoreHelp) getUserInfoInDB:[GetCore(HJAuthCoreHelp) getUid].userIDValue];
...
+ (BOOL)isOnMySpaceVC
{
    ...
    if ([vc.className isEqualToString:@"HJBaseNavigationController"]) {
        UINavigationController *navi = (UINavigationController *)vc;
        
        for (UIViewController *childVC in navi.childViewControllers) {
            if ([childVC.className isEqualToString:@"HJMySpaceVC"]) {
                hasPhotoReport = YES;
            }
        }
        
    }else if ([vc.className isEqualToString:@"HJTabBarController"]){
