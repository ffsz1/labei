...
#import "HJAuctionListEmptyCell.h"
...
@interface HJContributionListView()<
UITableViewDelegate,
...
@implementation HJContributionListView
...
- (void)awakeFromNib {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"HJAuctionListerCell" bundle:nil] forCellReuseIdentifier:@"HJAuctionListerCell"];
    [self.tableView registerNib:[UINib nibWithNibName:@"HJAuctionListEmptyCell" bundle:nil] forCellReuseIdentifier:@"HJAuctionListEmptyCell"];
    [self updateView];
...
- (void)updateView {
    ...
    if (self.roomInfo != nil) {
        [GetCore(HJRoomCoreV2Help) getNewRoomBounsList:self.type dataType:self.dataType];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.contribution.count == 0) {
        HJAuctionListEmptyCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HJAuctionListEmptyCell"];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        return cell;
    } else {
        HJAuctionListerCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HJAuctionListerCell"];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        HJRoomBounsListInfo *list = self.contribution[indexPath.row];
...
//            cell.lvImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"Lv%ld",(long)list.experLevel]];
            ...
            cell.countLeftImg.image = [UIImage imageNamed:@"hj_charts_icon_jinbin"];
...
//            cell.lvImageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"ml%ld",(long)list.charmLevel]];
        ...
        if (indexPath.row == 0) {
            ...
            cell.rankListImageView.image = [UIImage imageNamed:@"hj_charts_icon_no_1_1"];
//            cell.nicknameLabel.textColor = cell.noBottomView.backgroundColor = UIColorHex(ff6164);
            ...
            cell.rankNumLabel.hidden = YES;
            cell.rankListImageView.hidden = NO;
            cell.rankListImageView.image = [UIImage imageNamed:@"hj_charts_icon_no_2_1"];
//            cell.nicknameLabel.textColor = cell.noBottomView.backgroundColor = UIColorHex(ffc107);
            ...
            cell.rankNumLabel.hidden = YES;
            cell.rankListImageView.hidden = NO;
            cell.rankListImageView.image = [UIImage imageNamed:@"hj_charts_icon_no_3_1"];
//            cell.nicknameLabel.textColor = cell.noBottomView.backgroundColor = UIColorHex(339cfe);
            ...
            cell.nicknameLabel.textColor = [[YYTheme defaultTheme]colorWithHexString:@"#FFFFFF" alpha:1];
...
//        // 神豪
//        for (HJRoomBounsListInfo *info in self.contribution) {
//            info.sumGold = @"";
//        }
//    }
...
- (ZJScrollSegmentView *)segmentView {
    if (!_segmentView) {
        ...
        @weakify(self);
        ZJScrollSegmentView *segment = [[ZJScrollSegmentView alloc] initWithFrame:CGRectMake(0, 0, 200, 44) segmentStyle:style delegate:nil titles:@[@"神豪榜",@"心动榜"] titleDidClick:^(ZJTitleView *titleView, NSInteger index) {
            @strongify(self);
            if (index == 0) {
                self.type = @"1";
            } else if (index == 1) {
