...
#import "HJOpenRoomViewController.h"
...
@interface HJRoomViewControllerCenter()
<
    HJRoomCoreClient,
...
@property(nonatomic, strong) HJEnterRoomWithPwdView *pwdView;
@property(nonatomic, strong) TYAlertController *pwdAlertView;
@property(nonatomic, strong) RedPacketView *redView;
@property(nonatomic, strong) TYAlertController *redAlert;

@property (nonatomic, strong) HJRedInfo *info;

@end

@implementation HJRoomViewControllerCenter
...
- (void)openRoonWithType:(RoomType)type {
    ...
    @weakify(self);
    [HJHttpRequestHelper setupShouldShowErrorHUD:NO];
    [GetCore(HJRoomCoreV2Help) getRoomInfo:[GetCore(HJAuthCoreHelp) getUid].userIDValue success:^(ChatRoomInfo *roomInfo) {
        if (roomInfo == nil) { //当前没开房
            ...
            if (type == RoomType_Game) {
                [GetCore(HJRoomCoreV2Help) openRoom:[GetCore(HJAuthCoreHelp)getUid].userIDValue type:type title:title roomDesc:@"" backPic:@"" rewardId:nil];
            }else {
                UINavigationController *nav = [[HJRoomViewControllerFactory sharedFactory] instantiateRoomNavigationController];
                HJOpenRoomViewController *vc = (HJOpenRoomViewController *)[[HJRoomViewControllerFactory sharedFactory]instantiateOpenRoomViewController];
                vc.roomtype = type;
            ...
            ChatRoomInfo *info = roomInfo;
            if (type == info.type) { //要开的房间跟已经开的房间一样
                if (info.valid) { //房间有效
                    [self presentRoomViewWithRoomInfo:info]; //直接进入房间
                }else{
                    if (type == RoomType_Game) {
                        [GetCore(HJRoomCoreV2Help) openRoom:[GetCore(HJAuthCoreHelp) getUid].userIDValue type:type title:info.title roomDesc:info.roomDesc backPic:@"" rewardId:nil];
                    }else {
                        UINavigationController *nav = [[HJRoomViewControllerFactory sharedFactory] instantiateRoomNavigationController];
                        HJOpenRoomViewController *vc = (HJOpenRoomViewController *)[[HJRoomViewControllerFactory sharedFactory]instantiateOpenRoomViewController];
                        vc.roomtype = type;
...
//根据房间所有者id。获取房间信息
- (void)presentRoomViewWithRoomOwnerUid:(UserID)ownerUid succ:(void (^)(ChatRoomInfo *))succBlock fail:(void (^)(NSString *))failBlock{
    
    [YYLogger info:@"开始进入房间" message:@"%lld",ownerUid];
    if (ownerUid > 0) {
        @weakify(self);
        [HJHttpRequestHelper setupShouldShowErrorHUD:NO];
        [GetCore(HJRoomCoreV2Help) getRoomInfo:ownerUid success:^(ChatRoomInfo *roomInfo) {
            [YYLogger info:@"开始进入房间--获取房间信息成功" message:@"%@",roomInfo];
            if ([GetCore(HJRoomCoreV2Help) isBeKickedWithRoomid:[NSString stringWithFormat:@"%ld",(long)roomInfo.roomId]]) {
                if (roomInfo.roomPwd.length > 0 && [GetCore(HJAuthCoreHelp) getUid].userIDValue != roomInfo.uid) {
                    [MBProgressHUD hideHUD];
                    [YYLogger info:@"进入房间--有密码" message:@"要弹密码框"];
                    HJEnterRoomWithPwdView *pwdView = [HJEnterRoomWithPwdView loadFromNib];
...
//根据房间信息开房
- (void)presentRoomViewWithRoomInfo:(ChatRoomInfo *)roomInfo{
    ...
    GetCore(HJFaceCore).hideFaceList = roomInfo.hideFace;

    //当前房间信息
    ChatRoomInfo *currentRoomInfo = [GetCore(HJRoomCoreV2Help) getCurrentRoomInfo];
    ...
    if (self.current == nil) {//导航为nil
        ...
        if (roomInfo.type == RoomType_Game) {//哄趴
            ...
            if (!isReback) {
                [GetCore(HJRoomCoreV2Help) saveCharmInfoMap:nil];
                GetCore(HJRoomCoreV2Help).messages = nil;
                GetCore(HJImRoomCoreV2).micQueue = nil;
                
            }
            
//            GetCore(HJImRoomCoreV2).currentRoomInfo =
            
            vc = (HJGameRoomContainerVC *)[[HJRoomViewControllerFactory sharedFactory]instantiateGameRoomContainerViewController];
        }else {
        ...
        [MBProgressHUD hideHUD];
        [GetCore(HJUserCoreHelp) getUserInfo:roomInfo.uid refresh:YES success:^(UserInfo *info) {
            [YYLogger info:@"进入房间--获取房主信息" message:@"info:%@",info];
            GetCore(HJImRoomCoreV2).roomOwnerInfo = info;
            

            UIViewController *presentingViewController = [YYViewControllerCenter currentVisiableRootViewController];
            if ([presentingViewController isKindOfClass:[HJGameRoomContainerVC class]]) return;
            
            if ([presentingViewController isKindOfClass:[UINavigationController class]]) {
                if ([[(UINavigationController *)presentingViewController topViewController] isKindOfClass:[HJGameRoomContainerVC class]]) return;
            }
            //life
            UIViewController* currentVC = presentingViewController;
            
            if (!currentVC) {
                currentVC = [HJRoomViewControllerCenter currentVisiableRootViewController];
            }
//            [[YYViewControllerCenter currentVisiableRootViewController] presentViewController:nav animated:YES completion:^{
             [currentVC presentViewController:nav animated:YES completion:^{
                @strongify(self)
                
                //解决最小化，进别的房间的缓存问题
                GetCore(HJImRoomCoreV2).enterRoomInfo = roomInfo;
                ...
                if (currentRoomInfo == nil) { //点击进入房间
                    ...
                    if (currentRoomInfo.roomId != roomInfo.roomId) { //最小化a房间，点击进入b房间
                        ...
                        GetCore(HJGiftCore).lastGiftIsAll = NO;
                        GetCore(HJGiftCore).lastGiftInfo = nil;
                        [YYLogger info:@"进入房间--退出聊天室" message:@"最小化a房间，点击进入b房间--roomInfo.roomId:%zd----currentRoomInfo.roomId：%zd",roomInfo.roomId,GetCore(HJImRoomCoreV2).currentRoomInfo.roomId];
                        [GetCore(HJImRoomCoreV2) exitChatRoom:GetCore(HJImRoomCoreV2).currentRoomInfo.roomId];//退出a房间
                    }else{
...
//        return;
    ...
    if ([[YYViewControllerCenter currentViewController] isKindOfClass:[YYActionSheetViewController class]]) {
        [(YYActionSheetViewController *)[YYViewControllerCenter currentViewController]dismiss];
    }
    
    if ([[YYViewControllerCenter currentViewController] isKindOfClass:[UIAlertController class]]) {
        [(UIAlertController *)[YYViewControllerCenter currentViewController]dismissViewControllerAnimated:NO completion:nil];
    }
    ...
    if (isQuit) {
        ...
        GetCore(HJImRoomCoreV2).currentRoomInfo = nil;
    } else {
...
{
    if (self.current != nil) {
        HJGameRoomContainerVC *vc = self.current.viewControllers[0];
        [vc beKicked];
    }
    else if (GetCore(HJImRoomCoreV2).currentRoomInfo.roomId != 0) {
        [GetCore(HJImRoomCoreV2) exitChatRoom:GetCore(HJImRoomCoreV2).currentRoomInfo.roomId];
        [GetCore(HJMusicCore) stopMusic];
    }
...
- (void)onMeExitChatRoomSuccessV2
{
    if (self.currentRoomInfo != nil) {
        [GetCore(HJImRoomCoreV2) enterChatRoom:self.currentRoomInfo.roomId];
...
- (void)jumpInRoomWithRoomid:(NSString *)uid {
    if (uid.length > 0) {
        ...
        if ([GetCore(HJImLoginCore)isImLogin] && GetCore(HJVersionCoreHelp).versionInfo) {
            dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0/*延迟执行时间*/ * NSEC_PER_SEC));
            @weakify(self);
            dispatch_after(delayTime, dispatch_get_main_queue(), ^{
                @strongify(self);
                [self presentRoomViewWithRoomOwnerUid:self.notificationUid succ:^(ChatRoomInfo *roomInfo) {
...
- (void)onRequestVersionStatusSuccess:(VersionInfo *)versionInfo {
    if (versionInfo.status != Version_Notice) {
        if (self.notificationUid > 0) {
            if ([GetCore(HJImLoginCore)isImLogin]) {
                dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0/*延迟执行时间*/ * NSEC_PER_SEC));
                @weakify(self);
                dispatch_after(delayTime, dispatch_get_main_queue(), ^{
                    @strongify(self);
...
{
    if (self.notificationUid > 0) {
        if (GetCore(HJVersionCoreHelp).versionInfo.status != Version_Notice) {
            [self presentRoomViewWithRoomOwnerUid:self.notificationUid succ:^(ChatRoomInfo *roomInfo) {
                [[HJRoomViewControllerCenter defaultCenter] presentRoomViewWithRoomInfo:roomInfo];
            } fail:^(NSString *errorMsg) {
            }];
...
- (void)thereIsNoMicoPrivacy {
    ...
    if (![[[HJRoomViewControllerCenter defaultCenter]getCurrentVC] isKindOfClass:[UIAlertController class]] || ![[[HJRoomViewControllerCenter defaultCenter]getCurrentVC] isKindOfClass:[TYAlertController class]]) {
        [[[HJRoomViewControllerCenter defaultCenter]getCurrentVC]presentViewController:alert animated:YES completion:nil];
    }
}
...
- (void)onCloseRoomSuccess {
    ...
    if (self.needOpen) {
        if (self.needOpenRoomType != 0) {
            UserInfo *info = [GetCore(HJUserCoreHelp) getUserInfoInDB:[GetCore(HJAuthCoreHelp) getUid].userIDValue];
            NSString *title = [NSString stringWithFormat:@"%@的房间",info.nick];
            [GetCore(HJRoomCoreV2Help)openRoom:[GetCore(HJAuthCoreHelp)getUid].userIDValue type:self.needOpenRoomType title:title roomDesc:@"" backPic:@"" rewardId:nil];
        }
    }
}
...
//        if (info.uid == [GetCore(HJAuthCoreHelp) getUid].userIDValue) {
//
//            if (info.type == 1) {
//                self.info = info;
//                // 第一次注册
//                [GetCore(HJUserCoreHelp) roomRcmdGet];
//            }
//            else {
//
...
- (void)showRedPacketWithInfo:(HJRedInfo *)info {
    if (!info.needAlert) return;
    
    RedPacketView *redView = [RedPacketView loadFromNib];
...
- (void)roomRcmdGetSuccessWithResult:(NSDictionary *)result {
    ...
    if (roomId == 0) {
        ...
        self.info = nil;
        [[HJTopAlertViewTool shareHJTopAlertViewTool] showNewUserAlertWithViewController:[self getCurrentVC] roomId:roomId uid:uid title:title avatar:avatar];
    }
}
...
- (void)showAuthorizationAlertWithCode:(NSInteger)code message:(NSString *)message {
    ...
    [JXAuthorizationAlertHelper showAuthorizationAlertWithViewController:[UIApplication sharedApplication].jx_topViewController code:code message:message didTapActionHandler:^(UIViewController * _Nullable toViewController) {
...
- (void)showTheRoomIsOpen:(NSString *)roomname {
    ...
    UIAlertAction *enter = [UIAlertAction actionWithTitle:NSLocalizedString(XCRoomClose, nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [MBProgressHUD showMessage:NSLocalizedString(XCHudLoadingTip1, nil)];
        self.needOpen = YES;
        [GetCore(HJRoomCoreV2Help) closeRoom:[GetCore(HJAuthCoreHelp)getUid].userIDValue];
    }];
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:NSLocalizedString(XCRoomCancel, nil) style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        [MBProgressHUD hideHUD];
    }];
...
- (void)onReceiveTurntableMessage {
    XCTurntableAlertView *view = [XCTurntableAlertView loadFromNib];
...
- (void)onGoBtnClick {
    ...
    if (self.current) { //当前界面在房间内
        ...
        HJTabBarController *vc = (HJTabBarController *)[UIApplication  sharedApplication].keyWindow.rootViewController;
...
- (void)findTheWkWebViewInNav:(UINavigationController *)nav {
    ...
    for (UIViewController *item in nav.viewControllers) {
        if ([item isKindOfClass:[HJWKWebViewController class]] && ![[self getCurrentVC] isKindOfClass:[HJWKWebViewController class]]) { //在导航堆栈里面找到这个控制器并且不是当前控制器
