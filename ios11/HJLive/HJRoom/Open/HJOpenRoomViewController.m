...
@interface HJOpenRoomViewController ()<HJRoomCoreClient,UINavigationControllerDelegate, UIImagePickerControllerDelegate, HJFileCoreClient,UserProtocolViewDelegate,UITextFieldDelegate>
...
@property (weak, nonatomic) IBOutlet StateButton *openRoomBtn;
@property (nonatomic, assign) BOOL isSetCover;
@property (nonatomic, strong) NSString* coverUrl;
@property (nonatomic, strong) ChatRoomInfo *lastRoomInfo;
@property (nonatomic, strong) UserProtocolView *userProtocolView;
- (IBAction)onOpenRoomBtnClicked:(id)sender;

@end

@implementation HJOpenRoomViewController
...
- (void)initView
{
    ...
    [self.titleField setValue:[[YYDefaultTheme defaultTheme] colorWithHexString:@"#cccccc" alpha:1.0] forKeyPath:@"_placeholderLabel.textColor"];
    
    [self.noticeTextView setPlaceholder:NSLocalizedString(XCRoomFillInRoomAd, nil)];
    
    self.userProtocolView = [UserProtocolView loadFromNIB];
    ...
    [[GetCore(HJRoomCoreV2Help) requestRoomInfo:[GetCore(HJAuthCoreHelp) getUid].userIDValue] subscribeNext:^(id x) {
    }];
}
...
{
    [GetCore(HJRoomCoreV2Help) openRoom:[GetCore(HJAuthCoreHelp) getUid].userIDValue type:self.roomtype title:self.titleField.text roomDesc:self.noticeTextView.text backPic:url rewardId:nil];
}
...
- (IBAction)onOpenRoomBtnClicked:(id)sender {
    ...
    if (self.isSetCover) {
        [GetCore(HJFileCore) qiNiuUploadImage:self.coverBg.image uploadType:UploadImageTypeChatRoot];
    } else {
        NSString *backPic = self.lastRoomInfo.backPic;
        [GetCore(HJRoomCoreV2Help) openRoom:[GetCore(HJAuthCoreHelp) getUid].userIDValue type:self.roomtype title:self.titleField.text roomDesc:self.noticeTextView.text backPic:backPic rewardId:nil];
    }
}
...
- (void)showIsInLiveAlert {
    ...
    UIAlertAction *enter = [UIAlertAction actionWithTitle:NSLocalizedString(XCRoomEnterRoom, nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        @weakify(self);
        [[HJRoomViewControllerCenter defaultCenter]presentRoomViewWithRoomOwnerUid:[GetCore(HJAuthCoreHelp) getUid].userIDValue succ:^(ChatRoomInfo *roomInfo) {
            [MBProgressHUD hideHUD];
            @strongify(self);
            [self.navigationController dismissViewControllerAnimated:YES completion:^{
                [[HJRoomViewControllerCenter defaultCenter] presentRoomViewWithRoomInfo:roomInfo];
...
- (void)updateViewByLastRoomInfo:(ChatRoomInfo *)roomInfo
{
