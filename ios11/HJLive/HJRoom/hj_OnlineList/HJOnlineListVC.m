...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (GetCore(HJImRoomCoreV2).currentRoomInfo.type == RoomType_Game) {
        HJOnlineListCell *cell = (HJOnlineListCell *)[tableView dequeueReusableCellWithIdentifier:@"HJOnlineListCell"];
        ...
        HJRoomMemberListCell *cell = (HJRoomMemberListCell *)[tableView dequeueReusableCellWithIdentifier:@"HJRoomMemberListCell"];
...
- (void)fetchRoomUserListSuccess:(int)state {
    ...
    if (GetCore(HJImRoomCoreV2).currentRoomInfo.type == RoomType_Game) {
        [self.onlineUserList addObjectsFromArray:GetCore(HJImRoomCoreV2).noMicMembers];
...
- (void)initView {
    ...
    if (GetCore(HJImRoomCoreV2).currentRoomInfo.type == RoomType_Game) {
        [self.tableView registerNib:[UINib nibWithNibName:@"HJOnlineListCell" bundle:nil] forCellReuseIdentifier:@"HJOnlineListCell"];
    }else {
...
- (void)showAlertWithAddBlackList:(UserID)uid {
    ...
    [[GetCore(HJUserCoreHelp) getUserInfoByUid:uid refresh:NO] subscribeNext:^(id x) {
...
- (void)pullDownRefresh:(int)page{
    ...
    if (GetCore(HJImRoomCoreV2).currentRoomInfo.type == RoomType_Game) {
        [GetCore(HJImRoomCoreV2) queryNoMicChatRoomMembersWithPage:self.currentPage state:0];

    }else {
        [GetCore(HJImRoomCoreV2) queryChatRoomMembersWithPage:self.currentPage state:0];
    }
}
...
- (void)pullUpRefresh:(int)page lastPage:(BOOL)isLastPage{
    self.currentPage = page;
    if (GetCore(HJImRoomCoreV2).currentRoomInfo.type == RoomType_Game) {
        [GetCore(HJImRoomCoreV2) queryNoMicChatRoomMembersWithPage:self.currentPage state:1];

    }else {
        [GetCore(HJImRoomCoreV2) queryChatRoomMembersWithPage:self.currentPage state:1];
    }
}
