...
@interface HJLiveGiftShowCustom ()

@property (nonatomic ,strong) NSMutableDictionary * showViewDict;/**< key([self getDictKey]):value(HJLiveGiftShowView*) */
//用来记录模型的顺序
@property (nonatomic ,strong) NSMutableArray * showViewArr;/**< [HJLiveGiftShowView] */

@end

@implementation HJLiveGiftShowCustom
...
+ (instancetype)addToView:(UIView *)superView{
    HJLiveGiftShowCustom * v = [[HJLiveGiftShowCustom alloc]init];
...
- (void)addHJLiveGiftShowModel:(HJLiveGiftShowModel *)showModel{
    [self addHJLiveGiftShowModel:showModel showNumber:0];
}

- (void)addHJLiveGiftShowModel:(HJLiveGiftShowModel *)showModel showNumber:(NSInteger)showNumber{
    if (!showModel || ![showModel isKindOfClass:[HJLiveGiftShowModel class]]) {
        return;
    }
    
    HJLiveGiftShowView * oldShowView = [self.showViewDict objectForKey:[self getDictKey:showModel]];
    
    //判断是否强制修改显示的数字
    BOOL isResetNumber = showNumber > 0 ? YES : NO;

    //如果不存在旧模型
    if (!oldShowView || ![oldShowView isKindOfClass:[HJLiveGiftShowView class]]) {
        //如果当前弹幕数量大于最大限制
        if (self.showViewArr.count >= live_maxGiftShowCount) {
            ...
            if (![self.showViewArr containsObject:kGiftViewRemoved]) {
                //排序 最小的时间在第一个
                NSArray * sortArr = [self.showViewArr sortedArrayUsingComparator:^NSComparisonResult(HJLiveGiftShowView * obj1, HJLiveGiftShowView * obj2) {
                    return [obj1.creatDate compare:obj2.creatDate];
                }];
                HJLiveGiftShowView * oldestView = sortArr.firstObject;
                //重置模型
        ...
        //创建新模型
        HJLiveGiftShowView * newShowView = [[HJLiveGiftShowView alloc]initWithFrame:CGRectMake(0, showViewY, 0, 0)];
        //赋值
        ...
        newShowView.HJLiveGiftShowViewTimeOut = ^(HJLiveGiftShowView * willReMoveShowView){
            //从数组移除
...
- (void)resetY{
    for (int i = 0; i < self.showViewArr.count; i++) {
        HJLiveGiftShowView * show = self.showViewArr[i];
        if ([show isKindOfClass:[HJLiveGiftShowView class]]) {
...
- (void)sortShowArr{
    ...
    for (int i = 0; i < self.showViewArr.count; i++) {
        for (int j = i; j < self.showViewArr.count; j++) {
            HJLiveGiftShowView * showViewI = self.showViewArr[i];
            HJLiveGiftShowView * showViewJ = self.showViewArr[j];
            if ([showViewI isKindOfClass:[HJLiveGiftShowView class]] && [showViewJ isKindOfClass:[HJLiveGiftShowView class]]) {
...
- (void)searchLiveShowViewFrom:(int)i{
    for (int j = i; j < self.showViewArr.count; j++) {
        HJLiveGiftShowView * next = self.showViewArr[j];
        if ([next isKindOfClass:[HJLiveGiftShowView class]]) {
...
#pragma mark - Private
- (void)resetView:(HJLiveGiftShowView *)view nowModel:(HJLiveGiftShowModel *)model isChangeNum:(BOOL)isChange number:(NSInteger)number{
...
- (NSString *)getDictKey:(HJLiveGiftShowModel *)model{
    //默认以 用户名+礼物类型 为key
