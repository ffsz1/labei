...
@implementation HJLiveGiftShowView
...
- (void)setModel:(HJLiveGiftShowModel *)model{
    if (!model) {
        return;
    }
    _model = model;
    ...
    [self.iconIV yy_setImageWithURL:[NSURL URLWithString:model.user.iconUrl] placeholder:[UIImage imageNamed:default_avatar]];
    
    self.sendLabel.text = model.giftModel.rewardMsg;
    [self.giftIV yy_setImageWithURL:[NSURL URLWithString:model.giftModel.picUrl] placeholder:[UIImage imageNamed:default_avatar]];
    ...
    if (self.liveTimerForSecond > kTimeOut) {
        ...
        [UIView animateWithDuration:kRemoveAnimationTime delay:kNumberAnimationTime options:UIViewAnimationOptionCurveEaseIn animations:^{
            ...
            if (finished) {
                ...
                if (self.HJLiveGiftShowViewTimeOut) {
                    self.HJLiveGiftShowViewTimeOut(self);
                }
...
//        _backIV.image = [UIImage imageNamed:@"hj_w_liveGiftBack"];
        _backIV.backgroundColor = RGBACOLOR(0, 0, 0, 0.16);
//        _backIV.layer.borderColor = RGBACOLOR(255, 255, 255, 0.16).CGColor;
//        _backIV.layer.borderWidth = 3;
        _backIV.layer.cornerRadius = 25;
...
- (HJLiveGiftShowNumberView *)numberView{
    if (!_numberView) {
        HJLiveGiftShowNumberView * nv = [[HJLiveGiftShowNumberView alloc]init];
        [self addSubview:nv];
        _numberView = nv;
    }
    return _numberView;
