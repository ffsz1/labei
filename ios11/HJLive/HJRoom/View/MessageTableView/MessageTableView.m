...
#import "HJGiftSendAllMicroAvatarInfo.h"
...
@property (nonatomic, strong) YYActionSheetViewController* startAuctionSheet;//开启拍卖
...
+ (instancetype)loadFromNib {
    return [[NSBundle mainBundle]loadNibNamed:@"MessageTableView" owner:self options:nil].lastObject;
}
...
- (void)awakeFromNib {
    ...
    [self.tableView registerClass:[NewMessageCell class] forCellReuseIdentifier:@"NewMessageCell"];
    [self.tableView registerNib:[UINib nibWithNibName:@"HJPlayingFaceMessageCell2" bundle:nil] forCellReuseIdentifier:@"HJPlayingFaceMessageCell2"];
    
    [self.tableView registerNib:[UINib nibWithNibName:@"HJMessagePKCell" bundle:nil] forCellReuseIdentifier:@"HJMessagePKCell"];
    ...
    //盲盒飘屏点击头像通知回调
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(notificationMangheGift:) name:@"NotificationMangheGift" object:nil];
    
}

//盲盒飘屏点击头像通知回调
- (void)notificationMangheGift:(NSNotification *)notification {
    UserID infouid = [notification.object[@"infouid"] integerValue];
    [self showPersonalViewForFrom:infouid withName:@""];
}
...
- (void)handleMessageTypeWith:(HJMessageType)messageType{
    self.messages = [NSMutableArray arrayWithArray:GetCore(HJRoomCoreV2Help).messages];
    ...
    if (messageType == HJMessageTypeTalk) {
        for (HJIMMessage * msg in self.messages) {
...
- (void)handleNotificationCell:(NewMessageCell *)newCell Message:(HJIMMessage *)msg indexPath:(NSIndexPath *)indexPath{
...
- (void)handleZDCell:(NewMessageCell *)newCell Message:(HJIMMessage *)msg indexPath:(NSIndexPath *)indexPath {
    ...
    Attachment *attachment = obj.attachment;
...
//            newCell.bgImage.image = [UIImage imageNamed:@"msg_bg_red"];
    ...
    if (!attributed) {
        ...
        if ([dic objectForKey:@"params"]) {
            ...
            giftPrice = [NSString stringWithFormat:@"（%@开心）", [NSString hm_stringFromDouble:[paramsDic[@"goldPrice"] doubleValue] roundingScale:0]];
            if (giftPrice != nil) {
...
//          NSString *text = [NSString stringWithFormat:@"恭喜 %@ 捡海螺获得礼物 “%@” x%@", nick, gitName, gitCount];
        NSString *text = [NSString stringWithFormat:@"哇塞 %@ 在开盲盒中获得了 %@ x%@", nick, gitName, gitCount];
        UIColor *redColor = [[YYDefaultTheme defaultTheme]colorWithHexString:@"#2ACEF5" alpha:1.0];
          UIColor *giftColor = [[YYDefaultTheme defaultTheme]colorWithHexString:@"#FFCA28" alpha:1.0];
        
        UIColor *color1 = [[YYDefaultTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:1.0];
...
- (void)handleSecretCell:(NewMessageCell *)newCell Message:(HJIMMessage *)msg indexPath:(NSIndexPath *)indexPath{
    ...
    if (!attributed) {
        ...
        UIColor *redColor = [[YYDefaultTheme defaultTheme]colorWithHexString:@"#FFD800" alpha:1.0];
        UIColor *color1 = [[YYDefaultTheme defaultTheme] colorWithHexString:@"#F2F2F2" alpha:1];
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:text];
        [str addAttribute:NSForegroundColorAttributeName
                    value:color1
                    range:text.rangeOfAll];
...
#pragma mark - handleCellForCell
- (void)handleTextCell:(NewMessageCell *)newCell Message:(HJIMMessage *)msg indexPath:(NSIndexPath *)indexPath{
...
/** 送礼物*/
- (void)handleGiftCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
    NSMutableAttributedString *attributed = [self._contentAttributedStringForIndexPath objectForKey:msg.messageId];
    ...
    if (!attributed) {
        ...
        if (giftInfo == nil) {
            [GetCore(HJGiftCore) findGiftInfoByGiftId:info.giftId giftyType:GiftTypeMystic];
        }
        
        
        NSString *text = [NSString stringWithFormat:@"%@送%@%@X%ld", info.nick,info.targetNick,giftInfo.giftName,(long)info.giftNum];
        
        UIColor *yellowColor = [[YYDefaultTheme defaultTheme]colorWithHexString:@"#FFEC00" alpha:1.0];
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:text];
...
/** 全麦送*/
- (void)handleWholeMicSendCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
...
- (void)handlechangeRoomNameSendCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
    ...
    if (attachment.second == Custom_Noti_Header_ChargeRoomName) {
        ...
        ChatRoomMember *member = GetCore(HJImRoomCoreV2).roomOwner;
...
/** 打Call*/
- (void)handlePlayCallSendCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
        ...
        if (!attributed) {
            ...
            NSString *giftName = [NSString stringWithFormat:@"%@",data[@"giftName"]];
             ...
             NSString *uid = [NSString stringWithFormat:@"%@",data[@"uid"]];
                ...
                NSString *text = [NSString stringWithFormat:@"%@为%@打Call送出%@X%@",sendName,targetName,giftName,@"1"];
                UIColor *yellowColor = [[YYDefaultTheme defaultTheme]colorWithHexString:@"#FFEC00" alpha:1.0];
                NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:text];
...
//玩法数据处理
- (void)handleRoomWanfaCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
    if (attachment.second == Custom_Noti_Header_Wanfa) {
        ...
        if (!attributed) {
            ...
            UIColor *redColor = [[YYDefaultTheme defaultTheme]colorWithHexString:@"#07CFFF" alpha:1.0];
...
- (void)handleRoomTipCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
    if (attachment.second == Custom_Noti_Header_Room_Tip_ShareRoom) {
        ...
        if (!attributed) {
            ...
            UIColor *whiteColor = [[YYDefaultTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:1];
            NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:text];
...
- (void)handleLongZhuCell:(NewMessageCell *)newCell  Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath {
    
    NSMutableAttributedString *attributed = [self._contentAttributedStringForIndexPath objectForKey:msg.messageId];
    if (!attributed) {
        attributed = [self getLongZhuAttributedStringWithMessage:msg attachment:attachment];
        
        HJRoomLongZhuMsgModel *info = [HJRoomLongZhuMsgModel yy_modelWithDictionary:attachment.data];
...
- (void)handleSystemNewsCell:(NewMessageCell *)newCell Message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath{
    ...
    if (!attributed) {
        ...
        if ([dataDict isKindOfClass:[NSDictionary class]]) {
            ...
            if (attachment.second == Custom_Noti_Sub_Gift_Effect_Close) {
                ...
                [str addAttributes:@{NSForegroundColorAttributeName : [[YYDefaultTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:0.6f]} range:NSMakeRange(0, text.length)];
...
//                }
                ...
                str = [[NSMutableAttributedString alloc] initWithString:text];
                [str addAttributes:@{NSForegroundColorAttributeName : [[YYDefaultTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:0.6f]} range:NSMakeRange(0, text.length)];
//                [str addAttributes:@{NSForegroundColorAttributeName : [[YYDefaultTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:1]} range:[text rangeOfString:name]];
...
- (void)handleFaceCell:(NewMessageCell *)cell message:(HJIMMessage *)msg attachment:(Attachment *)attachment indexPath:(NSIndexPath *)indexPath {
...
- (void)configPKCell:(HJMessagePKCell *)cell message:(HJIMMessage *)msg attachment:(Attachment *)attachment
{
//    GiftReceiveInfo *info = [GiftReceiveInfo yy_modelWithDictionary:attachment.data];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (msg.messageType == HJIMMessageTypeCustom) {
        ...
        if (attachment.first == Custom_Noti_Header_Mora) {
            HJMessagePKCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HJMessagePKCell"];
            [self configPKCell:cell message:msg attachment:attachment];
            return cell;
        }
    }

    NewMessageCell * newCell = [tableView dequeueReusableCellWithIdentifier:identifier];
    ...
    if (msg.messageType == Custom_Noti_Header_Winning ||
        ...
        newCell.labelContentView.backgroundColor = [[YYDefaultTheme defaultTheme] colorWithHexString:@"#050505" alpha:0.17];
...
//                [self showPersonalView:msg.from.userIDValue withName:msg.member.nick];
        ...
        if (content.eventType == JXIMChatroomEventTypeEnter) {//enter room
            ChatRoomMember *member = msg.member;
...
- (NSMutableAttributedString *)getUserAttributedStringWithMessage:(HJIMMessage *)msg {
    return [self getUserAttributedStringWithMessage:msg isShowMyName:NO];
}

- (NSMutableAttributedString *)getUserAttributedStringWithMessage:(HJIMMessage *)msg isShowMyName:(BOOL)isShowMyName {
    if (!msg) return nil;
        ...
        if (msg.messageType == HJIMMessageTypeCustom) {
            ...
            if (attachment.first == Custom_Noti_Header_Face) {
                if (attachment.second == Custom_Noti_Sub_Face_Send) {
                    ...
                    HJFaceReceiveInfo *info = [faceattachement.data firstObject];
                    member = [ChatRoomMember new];
...
- (NSMutableAttributedString *)getTextAttributedStringWithMessage:(HJIMMessage *)msg {
    if (!msg) return nil;
...
- (NSMutableAttributedString *)getEnterNotificationAttributedStringWithMessage:(HJIMMessage *)msg {
    if (!msg) return nil;
    ...
    NSString *text = nil;
    NSMutableAttributedString *str = nil;
    UIColor *color = [[YYDefaultTheme defaultTheme] colorWithHexString:@"#FFD800" alpha:1];
    UIColor *color2 = [[YYDefaultTheme defaultTheme] colorWithHexString:@"#F2F2F2" alpha:1];
...
- (NSMutableAttributedString *)getFaceAttributedStringWithMessage:(HJIMMessage *)message {
    if (message) {
            ...
            if (arr.count > 0) {
                ...
                for (int i = 0; i < arr.count; i++) {
                    ...
                    for (int j = 0; j < item.resultIndexes.count; j++) {
                        ...
                        UIImage *face = [GetCore(HJFaceCore)findFaceImageById:item.faceId index:[index integerValue]];
                        YYAnimatedImageView * imageView = [[YYAnimatedImageView alloc] init];
...
- (NSMutableAttributedString *)getLongZhuAttributedStringWithMessage:(HJIMMessage *)message attachment:(Attachment *)attachment {
...
//    NSLog(@"reloadChatList");
    ...
    HJIMMessage *msg = self.messages.lastObject;
...
- (void)showMsgRoomOwnerOperation:(UserID)userId name:(NSString *)name {
    ...
    ChatRoomMember *myMember = GetCore(HJImRoomCoreV2).myMember;;
     ...
     @weakify(self);
    [[GetCore(HJImRoomCoreV2) rac_queryChartRoomMemberByUid:[NSString stringWithFormat:@"%lld",userId]] subscribeNext:^(id x) {
        @strongify(self);
...
//设置房主弹框
- (void)setupCreatorSheet:(UIAlertController *)alter userId:(UserID)userId name:(NSString *)name member:(ChatRoomMember *)member{
    ChatRoomMember *myMember = GetCore(HJImRoomCoreV2).myMember;;
    if (myMember.is_creator) {
        if (!member.is_creator) {
...
//                    [GetCore(HJRoomQueueCoreV2Help) inviteUpFreeMic:userId];
//                }]];
            ...
            [alter addAction:[UIAlertAction actionWithTitle:@"踢出房间" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [HJMidSure kickUser:userId didKickFinish:nil];
            }]];
            ...
            if (member.is_manager) {
                ...
                [alter addAction:[UIAlertAction actionWithTitle:@"移除管理员" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [GetCore(HJImRoomCoreV2) markManagerList:userId enable:NO];
                }]];
                ...
                [alter addAction:[UIAlertAction actionWithTitle:@"设置管理员" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [GetCore(HJImRoomCoreV2) markManagerList:userId enable:YES];
                }]];
...
//设置管理员弹框
- (void)setupManagerSheet:(UIAlertController *)alter userId:(UserID)userId name:(NSString *)name member:(ChatRoomMember *)member{
    ChatRoomMember *myMember = GetCore(HJImRoomCoreV2).myMember;;
...
//    UserID uid2 = [GetCore(HJAuthCoreHelp) getUid].userIDValue;
...
- (void)showPersonalView:(UserID)uid withName:(NSString *)name {
    ...
    if (uid == uid2) {
        ...
        if (name.length) {
            ...
            @weakify(self);
            [[GetCore(HJUserCoreHelp) getUserInfoByUidV2:uid] subscribeNext:^(id x) {
                @strongify(self);
...
//黑名单
- (void)showAlertWithAddBlackList:(ChatRoomMember *)member name:(NSString *)name uid:(NSInteger)uid {
    __block NSString *title = [NSString stringWithFormat:@"%@%@",NSLocalizedString(XCRoomMoveToBlackTitle, nil),name];
    
    [GetCore(HJUserCoreHelp) getUserInfo:[member.account longLongValue] refresh:NO success:^(UserInfo *info) {
    ...
    UIAlertAction *enter = [UIAlertAction actionWithTitle:NSLocalizedString(XCRoomConfirm, nil) style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        [GetCore(HJImRoomCoreV2) markBlackList:uid enable:YES];
    }];
...
- (BOOL)isShowUserCard:(ChatRoomMember *) member{
    ChatRoomMember *myMember = GetCore(HJImRoomCoreV2).myMember;;
...
- (HJRoomMsgHeaderView *)headerView
{
    if (!_headerView) {
        _headerView = [[HJRoomMsgHeaderView alloc] initWithFrame:CGRectMake(0, 0, XC_SCREE_W, 72)];
    }
    return _headerView;
}
