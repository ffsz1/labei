...
#import "HJZaJinDanRankListCell.h"
...
@interface HJZaJinDanRankList () <UITableViewDelegate,UITableViewDataSource,HJRoomCoreClient,HJImRoomCoreClientV2>
...
@implementation HJZaJinDanRankList
...
- (void)awakeFromNib {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"HJZaJinDanRankListCell" bundle:nil] forCellReuseIdentifier:@"HJZaJinDanRankListCell"];
...
//    self.tableView.mj_footer.hidden = YES;
    [GetCore(HJRoomCoreV2Help) userGiftPurseGetRankWithType:self.type];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HJZaJinDanRankListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HJZaJinDanRankListCell"];
    cell.model = self.dataArr[indexPath.row];

    
    if (indexPath.row == 0 ) {
        cell.crownIcon.hidden = NO;
        [cell.crownIcon setImage:[UIImage imageNamed:@"hj_dacall_call_gold"]];
        cell.rankingLabel.textColor = [UIColor darkGrayColor];
    }else if (indexPath.row == 1) {
        cell.crownIcon.hidden = NO;
        [cell.crownIcon setImage:[UIImage imageNamed:@"hj_dacall_call_silvery"]];
        cell.rankingLabel.textColor = [UIColor darkGrayColor];
    }
    else if (indexPath.row == 2) {
        cell.crownIcon.hidden = NO;
        [cell.crownIcon setImage:[UIImage imageNamed:@"hj_dacall_call_orange"]];
        cell.rankingLabel.textColor = [UIColor darkGrayColor];
    }
    else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HJGiftPurseRank *rank = self.dataArr[indexPath.row];
    if (rank) {
...
- (ZJScrollSegmentView *)segmentView {
    if (!_segmentView) {
        ...
        style.normalTitleColor = UIColorHex(6087E5);
        ...
        style.scrollLineImage = [UIImage imageNamed:@"hj_room_dacall_rank_slider"];
        style.scrollLineIsBackground = YES;
        
        @weakify(self);
        ZJScrollSegmentView *segment = [[ZJScrollSegmentView alloc] initWithFrame:CGRectMake(0, 0, 252, 34) segmentStyle:style delegate:nil titles:@[@"今日",@"昨日",@"周榜"] titleDidClick:^(ZJTitleView *titleView, NSInteger index) {
            @strongify(self);
            NSInteger type = self.type;
            if (index == 0) {
                self.type = ZaJinDanRankListTypeToday;
