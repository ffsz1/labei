...
@interface HJOnlineListTwoView()<
UITableViewDelegate,
UITableViewDataSource,
HJImRoomCoreClientV2,
HJRoomCoreClient
>
@property (strong, nonatomic) NSMutableArray<ChatRoomMember *> *onlineUserList;
@property (weak, nonatomic) IBOutlet UIView *online;
@end

@implementation HJOnlineListTwoView
{
    int pageNo;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ChatRoomMember *member = [self.onlineUserList safeObjectAtIndex:(int)indexPath.row];
...
- (void)showAlertViewWithMember:(ChatRoomMember *)member {
    UserID uid = [member.account longLongValue];
    if (self.alertUserInformation) {
        self.alertUserInformation(uid);
    }
...
//        [GetCore(HJOnlineCore) showGitToUidFromOnline:uid withName:member.nick];
//        [self removeCore];
...
//    }];
    ...
    if (myMember.is_creator || myMember.is_manager) {
        if ([GetCore(HJRoomQueueCoreV2Help) isOnMicro:uid]) {
            ...
            [alter addAction:[UIAlertAction actionWithTitle:@"抱Ta下麦" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                @strongify(self);
                NSString *positon = [GetCore(HJRoomQueueCoreV2Help) findThePositionByUid:uid];
                if (positon) {
                    [GetCore(HJRoomQueueCoreV2Help) kickDownMic:uid position:positon.intValue];
                }
            }]];
...
//            }];
            ...
            [alter addAction:[UIAlertAction actionWithTitle:@"封锁此座位" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                @strongify(self);
                ChatRoomMember *seq = [GetCore(HJRoomQueueCoreV2Help) findTheMemberByUserId:uid];
                NSString *positon = [GetCore(HJRoomQueueCoreV2Help) findThePositionByUid:uid];
                if (seq) {
                    if (positon) {
                        [GetCore(HJRoomQueueCoreV2Help) kickDownMic:uid position:positon.intValue];
                ...
                if (positon) {
                    [GetCore(HJRoomQueueCoreV2Help) lockMicPlace:positon.intValue];
                }
            }]];
            
//            [sheet addButtonWithTitle:NSLocalizedString(XCRoomLockThatSeat, nil) block:^(YYActionSheetViewController *controller) {
//                @strongify(self);
//                ChatRoomMember *seq = [GetCore(HJRoomQueueCoreV2Help) findTheMemberByUserId:uid];
//                NSString *positon = [GetCore(HJRoomQueueCoreV2Help) findThePositionByUid:uid];
//                if (seq) {
...
//            [sheet addButtonWithTitle:NSLocalizedString(XCRoomBringHimUpMic, nil) block:^(YYActionSheetViewController *controller) {
//                @strongify(self);
//                [GetCore(HJRoomQueueCoreV2Help) inviteUpFreeMic:uid];
//            }];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    HJRoomMemberListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HJRoomMemberListCell"];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    ChatRoomMember *member = [self.onlineUserList safeObjectAtIndex:indexPath.row];
...
//    cell.nameLabel.textColor = [UIColor blackColor];
    ...
    [[[GetCore(HJImRoomCoreV2) rac_fetchMemberUserInfoByUid:member.account] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(id x) {
        NIMUser *user  = (NIMUser *)x;
        if (user.userInfo.gender == NIMUserGenderMale) {
            cell.genderImageView.image = [UIImage imageNamed:@"hj_sex_male_logo"];
        }else if (user.userInfo.gender == NIMUserGenderFemale) {
            cell.genderImageView.image = [UIImage imageNamed:@"hj_sex_female_logo"];
        }
    }];
    
    if (member.account.integerValue == GetCore(HJImRoomCoreV2).roomOwner.account.integerValue && member.account.integerValue != 0) {
        ...
        if ([GetCore(HJRoomQueueCoreV2Help) isOnMicro:[member.account longLongValue]]) {
            cell.roleImageView.hidden = NO;
            cell.onMicroWidthCOnstraint.constant = 25;
...
- (void)awakeFromNib {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"HJRoomMemberListCell" bundle:nil] forCellReuseIdentifier:@"HJRoomMemberListCell"];
    ...
    MJRefreshNormalHeader *header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        @strongify(self);
        pageNo = 1;
        [GetCore(HJImRoomCoreV2) queryChatRoomMembersWithPage:pageNo state:0];
    }];
    ...
    MJRefreshAutoNormalFooter *footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        ...
        [GetCore(HJImRoomCoreV2) queryChatRoomMembersWithPage:pageNo state:1];
    }];
...
- (NSMutableArray<ChatRoomMember *> *)onlineUserList{
...
//    }
    ...
    pageNo++;
    self.onlineUserList = GetCore(HJImRoomCoreV2).displayMembers;
//    [self.onlineUserList addObjectsFromArray:GetCore(HJImRoomCoreV2).displayMembers];
    [self.tableView reloadData];
