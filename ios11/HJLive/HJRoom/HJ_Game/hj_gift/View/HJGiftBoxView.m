...
#import "HJRenqipiaoIntrodutionView.h"


@interface HJGiftBoxView ()
...
//麦上用户
@property (nonatomic,strong) HJGiftMicView *micView;

@property (nonatomic,strong) HJGiftCollectionView *giftView;

@property (assign,nonatomic) XBDGiftBoxType type;

@property (strong,nonatomic) HJPointCoinWalletModel *pointCoinModel;
...
@implementation HJGiftBoxView

//+ (instancetype)shareGiftBoxView
//{
////    static HJGiftBoxView *_sharedSingleton = nil;
////    static dispatch_once_t onceToken;
////    dispatch_once(&onceToken, ^{
////        _sharedSingleton = [[NSBundle mainBundle]loadNibNamed:@"HJGiftBoxView" owner:self options:nil][0];
////    });
//    return [[NSBundle mainBundle]loadNibNamed:@"HJGiftBoxView" owner:self options:nil][0];
//
//}

+ (void)show:(UserID)tagUID
{
    [HJGiftBoxView show:tagUID isAllMic:NO isChat:NO];
}

+ (void)showAllMic:(UserID)tagUID
{
    [HJGiftBoxView show:tagUID isAllMic:YES isChat:NO];
}

+ (void)showChat:(UserID)tagUID
{
    [HJGiftBoxView show:tagUID isAllMic:NO isChat:YES];
}

+ (void)show:(UserID)tagUID isAllMic:(BOOL)isAllMic isChat:(BOOL)isChat
{
    HJGiftBoxView *shareView = [[NSBundle mainBundle]loadNibNamed:@"HJGiftBoxView" owner:self options:nil][0];
    ...
    shareView.coinNumLabel.text = [NSString stringWithFormat:@"%.0f",[GetCore(PurseCore).balanceInfo.goldNum doubleValue]];
...
//    HJRenqipiaoIntrodutionVC* vc = [[HJRenqipiaoIntrodutionVC alloc] init];
    ...
    [HJRenqipiaoIntrodutionView showRenqipiaoIntrodutionView];
...
- (IBAction)normalBtnAction:(id)sender {
    ...
    [self resetNumBtn];
    [self.numBtn_define setTitle:@"自定义数量" forState:UIControlStateNormal];
...
- (IBAction)bagBtnAction:(id)sender {
    ...
    [self resetNumBtn];
    [self.numBtn_define setTitle:@"ALL 全部" forState:UIControlStateNormal];
    [self.numBtn setTitle:@"1" forState:UIControlStateNormal];
}
//活动
...
//自定义数量
- (IBAction)numAction_define:(UIButton *)sender {
    
    if (![sender.titleLabel.text isEqualToString:@"自定义数量"]) {
        [self hidenNumView];
        GiftInfo *giftModel = [self.giftView getGiftModel];
        [self.numBtn setTitle:[NSString stringWithFormat:@"%zd",giftModel.userGiftPurseNum] forState:UIControlStateNormal];
        [self resetNumBtn];
        self.freeNum = giftModel.userGiftPurseNum;
...
//发送私聊礼物消息
- (void)setChatGift:(GiftInfo *)giftModel giftNum:(NSInteger)giftNum
{
    UserInfo *info = [GetCore(HJUserCoreHelp) getUserInfoInDB:self.uid];
    
    if (self.type == XBDGiftBoxTypeNormal) {
        
        [GetCore(HJGiftCore)sendChatGift:giftModel.giftId info:info giftNum:giftNum targetUid:self.uid  giftyType:GiftTypeNormal];;
    }
    
    if (self.type == XBDGiftBoxTypeBag) {
        [GetCore(HJGiftCore)sendChatGift:giftModel.giftId info:info giftNum:giftNum targetUid:self.uid  giftyType:GiftTypeMystic];
    }
    
    if (self.type == XBDGiftBoxTypePoint) {
        [GetCore(HJGiftCore)sendChatPoint:giftModel.giftId info:info giftNum:giftNum targetUid:self.uid  giftyType:GiftTypeNormal];
    }
...
//发送单人礼物消息
- (void)setSingleGift:(GiftInfo *)giftModel giftNum:(NSInteger)giftNum
{
    
    if (self.type == XBDGiftBoxTypeNormal) {
        [GetCore(HJGiftCore) sendRoomGift:giftModel.giftId targetUid:self.uid type:3 giftNum:giftNum giftyType:GiftTypeNormal goldPrice:giftModel.goldPrice];
    }
    
    if (self.type == XBDGiftBoxTypeBag) {
        [GetCore(HJGiftCore) sendRoomGift:giftModel.giftId targetUid:self.uid type:3 giftNum:giftNum giftyType:GiftTypeMystic goldPrice:giftModel.goldPrice];
    }
    
    if (self.type == XBDGiftBoxTypePoint) {
        [GetCore(HJGiftCore) sendPoint:giftModel.giftId targetUid:self.uid type:3 giftNum:giftNum giftyType:GiftTypeDiandianCoin goldPrice:giftModel.goldPrice];
    }
    
}

- (void)sendOnMicGift:(GiftInfo *)giftModel giftNum:(NSInteger)giftNum
{
    ...
    if (self.type == XBDGiftBoxTypeNormal) {
        [GetCore(HJGiftCore) sendAllMicroGiftByUids:uidsStr giftId:giftModel.giftId giftNum:giftNum roomUid:GetCore(HJImRoomCoreV2).currentRoomInfo.uid isAllMicroSend:isAllMic giftyType:GiftTypeNormal goldPrice:giftModel.goldPrice];
    }
    
    if (self.type == XBDGiftBoxTypeBag) {
        [GetCore(HJGiftCore) sendAllMicroGiftByUids:uidsStr giftId:giftModel.giftId giftNum:giftNum roomUid:GetCore(HJImRoomCoreV2).currentRoomInfo.uid isAllMicroSend:isAllMic giftyType:GiftTypeMystic goldPrice:giftModel.goldPrice];
    }
    
    if (self.type == XBDGiftBoxTypePoint) {
        [GetCore(HJGiftCore) sendPointWholeMicro:uidsStr giftId:giftModel.giftId giftNum:giftNum roomUid:GetCore(HJImRoomCoreV2).currentRoomInfo.uid isAllMicroSend:isAllMic giftyType:GiftTypeDiandianCoin goldPrice:giftModel.goldPrice];
    }
...
//        self.iconImageView.image = [UIImage imageNamed:@"hj_sign_alter_coin"];
...
//        self.addCoinLabel.hidden = YES;
        ...
        self.iconImageView.image = [UIImage imageNamed:@"xc_room_gift_icon_jinbi"];
        self.coinNumLabel.text = [NSString stringWithFormat:@"%.0f",[GetCore(PurseCore).balanceInfo.goldNum doubleValue]];
...
- (HJGiftMicView *)micView
{
    if (!_micView) {
        _micView = [[HJGiftMicView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 85)];
...
- (HJGiftCollectionView *)giftView
{
     __weak typeof(self) weakSelf = self;
    if (!_giftView) {
        _giftView = [[HJGiftCollectionView alloc] initWithFrame:CGRectZero];
        _giftView.allBagNumberBlack = ^(double allBagNumber) {
                       NSInteger allBagNumberr = (NSInteger)allBagNumber;
                       weakSelf.backPackNumLabel.text = [NSString stringWithFormat:@"总：%ld 开心",allBagNumberr];
                   };
//             _giftView.selectCellBlack = ^(GiftInfo * _Nonnull selectInfo) {
