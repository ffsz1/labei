...
@interface HJGiftMicView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
- (void)getOnMicUIDs
{
    ...
    //添加全麦位
    HJIMQueueItem *allMic = [[HJIMQueueItem alloc] init];
...
- (NSMutableArray *)getArr
{
    ...
    if (!hadOwner) {
        if (myInfo.uid != ownerInfo.uid) {
            HJIMQueueItem *ownerMic = [[HJIMQueueItem alloc] init];
            ownerMic.position = -1;
            
            HJRoomQueueInfo *queueInfo = [[HJRoomQueueInfo alloc] init];
...
- (void)setSelUID
{
    ...
    if (self.targetUID>0) {
        for (int i = 1; i<self.micArr.count; i++) {
            HJIMQueueItem *info = self.micArr[i];
...
{
    HJGiftMicCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJGiftMicCell" forIndexPath:indexPath];
    
    cell.item = indexPath.item;
    
    if (indexPath.item>0) {
        HJIMQueueItem *info = self.micArr[indexPath.item];
        cell.info = info;
    }else{
        cell.info = nil;
    }
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerNib:[UINib nibWithNibName:@"HJGiftMicCell" bundle:nil] forCellWithReuseIdentifier:@"HJGiftMicCell"];
