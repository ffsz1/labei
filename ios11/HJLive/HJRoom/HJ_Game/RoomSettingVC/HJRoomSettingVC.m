...
@implementation HJRoomSettingVC
{
...
- (void)initView {
    ...
    self.tableView.backgroundColor = [[YYTheme defaultTheme]colorWithHexString:@"#F5F5F5" alpha:1];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        if (indexPath.row == 6) {
            ...
            //背景            
            HJRoomBackgroundSettingVC *viewController = [HJRoomBackgroundSettingVC new];
            @weakify(self);
...
//            [self.navigationController pushViewController:vc animated:YES];
            ...
            UIViewController *vc = HJRoomStoryBoard(@"HJRoomBlackListVC");
            [self.navigationController pushViewController:vc animated:YES];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    HJHomeTag *tag = [self.tagList safeObjectAtIndex:indexPath.row];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HJHomeTag *tag = [self.tagList safeObjectAtIndex:indexPath.row];
    HJRoomTagCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJRoomTagCell" forIndexPath:indexPath];
    ...
    if (tag.id == selectedChildenId) {
        cell.bottomView.backgroundColor = [[YYTheme defaultTheme] colorWithHexString:@"#9F62FB" alpha:1.0];
...
- (void)initData {
    [GetCore(HJHomeCore)requestRoomAllTag];
...
- (void)save {
    ...
    if (![self.roomPasswordTextField.text isEqualToString:self.roomInfo.roomPwd] || ![self.roomnameTextField.text isEqualToString:self.roomInfo.title] || ![self.selectedButton.titleLabel.text isEqualToString:self.roomInfo.roomTag]) {
        ...
        if (GetCore(HJImRoomCoreV2).myMember.is_creator) {
            [GetCore(HJRoomCoreV2Help) updateGameRoomInfo:self.roomInfo.uid backPic:valueStr title:self.roomnameTextField.text  roomTopic:nil roomNotice:nil roomPassword:self.roomPasswordTextField.text tag:selectedChildenId playInfo:GetCore(HJImRoomCoreV2).currentRoomInfo.playInfo giftEffectSwitch:self.giftEffectSwitch.isOn giftCardSwitch:self.carEffectSwitch.isOn publicChatSwitch:GetCore(HJImRoomCoreV2).currentRoomInfo.publicChatSwitch];
        }else if (GetCore(HJImRoomCoreV2).myMember.is_manager) {
            [GetCore(HJRoomCoreV2Help) managerUpdateGameRoomInfo:[GetCore(HJAuthCoreHelp)getUid].userIDValue backPic:valueStr  title:self.roomnameTextField.text roomTopic:nil roomNotice:nil roomPassword:self.roomPasswordTextField.text tag:selectedChildenId playInfo:GetCore(HJImRoomCoreV2).currentRoomInfo.playInfo giftEffectSwitch:self.giftEffectSwitch.isOn giftCardSwitch:self.carEffectSwitch.isOn publicChatSwitch:GetCore(HJImRoomCoreV2).currentRoomInfo.publicChatSwitch];
        }
    } else {
