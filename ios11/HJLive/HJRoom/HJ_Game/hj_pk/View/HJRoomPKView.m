...
@property (strong,nonatomic) HJRoomPKSureView *pkSureView;
@property (strong,nonatomic) HJRoomPKRecordModelView *recordView;
...
@implementation HJRoomPKView

+ (void)show
{
    [HJRoomPKView loadXib:XBDRoomPKTypeSend joinModel:nil];
}

+ (void)showSurePKView:(HJRoomPKJoinModel *)joinModel
{
    [HJRoomPKView loadXib:XBDRoomPKTypeSure joinModel:joinModel];
}

+ (void)showFailView
{
    [HJRoomPKView loadXib:XBDRoomPKTypeFail joinModel:nil];
}

+ (void)loadXib:(XBDRoomPKType)type joinModel:(HJRoomPKJoinModel *)joinModel
{
    HJRoomPKView *shareView = [[NSBundle mainBundle]loadNibNamed:@"HJRoomPKView" owner:self options:nil].lastObject;
...
- (IBAction)surebtnAction:(id)sender {
    ...
    HJRoomPKGiftModel *giftModel = self.giftArr[0];
    ...
    [HJHttpRequestHelper pk_confirmPk:probability choose:choose giftId:giftModel.giftId giftNum:giftModel.giftNum success:^(id  _Nonnull data) {
...
- (HJRoomPKSureView *)pkSureView
{
    if (!_pkSureView) {
        _pkSureView = [[NSBundle mainBundle]loadNibNamed:@"HJRoomPKSureView" owner:self options:nil].lastObject;
...
- (HJRoomPKRecordModelView *)recordView
{
    if (!_recordView) {
        _recordView = [[NSBundle mainBundle]loadNibNamed:@"HJRoomPKRecordModelView" owner:self options:nil].lastObject;
...
- (void)getProbabilityData
{
    ...
    __weak typeof(self)weakSelf = self;
    [HJHttpRequestHelper pk_getProbability:^(NSArray * _Nonnull arr) {
...
- (void)getGiftData
{
    ...
    HJRoomPKProbability *model = _probabilityArr[idnex];
    __weak typeof(self)weakSelf = self;
    [HJHttpRequestHelper pk_getMoraInfo:model.probability success:^(NSArray* arr,int num,int moraTime) {
        weakSelf.giftArr = arr;
        weakSelf.timesLabel.text = [NSString stringWithFormat:@"%d/50",num];
        weakSelf.timeLabel.text = [NSString stringWithFormat:@"若无人参与,开心将在%d分钟内退还",moraTime];
    } failure:^(NSNumber * _Nonnull resCode, NSString * _Nonnull message) {
...
- (void)setProbabilityArr:(NSArray *)probabilityArr
{
    ...
    if (probabilityArr.count>0) {
        ...
        HJRoomPKProbability *model = _probabilityArr[0];
...
- (void)setGiftArr:(NSArray *)giftArr
{
    ...
    if (giftArr.count>0) {
        HJRoomPKGiftModel *model = self.giftArr[0];
    ...
    if (giftArr.count>1) {
        HJRoomPKGiftModel *model = self.giftArr[1];
    ...
    if (giftArr.count>2) {
        HJRoomPKGiftModel *model = self.giftArr[2];
