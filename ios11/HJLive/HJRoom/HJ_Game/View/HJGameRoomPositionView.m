...
#import "HJGameRoomPositionView.h"
#import "HJGameRoomPositionCell.h"
#import "HJGameRoomSpeakingPositionCell.h"
...
@interface HJGameRoomPositionView ()
<
...
@property (strong, nonatomic) NSMutableArray<HJIMQueueItem *> *mircoList;
...
@implementation HJGameRoomPositionView
#pragma mark - life cycle
+ (instancetype)loadFromNib {
    return [[NSBundle mainBundle]loadNibNamed:@"HJGameRoomPositionView" owner:self options:nil].lastObject;
}
- (void)layoutSubviews {
...
- (void)awakeFromNib {
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"HJGameRoomPositionCell" bundle:nil] forCellWithReuseIdentifier:@"HJGameRoomPositionCell"];
    [self.speakingCollectionView registerNib:[UINib nibWithNibName:@"HJGameRoomPositionCell" bundle:nil] forCellWithReuseIdentifier:@"HJGameRoomPositionCell"];
    [self.speakingCollectionView registerNib:[UINib nibWithNibName:@"HJGameRoomSpeakingPositionCell" bundle:nil] forCellWithReuseIdentifier:@"HJGameRoomSpeakingPositionCell"];
...
- (void)updateMicCharm:(NSMutableArray<HJRoomMemberCharmInfoModel *> *)memberCharmInfoList {
    ...
    self.mircoList = GetCore(HJImRoomCoreV2).micQueue;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HJIMQueueItem *item = [self.mircoList safeObjectAtIndex:indexPath.item+1];
    ChatRoomMember *member = item.queueInfo.chatRoomMember;
    HJRoomMicInfo *state = item.queueInfo.mic_info;
    if (collectionView == self.collectionView) {
        HJGameRoomPositionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJGameRoomPositionCell" forIndexPath:indexPath];
        ...
        cell.positionStatusNormalBg.image = [UIImage imageNamed:indexPath.item==7?@"xbd_room_8set":@"xc_room_icon_paimai"];
...
//        cell.nicknameLabel.textColor = [UIColor whiteColor];
        cell.micOrderLabel.textColor = [[YYTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:0.4];
...
//                cell.nicknameLabel.hidden = YES;
                cell.micOrderLabel.backgroundColor = [[YYTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:0.1];
                cell.micOrderLabel.textColor = [[YYTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:0.4];
...
//                    [cell.sexBtn setTitleColor:UIColorHex(ff5c8d) forState:UIControlStateNormal];
                ...
                cell.nicknameLabel.text = member.nick;
                 cell.nickName_leading_layout.constant = (kScreenWidth>380)?45:40;
                 cell.nicknameLabel.textAlignment = NSTextAlignmentLeft;
                cell.micOrderLabel.textColor = [[YYTheme defaultTheme] colorWithHexString:@"#FFFFFF" alpha:1];
        ...
        cell.HJRoomMicInfo = state;
        cell.micOrderLabel.text = [NSString stringWithFormat:@"%d",indexPath.item + 1];
        return cell;
    } else if (collectionView == self.speakingCollectionView) {
        HJGameRoomSpeakingPositionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJGameRoomSpeakingPositionCell" forIndexPath:indexPath];
...
//                HJRippleAnimationView *animationView = [[HJRippleAnimationView alloc]initWithFrame:CGRectMake(0, 0, cell.speakingAnimateImageView.width, cell.speakingAnimateImageView.height) gender:member.gender];
//
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == self.collectionView) {
        UserID myUid = [GetCore(HJAuthCoreHelp)getUid].userIDValue;
        ChatRoomMember *myMember = GetCore(HJRoomQueueCoreV2Help).myMember;
        if (state.posState == JXIMRoomMicPostionStateUnlock) {
            if (member != nil) {//麦序上有人
               [self showMenu:myMember withUid:myUid member:member index:indexPath];
                ...
                if (myMember.is_creator || myMember.is_manager) {
                    ...
                    [GetCore(HJRoomQueueCoreV2Help) upMic:(int)indexPath.row];//上指定麦
                }
            }
...
- (void)showMenu:(ChatRoomMember *)myMember withUid:(UserID)myUid member:(ChatRoomMember *)member index:(NSIndexPath *)indexPath{
...
- (void)cleanLocationCharmData
{
    for (NSString *key in self.charmInfoMap.allKeys) {
        ...
        HJIMQueueItem *hatModel;
        
        for (HJIMQueueItem *item in self.mircoList) {
            ChatRoomMember *member = item.queueInfo.chatRoomMember;
...
- (void)userBeAddBlack:(ChatRoomMember *)member{
    [GetCore(HJImRoomCoreV2) queryQueueWithRoomId:[NSString stringWithFormat:@"%ld",self.roomInfo.roomId]];
}
...
- (void)bekillSuccess {
    ...
    //重新获取最新的麦序
    [GetCore(HJImRoomCoreV2) queryQueueWithRoomId:[NSString stringWithFormat:@"%ld",(long)GetCore(HJImRoomCoreV2).currentRoomInfo.roomId]];
}
...
- (void)reloadMySpeakingView:(UserID)myUID{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        HJIMQueueItem *item = [self.mircoList safeObjectAtIndex:i+1];
        ChatRoomMember *member = item.queueInfo.chatRoomMember;
...
//                HJRippleAnimationView *animationView = [[HJRippleAnimationView alloc]initWithFrame:CGRectMake(0, 0, speakingView.width, speakingView.height) gender:member.gender];
//
//                animationView.delegate = self;
...
//    if (GetCore(RoomQueueCore).isLoad) {
        ...
        for (HJFaceReceiveInfo *item in faceRecieveInfos) {
            NSString *position = [GetCore(HJRoomQueueCoreV2Help) findThePositionByUid:item.uid];
            if (position != nil && item.uid != self.roomInfo.uid) {
                FLAnimatedImageView *imageView = [self findTheImageViewByPosition:position];
                [self bringSubviewToFront:imageView];
                HJFaceConfigInfo *faceInfo = [GetCore(HJFaceCore)findFaceInfoById:item.faceId];
                [[HJFaceImageTool shareFaceImageTool]queryImage:item imageView:imageView success:^(HJFaceReceiveInfo *info) {

                } failure:^(NSError *error) {
                    imageView.transform = CGAffineTransformMakeScale(1, 1);
                }];
                if (item.uid == [GetCore(HJAuthCoreHelp) getUid].userIDValue && faceInfo.resultCount > 0) {
                    GetCore(HJFaceCore).isShowingFace = YES;
                }
            }
        }
}

- (HJFaceReceiveInfo *)findIsNeedPlayFaceBy:(UserID)uid {
    for (HJFaceReceiveInfo *item in self.faceArr) {
        if (item.uid == uid) {
            return item;
        }
    }
...
- (void)onReceiveLongZhuWithModel:(HJRoomLongZhuMsgModel *)model isSuPei:(BOOL)isSuPei {
    ...
    NSString *position = [GetCore(HJRoomQueueCoreV2Help) findThePositionByUid:model.uid.integerValue];
...
- (void)showBalanceNotEnough {
    ...
    [[HJRoomViewControllerCenter defaultCenter].current presentViewController:alert animated:YES completion:^{
    ...
    if (myMember.is_creator || myMember.is_manager) {
        if (state.micState == JXIMRoomMicPostionStateLock) {
            [alter addAction:[UIAlertAction actionWithTitle:@"解除此座位禁麦" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [GetCore(HJRoomQueueCoreV2Help) openMic:(int)indexPath.row];
            }]];
        }
    }
...
//        })];
        if (!member.is_creator) {
            ...
            [alter addAction:[UIAlertAction actionWithTitle:@"抱Ta下麦" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [GetCore(HJRoomQueueCoreV2Help) kickDownMic:uid position:indexPath.row];
            }]];
...
//            })];
            ...
            if (!member.is_creator) {
                ...
                [alter addAction:[UIAlertAction actionWithTitle:@"禁麦此座位" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                    [GetCore(HJRoomQueueCoreV2Help) closeMic:(int)indexPath.row];
                }]];
...
//                [GetCore(HJRoomQueueCoreV2Help) removeChatroomQueueWithPosition:[NSString stringWithFormat:@"%ld",(long)indexPath.row] success:^(BOOL success) {
//                    [GetCore(HJRoomQueueCoreV2Help) lockMicPlace:indexPath.row];
...
//                [GetCore(HJRoomQueueCoreV2Help) freeMicPlace:(int)indexPath.row];
//
//
//            }]];
...
//        HJOnlineListVC *vc =(HJOnlineListVC *)[[HJRoomViewControllerFactory sharedFactory]instantiateOnlineListViewController];
//        vc.pos = (int)indexPath.row;
//        [[HJRoomViewControllerCenter defaultCenter].current pushViewController:vc animated:YES];
        
        HJRoomOnlineListVC *vc = HJRoomStoryBoard(@"HJRoomOnlineListVC");
        vc.pos = (int)indexPath.row;
        [[HJRoomViewControllerCenter defaultCenter].current pushViewController:vc animated:YES];
    ...
    if (!myMember.is_creator) {
        ...
        [alter addAction:[UIAlertAction actionWithTitle:@"移动到此座位" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            if(state.posState == JXIMRoomMicPostionStateUnlock){
                [GetCore(HJRoomQueueCoreV2Help) upMic:(int)indexPath.row];
            }else{
...
- (void)showOwnerPanelSheet:(UserID)uid indexPath:(NSIndexPath *)indexPath member:(ChatRoomMember *)member {
...
//    })];
    ...
    [alter addAction:[UIAlertAction actionWithTitle:@"抱Ta下麦" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [GetCore(HJRoomQueueCoreV2Help) kickDownMic:uid position:(int)indexPath.row];
    }]];
...
- (void)saveThePositionInCore {
    ...
    [GetCore(HJRoomCoreV2Help) savePosition:positionArr];
...
{
    NSArray *speakerList = GetCore(HJRoomCoreV2Help).speakingList;
...
- (CGPoint)findThePositionCenterByPosition:(NSString *)position {
    ...
    HJGameRoomPositionCell *cell = (HJGameRoomPositionCell *)[self.collectionView cellForItemAtIndexPath:indexPath];
    if (cell != nil) {
        return cell.center;
    }else {
...
//            [_speakingAnimatePicArr addObject:[UIImage imageNamed:[NSString stringWithFormat:@"hj_room_speak_1_0000%d",i]]];
        }
    }
...
- (NSMutableArray *)speakingAnimatePicArrForGirl {
    if (_speakingAnimatePicArrForGirl == nil && _speakingAnimatePicArrForGirl.count == 0) {
        ...
        for (int i=0; i<30; i++) {
            ...
            [_speakingAnimatePicArrForGirl addObject:[UIImage imageNamed:[NSString stringWithFormat:@"xc_room_speak_1_0000%d",i]]];//@"hj_room_speak_1_0000%d"
        }
    }
