...
@property (nonatomic, strong) HJMICMatchRecordView *recordView;
...
@implementation HJMICMatchUserView
...
#pragma mark - Event
- (void)recordAction:(HJMICMatchRecordView*)matchRecordView {
...
//    if (isAttenion) {
//        attentionName = @"hj_mic_icon_attentioned";
//    } else {
//        attentionName = @"hj_mic_icon_attention";
//    }
...
- (void)initData
{
    ...
    if (_userInfo.gender == UserInfo_Male) {
        sexName = @"hj_sex_male_logo";
        sexColor = @"#00B7EE";
    } else {
        sexName = @"hj_sex_female_logo";
        sexColor = @"#FF9CC8";
    }
    
    UserID myUserId = [[GetCore(HJAuthCoreHelp) getUid] userIDValue];
...
- (UIImageView *)charmLevelimageView {
    if (!_charmLevelimageView) {
        ...
        _charmLevelimageView.image = [UIImage imageNamed:@"hj_meili_charm_icon"];
...
- (UIImageView *)experLevelimageView {
    if (!_experLevelimageView) {
        ...
        _experLevelimageView.userInteractionEnabled = YES;
         _experLevelimageView.image = [UIImage imageNamed:@"hj_caifu_expert_icon"];
...
- (UIImageView *)flagImageView {
    if (!_flagImageView) {
        ...
        _flagImageView.userInteractionEnabled = YES;
         _flagImageView.image = [UIImage imageNamed:@"hj_my_hongxin_icon"];
...
- (HJMICMatchRecordView *)recordView {
    if (!_recordView) {
        _recordView = [HJMICMatchRecordView new];
        _recordView.exclusiveTouch = YES;
        @weakify(self);
        _recordView.didTapHandler = ^(HJMICMatchRecordView *matchRecordView){
            @strongify(self);
...
- (UIButton *)sexView {
    if (!_sexView) {
        ...
        [_sexView setImage:[UIImage imageNamed:@"hj_mic_icon_male"] forState:UIControlStateNormal];
...
- (UIButton *)attentionView {
    if (!_attentionView) {
        ...
        [_attentionView setBackgroundImage:[UIImage imageNamed:@"hj_match_zhuye"] forState:UIControlStateNormal];
