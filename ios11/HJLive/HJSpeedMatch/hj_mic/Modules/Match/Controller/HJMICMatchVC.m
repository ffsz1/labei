...
@interface HJMICMatchVC () <HJMICCoreClient>
...
- (void)skipToMatchUser {
    if (![[YYViewControllerCenter currentViewController] isKindOfClass:[HJMICMatchVC class]]) return;
    
    HJMICMatchUserVC *vc = [HJMICMatchUserVC new];
...
- (void)updateUsers {
    ...
    HJMICUserInfo *leftUser = nil;
    HJMICUserInfo *rightUser = nil;
    HJMICUserInfo *bottomUser = nil;
...
- (void)updateControls {
    ...
    HJMICUserInfo *user = [self.users objectOrNilAtIndex:0];
...
- (SVGAImageView *)animationView {
    if (!_animationView) {
        ...
        _animationView.imageName = @"hj_mic_effect_match_radio";
...
- (UIImageView *)matchTipsView {
    if (!_matchTipsView) {
        ...
        _matchTipsView.image = [UIImage imageNamed:@"hj_mic_icon_match_tips"];
...
- (UIImageView *)recordTipsView {
    if (!_recordTipsView) {
        ...
        _recordTipsView.image = [UIImage imageNamed:@"hj_mic_icon_record_tips"];
