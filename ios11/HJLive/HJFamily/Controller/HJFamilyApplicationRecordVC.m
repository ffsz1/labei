...
#import "HJFamilyApplicationRecordCell.h"
...
@interface HJFamilyApplicationRecordVC ()<UITableViewDataSource, UITableViewDelegate, HJFamilyCoreClient>

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray<HJFamilyMessage *> *records;
...
@implementation HJFamilyApplicationRecordVC
...
- (void)loadRecords {
    [GetCore(HJFamilyCore) familyGetFamilyMessageWithFamilyId:self.familyId page:self.page count:self.count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    HJFamilyApplicationRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:HJFamilyApplicationRecordCellID forIndexPath:indexPath];
...
- (void)applyUser:(XCFamilyApplicationStatus)status atIndexPath:(NSIndexPath *)indexPath {
    HJFamilyMessage *buffer = [self.records objectOrNilAtIndex:indexPath.row];
    self.selectedIndexPath = [NSIndexPath indexPathForRow:indexPath.row inSection:indexPath.section];
    self.currentStatus = status;
    [GetCore(HJFamilyCore) familyApplyFamilyWithFamilyId:self.familyId userId:[@(buffer.uid) description] type:buffer.type status:status];
}
...
- (void)configureCell:(HJFamilyApplicationRecordCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    HJFamilyMessage *record = [self.records objectOrNilAtIndex:indexPath.row];
...
- (NSMutableArray<HJFamilyMessage *> *)records {
    if (!_records) {
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[HJFamilyApplicationRecordCell class] forCellReuseIdentifier:HJFamilyApplicationRecordCellID];
