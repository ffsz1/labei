...
#import "HJFamilyMessageController.h"
#import "XCFamilyMessageCell.h"
...
@interface HJFamilyMessageController ()<UITableViewDataSource,UITableViewDelegate,HJFamilyCoreClient>
...
@implementation HJFamilyMessageController
...
- (void)familyApplyFamilySuccess {
    ...
    for (HJFamilyMessage *msg in self.dataArr) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XCFamilyMessageCell *cell = [tableView dequeueReusableCellWithIdentifier:@"XCFamilyMessageCell"];
    
    HJFamilyMessage *model = self.dataArr[indexPath.row];
    cell.model = model;
    ...
    [cell setAgreeBtnActionBlock:^{
        ...
        if (self.erbanNo == 0) {
            ...
            [GetCore(HJFamilyCore) familyApplyFamilyWithFamilyId:GetCore(HJFamilyCore).myFamily.familyId userId:[@(model.uid) description] type:model.type status:1];
        }
    }];
    ...
    [cell setRefuseBtnActionBlock:^{
        ...
        if (self.erbanNo == 0) {
            [MBProgressHUD showMessage:@""];
            self.erbanNo = model.erbanNo;
            [GetCore(HJFamilyCore) familyApplyFamilyWithFamilyId:GetCore(HJFamilyCore).myFamily.familyId userId:[@(model.uid) description] type:model.type status:2];
        }
    }];
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"XCFamilyMessageCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"XCFamilyMessageCell"];
    }
    return _tableView;
}
