//
//  HJFamilyManageViewController.m
//  HJLive
//
...
#import "HJFamilyApplicationRecordVC.h"

#import "HJFamilyManageItemCell.h"
...
@interface HJFamilyManageViewController ()<UITableViewDataSource, UITableViewDelegate>

@property (nonatomic, strong) UITableView *tableView;

@property (nonatomic, strong) NSMutableArray<HJFamilyManageItemModel *> *items;

@end

@implementation HJFamilyManageViewController
...
#pragma mark - Event
- (void)exitFamily {
    HJFamilyAlertView *exitView  = [[HJFamilyAlertView alloc] initWithFrame:CGRectMake(0, 0, 290, 199)];
    ...
    exitView.didTapActionHandler = ^{
        ...
        [GetCore(HJFamilyCore) applyExitFamilyWithFamilyId:self.familyInfo.familyId];
    };
...
- (void)skipToManageFamily {
    HJFamilySettingVC *viewController = [[HJFamilySettingVC alloc] init];
    viewController.familyInfo = self.familyInfo;
    [self.navigationController pushViewController:viewController animated:YES];
}

- (void)skipToApplicationRecord {
    HJFamilyApplicationRecordVC *viewController = [[HJFamilyApplicationRecordVC alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    HJFamilyManageItemCell *cell = [tableView dequeueReusableCellWithIdentifier:HJFamilyManageItemCellID forIndexPath:indexPath];
    [self configureCell:cell atIndexPath:indexPath];
    return cell;
}

- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    HJFamilyManagerHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:HJFamilyManagerHeaderViewID];
    [self configureHeader:headerView inSection:section];
    return headerView;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    HJFamilyManageItemModel *item = [self.items objectOrNilAtIndex:indexPath.row];
    switch (item.type) {
...
- (void)configureCell:(HJFamilyManageItemCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    HJFamilyManageItemModel *item = [self.items objectOrNilAtIndex:indexPath.row];
...
- (void)configureHeader:(HJFamilyManagerHeaderView *)header inSection:(NSInteger)section {
...
- (void)setupDatas {
    ...
    switch (self.familyInfo.roleStatus) {
        ...
        {
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeManageFamily enable:YES]];
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeApplicationRecord enable:YES]];
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeMessageNotice enable:open]];
        }
            break;
        case XCFamilyRoleStatusManager:
        {
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeManageFamily enable:YES]];
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeApplicationRecord enable:YES]];
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeExit enable:YES]];
            [self.items addObject:[HJFamilyManageItemModel itemWithType:XCFamilyManageItemTypeMessageNotice enable:open]];
        }
            break;
...
- (NSMutableArray<HJFamilyManageItemModel *> *)items {
    if (!_items) {
        _items = @[].mutableCopy;
    }
    return _items;
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[HJFamilyManageItemCell class] forCellReuseIdentifier:HJFamilyManageItemCellID];
        [_tableView registerClass:[HJFamilyManagerHeaderView class] forHeaderFooterViewReuseIdentifier:HJFamilyManagerHeaderViewID];
