...
#import "HJUserCache.h"
#import "JPUSHService.h"

@interface HJUserCoreHelp ()<HJAuthCoreClient>

@end


@implementation HJUserCoreHelp
- (instancetype)init
{
    self = [super init];
    if (self) {
        ...
        [[self rac_signalForSelector:@selector(onLoginSuccess) fromProtocol:@protocol(HJAuthCoreClient)] subscribeNext:^(RACTuple *tuple) {
            ...
            [JPUSHService setAlias:[GetCore(HJAuthCoreHelp) getUid] completion:^(NSInteger iResCode, NSString *iAlias, NSInteger seq) {
...
- (void)getVoiceText:(UserID)uid {
    [HJHttpRequestHelper getVoiceTextWithUid:uid success:^(NSString *text) {
...
- (RACSignal *)saveUserInfoWithUserID:(UserID)userId userInfos:(NSDictionary *)userInfos
{
    ...
    return [[RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        [HJHttpRequestHelper updateUserInfo:userId userInfos:userInfos success:^(UserInfo *userInfo) {
...
{
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        [HJHttpRequestHelper getUserInfo:userId success:^(UserInfo *userInfo) {
            [subscriber sendNext:userInfo];
            [subscriber sendCompleted];
        } failure:^(NSNumber *resCode, NSString *message) {
            [subscriber sendError:CORE_API_ERROR(UserCoreErrorDomain, resCode.integerValue, message)];
...
{
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        [HJHttpRequestHelper getUserInfos:userIdList success:^(NSArray *userInfoList) {
...
- (RACSignal *)getUserInfoByUid:(UserID)uid refresh:(BOOL)refresh {
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        [[[HJUserCache shareCache] getUserInfoFromCacheWith:uid] subscribeNext:^(id x) {
            if (x) { //缓存或者DB里面有
...
- (void)checkUserIsBanned:(NSString *)uid {
    //    if (userId == 0) {return;}
    [HJHttpRequestHelper checkUserHasBannedWith:[uid longLongValue] success:^(id data){
        @try {
...
- (void)getReceiveGift:(UserID)userId orderType:(OrderType)orderType type:(NSInteger)type {
    if (userId > 0) {
        ...
        [HJHttpRequestHelper getReceiveGift:userId orderType:orderType type:type success:^(NSArray * userGiftList){
...
- (void)uploadImageUrlToServer:(NSString *)url {
    [HJHttpRequestHelper uploadImageURLToServerWithURL:url success:^(BOOL isSuccess) {
...
- (void)deleteImageUrlToServerWithPid:(NSString *)pid {
    [HJHttpRequestHelper deleteImageToServerWithpid:pid success:^(BOOL isSuccess) {
...
- (void)_syncServerUserInfo:(UserID)userId success:(void (^)(UserInfo *))success
{
    [[self requestUserInfo:userId] subscribeNext:^(id x) {
        ...
        [[HJUserCache shareCache] saveUserInfo:newUserInfo];
        success(newUserInfo);
    }];
}
...
- (void)userReportSaveWithUid:(NSInteger)uid
    ...
    [HJHttpRequestHelper userReportSaveWithUid:uid reportType:reportType type:type phoneNo:phoneNo success:^{
 ...
 */
- (void)roomRcmdGet {
    [HJHttpRequestHelper roomRcmdGetWithsuccess:^(NSDictionary *result) {
...
- (void)userSettingSaveWithLikedSend:(BOOL)likedSend
    ...
    [HJHttpRequestHelper userSettingSaveWithLikedSend:likedSend success:success failure:failure];
}
...
- (void)userSettingV1GetWithSuccess:(void (^)(BOOL likedSend))success failure:(void (^)(NSNumber *resCode, NSString *message))failure {
    ...
    [HJHttpRequestHelper userSettingV1GetWithSuccess:success failure:failure];
}
...
- (void)getRichLevel {
    ...
    [HJHttpRequestHelper getRichLevel:^(HJLevelModel *model) {
...
- (void)getMeiliLevel {
    ...
    [HJHttpRequestHelper getMeiliLevel:^(HJLevelModel *model) {
