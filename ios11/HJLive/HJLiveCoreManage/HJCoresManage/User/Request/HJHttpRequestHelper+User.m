...
@implementation HJHttpRequestHelper (User)
...
+ (void)checkUserHasBannedWith:(NSInteger)uid success:(void (^)(id data))success
    ...
    NSString *userUid = [GetCore(HJAuthCoreHelp) getUid];
...
//        params[@"ticket"] = [GetCore(HJAuthCoreHelp) getTicket];
    ...
    [HJHttpRequestHelper GET:method params:params success:^(id data) {
        NSArray * userGifts = [NSArray yy_modelArrayWithClass:[UserGift class] json:data];
        if (success) {
            success(userGifts);
        }
    } failure:^(NSNumber *resCode, NSString *message) {
             ...
             failure:(void (^)(NSNumber *resCode, NSString *message))failure
{
    ...
    [HJHttpRequestHelper GET:method params:params success:^(id data) {
        HJLevelModel *userInfo = [HJLevelModel yy_modelWithDictionary:data];
        if (success) {
            success(userInfo);
        }
    } failure:^(NSNumber *resCode, NSString *message) {
                ...
                failure:(void (^)(NSNumber *resCode, NSString *message))failure
{
    ...
    [params setObject:@(userId) forKey:@"uid"];
    [params setObject:ticket forKey:@"ticket"];
    if (GetCore(HJLinkedMeCore).linkme) {
        if (GetCore(HJLinkedMeCore).linkme.roomuid != nil) {
            [params setObject:GetCore(HJLinkedMeCore).linkme.roomuid forKey:@"roomUid"];
        }
        if (GetCore(HJLinkedMeCore).linkme.uid != nil) {
            [params setObject:GetCore(HJLinkedMeCore).linkme.uid forKey:@"shareUid"];
        }
        if (GetCore(HJLinkedMeCore).channel != nil) {
            [params setObject:GetCore(HJLinkedMeCore).channel forKey:@"shareChannel"];
        }
    }
...
+ (void)userReportSaveWithUid:(NSInteger)uid
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"uid"] = @(uid);
    params[@"reportUid"] = @([GetCore(HJAuthCoreHelp) getUid].integerValue);
