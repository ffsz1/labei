...
@implementation HJHttpRequestHelper (Bill)
...
+ (void)getChatBillWithPageNo:(NSInteger)pageNo
    ...
    [HJHttpRequestHelper GET:method params:params success:^(id data) {
        ...
        for (NSDictionary *item in billList) {
            NSArray *subKeys = [item allKeys];
            NSString *key = subKeys[0];
            NSArray *oneDayArr = [NSArray yy_modelArrayWithClass:[HJChatBillInfo class] json:[item objectForKey:key]];
            NSString *finallyKey = [PLTimeUtil getDateWithYYMMDD:key];
            [keysList addObject:finallyKey];
            [dataArr addObject:oneDayArr];
        }
...
+ (void)getRechargeBillWithPageNo:(NSInteger)pageNo
    ...
    [HJHttpRequestHelper GET:method params:params success:^(id data) {
        ...
        for (NSDictionary *item in billList) {
            NSArray *subKeys = [item allKeys];
            NSString *key = subKeys[0];
            NSArray *oneDayArr = [NSArray yy_modelArrayWithClass:[HJRechargeBillInfo class] json:[item objectForKey:key]];
            NSString *finallyKey = [PLTimeUtil getDateWithYYMMDD:key];
            [keysList addObject:finallyKey];
            [dataArr addObject:oneDayArr];
        }
