...
//请求首页tag
- (void)requestHomeTag{
    [HJHttpRequestHelper requestHomeTag:^(NSArray *list) {
...
- (void)requestRoomAllTag{
    [HJHttpRequestHelper requestRoomAllTag:^(NSArray *list) {
...
- (void)requestHomeHotDataState:(int)state page:(int)page{
    [HJHttpRequestHelper requestHomeHotDataState:(int)state page:(int)page success:^(NSMutableDictionary *dictionary) {
...
- (void)requestHomeCommonData:(int)type state:(int)state page:(int)page{
    [HJHttpRequestHelper requestHomeCommonData:(int)type state:(int)state page:(int)page success:^(NSArray *list,int type) {
...
- (void)requestHomeOtherMenuData {
    [HJHttpRequestHelper requestHomeOtherMenuDataSuccess:^(NSArray *arr) {
...
- (void)getHomeGetindexWithPage:(NSInteger)page
    ...
    [HJHttpRequestHelper getHomeGetindexWithPage:page pageSize:pageSize success:^(NSArray *dataArr) {
...
- (void)getHomeAttentionRecommendData {
    [HJHttpRequestHelper requestAttentionRecommendListWithSuccess:^(NSArray<HJHomeRoomInfo *> *data) {
...
- (void)getMyAttentionDataWithPage:(NSInteger)page count:(NSInteger)count {
    [HJHttpRequestHelper requestAttentionWithPageNum:page count:count success:^(NSArray<HJHomeRoomInfo *> *data) {
...
- (void)setupAttentionRoom:(NSInteger)roomId
    ...
    [HJHttpRequestHelper requestRoomAttention:roomId success:^(id data) {
        !success ?: success();
    } failure:^(NSNumber *resCode, NSString *message) {
...
- (void)removeAttentionRoom:(NSString *)roomId
                    success:(void (^)(void))success
                    failure:(void (^)(NSNumber *resCode, NSString *message))failure {
    [HJHttpRequestHelper requestDelRoomAttention:roomId success:^(id data) {
        !success ?: success();
    } failure:^(NSNumber *resCode, NSString *message) {
        !failure ?: failure(resCode, message);
    }];
