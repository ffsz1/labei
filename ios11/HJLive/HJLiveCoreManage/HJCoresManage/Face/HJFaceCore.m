...
- (instancetype)init {
    ...
    if (self) {
        ...
        _faceInfos = [HJFaceInfoStorage getFaceInfos];
...
- (void)requestFaceJson {
    ...
    [HJHttpRequestHelper getTheFaceJson:^(NSArray *faceJsonList) {
        @strongify(self);
...
- (void)faceDownloadManager {
    ...
    if (version) {
        if ([version integerValue] < [self.version integerValue]) {
            ...
            HJFaceConfigInfo *info = nil;
            NSArray *faces = [self getFaceInfos];
            if (faces.count) {
                NSMutableArray<HJFaceConfigInfo *> *faceInfos = [faces firstObject];
                if (faceInfos.count) {
...
- (void)syncTheTmpJsonToNormalJson {
    ...
    [HJFaceInfoStorage saveFaceInfos:[self.faceInfos yy_modelToJSONString]];
}
...
//            [tempArr addObject:item];
            ...
            if (faceArr1.count) {
                ...
                if (faceArr2.count) {
                    ...
                    HJFaceConfigInfo *configInfo = [faceArr2 firstObject];
...
- (HJFaceConfigInfo *)findFaceInfoById:(NSInteger)faceId{
    if (self.faceInfos != nil) {
        for (int i = 0; i < self.faceInfos.count; i++) {
            HJFaceConfigInfo *faceInfo = self.faceInfos[i];
...
- (UIImage *)findFaceIconImageById:(NSInteger)faceId {
    HJFaceConfigInfo *configInfo = [self findFaceInfoById:faceId];
    NSString *faceName = [NSString stringWithFormat:@"%@_%d_%ld",configInfo.pinyin,configInfo.id,configInfo.iconPos];
    UIImage  *face;
...
- (NSMutableArray *)sortFaceInfosWithfaceInfoArr:(NSMutableArray *)faceInfoArr {
    ...
    for (HJFaceConfigInfo *item in faceInfoArr) {
...
- (NSMutableArray *)hideFaceWithInfo:(NSArray *)hideList WithFaceInfos:(NSMutableArray *)infos {
    for (NSString *faceId in hideList) {
        ...
        for (HJFaceConfigInfo *items in infos) {
...
- (NSMutableArray *)getFaceInfos {
    ...
    for (int i = 0; i < self.faceInfos.count; i++) {
        HJFaceConfigInfo *item = self.faceInfos[i];
        if (!item.isNobleFace) {
            [arr addObject:item];
...
- (void)sendAllFace:(HJFaceConfigInfo *)faceInfo {
    if (faceInfo) {
        if (GetCore(HJImRoomCoreV2).isInRoom) {
            NSMutableArray *chatroomMembers = GetCore(HJImRoomCoreV2).micMembers;
            NSMutableArray *faceRecieveInfos = [NSMutableArray array];
            //        HJFaceConfigInfo *child = [[HJFaceConfigInfo alloc]init];
            //麦序成员的配置
            if (chatroomMembers != nil && chatroomMembers.count > 0) {
                for (int i = 0; i < chatroomMembers.count; i++) {
                    HJFaceReceiveInfo *faceRecieveInfo = [[HJFaceReceiveInfo alloc]init];
            ...
            //        }
            //
            HJFaceSendInfo *sendInfo = [[HJFaceSendInfo alloc]init];
            sendInfo.data = faceRecieveInfos;
            //        sendInfo.uid = info.uid;
            sendInfo.uid = [GetCore(HJAuthCoreHelp)getUid].userIDValue;
...
- (void)sendFace:(HJFaceConfigInfo *)faceInfo {
    ChatRoomInfo *info = GetCore(HJImRoomCoreV2).currentRoomInfo;
    if (info != nil) {
        HJFaceReceiveInfo *faceRecieveInfo = [[HJFaceReceiveInfo alloc]init];
        ...
        if (obj.attachment != nil && [obj.attachment isKindOfClass:[Attachment class]]) {
            ...
            if (attachment.first == Custom_Noti_Header_Face) {
                if (attachment.second == Custom_Noti_Sub_Face_Send) {
                    ...
                    //                    HJFaceReceiveInfo *faceRecieveInfo = arr.firstObject;
                    //                    FaceInfo *faceInfo = [self findFaceInfoById:faceRecieveInfo.faceId];
                    ...
                    if (arr.count > 0) {
                        HJFaceReceiveInfo *faceRecieveInfo = arr.firstObject;
                        HJFaceConfigInfo *faceInfo = [self findFaceInfoById:faceRecieveInfo.faceId];
                        if (faceInfo.id > 1 && faceInfo != nil && faceRecieveInfo.resultIndexes.count > 0) {
                            HJFacePlayInfo *playInfo = [[HJFacePlayInfo alloc]init];
                            playInfo.delay = 3;
                            playInfo.HJFaceReceiveInfo = arr.firstObject;
                            playInfo.message = msg;
                            for (HJFaceReceiveInfo *item in arr) {
                                if (item.uid == [GetCore(HJAuthCoreHelp) getUid].userIDValue) {
        ...
        if (obj.attachment != nil && [obj.attachment isKindOfClass:[Attachment class]]) {
            ...
            if (attachment.first == Custom_Noti_Header_Face) {
                if (attachment.second == Custom_Noti_Sub_Face_Send) {
                    ...
                    //                        HJFacePlayInfo *playInfo = [[HJFacePlayInfo alloc]init];
                    //                        playInfo.delay = 3;
                    //                        playInfo.HJFaceReceiveInfo = faceRecieveInfo;
                    //                        [self handleFaceMessage:playInfo];
                    //                    }
                    if (arr.count > 0) {
                        HJFaceReceiveInfo *faceRecieveInfo = arr.firstObject;
                        ...
                        //                        [GetCore(FaceCore)findFaceInfoById:info.HJFaceReceiveInfo.faceId].children
                        
                        //                        FaceInfo *faceInfo = [self findFaceInfoById:]
...
- (void)handleFaceMessage:(HJFacePlayInfo *)HJFacePlayInfo {
    [self.receiveFace addObject:HJFacePlayInfo];
    if (self.timer== nil) {
...
- (void)startTheFaceQueueScanner {
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        if (self.receiveFace.count > 0) {
            ...
            for (HJFacePlayInfo *item in temp) {
...
- (BOOL)isEnableFace:(HJFaceConfigInfo *)info {
