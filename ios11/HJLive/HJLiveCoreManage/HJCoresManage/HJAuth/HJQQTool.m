...
- (void)getUserInfoResponse:(APIResponse*)response
{
    ...
    HJQqUserInfo *info = [HJQqUserInfo yy_modelWithJSON:message];
    info.openID = [self.oauth.openId copy];
    info.token = [self.oauth.accessToken copy];
    self.qqInfo = info;
    
    GetCore(HJAuthCoreHelp).qqInfo = info;
    GetCore(HJAuthCoreHelp).info = nil;
    
    [GetCore(HJAuthCoreHelp) loginWithOpenID:info.openID andUnionID:nil access_token:info.token andType:XCThirdPartLoginQQ];
}
