...
@interface HJMeetingCore()<HJImRoomCoreClient, AgoraRtcEngineDelegate>
@property(nonatomic, strong) NSMutableArray *speakingArray;
@property(nonatomic, assign) BOOL isSpeaking;
@end

@implementation HJMeetingCore
...
//    getAgoraKeyWith
    ...
    NSUInteger uid = [[GetCore(HJAuthCoreHelp) getUid] integerValue];
//    [self.engine enableAudioVolumeIndication:600 smooth:3];
    ...
    [HJHttpRequestHelper getAgoraKeyWith:name uid:[GetCore(HJAuthCoreHelp) getUid] success:^(NSString *token) {
...
- (void)fetchChatRoomMembers:(NSArray *)member {
    ...
    request.roomId = [NSString stringWithFormat:@"%lld",GetCore(HJImRoomCoreV2).currentRoomInfo.roomId];
    request.limit = 200;
    @weakify(self);
        ...
        for (AgoraRtcAudioVolumeInfo *userInfo in report) {
            ...
            if (uid.userIDValue == 0) {
                if (userInfo.volume > 30) {
                    [uids addObject:@([GetCore(HJAuthCoreHelp) getUid].userIDValue)];
                }
            }else {
                NSArray* mircoList = GetCore(HJImRoomCoreV2).micMembers;
                BOOL isOnMic = NO;
                for (ChatRoomMember* model in mircoList) {
