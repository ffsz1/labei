...
@interface HJImFriendCore()<HJImLoginCoreClient, NIMUserManagerDelegate, HJNotificationCoreClient,NIMSystemNotificationManagerDelegate,HJPraiseCoreClient>

@end

@implementation HJImFriendCore
{
...
- (void)sendInviteMessageWithUid:(NSString *)uid {
    NSString *bg = GetCore(HJImRoomCoreV2).roomOwnerInfo.avatar;
    NSString *avatar = GetCore(HJImRoomCoreV2).roomOwnerInfo.avatar;
    NSString *rTitle = GetCore(HJImRoomCoreV2).currentRoomInfo.title;
    if (rTitle.length > 6) {
    ...
    NSString *roomid = [NSString stringWithFormat:@"%lld",GetCore(HJImRoomCoreV2).currentRoomInfo.uid];
    ...
    [GetCore(HJImMessageCore) sendCustomMessageAttachement:attachement sessionId:[NSString stringWithFormat:@"%@",uid] type:NIMSessionTypeP2P];
}
...
- (void) updateMyFriends
{
    ...
    if (friends.count > 0) {
        ...
        [GetCore(HJUserCoreHelp) getUserInfos:uids refresh:YES success:^(NSArray *infoArr) {
...
- (void) updateBlackList
{
    ...
    if (blackList.count > 0) {
        for (NIMUser *user in blackList) {
            NSString *uid = user.userId;
            [GetCore(HJUserCoreHelp)getUserInfo:uid.userIDValue refresh:YES success:^(UserInfo *info) {
...
//            UserInfo *info = [GetCore(UserCore) getUserInfo:uid.userIDValue refresh:NO];
            [GetCore(HJUserCoreHelp)getUserInfo:uid.userIDValue refresh:NO success:^(UserInfo *info) {
