//
//  HJIMRequestManager+PublicRoom.m
//  HJLive
//
...
@implementation HJIMRequestManager (PublicRoom)
...
+ (void)enterPublicWithRoomId:(NSString *)roomId
    ...
    [GetCore(WebSocketCore) send:JX_IM_ROUTE_ENTER_PUBLIC_ROOM content:params success:^(id data) {
...
+ (void)sendPublicRoomMessage:(NSString *)roomId
    ...
    [GetCore(WebSocketCore) send:JX_IM_ROUTE_SEND_PUBLIC_ROOM_MESSAGE content:params success:^(id data) {
        !success ?: success();
    } failure:^(NSInteger code, NSString *errmsg) {
        !failure ?: failure(code, errmsg);
    }];
...
+ (void)getPublicRoomMembersWithRoomId:(NSString *)roomId
    ...
    [HJHttpRequestHelper IM_POST:JX_IM_ROUTE_PUBLIC_ROOM_INFO params:params success:^(id data) {
        HJIMPublicRoomInfo *roomInfo = [HJIMPublicRoomInfo yy_modelWithJSON:data];
