...
#import "HJMySpaceCarModel.h"
#import "HJMySpaceHeadWearModel.h"

@interface HJMySpaceGiftWallVC ()<UICollectionViewDelegateFlowLayout,UIScrollViewDelegate,UICollectionViewDataSource>
...
@implementation HJMySpaceGiftWallVC


- (void)viewDidLoad {
    [super viewDidLoad];
    
    [self.collectionView registerNib:[UINib nibWithNibName:@"HJMySpaceGiftCCell" bundle:nil] forCellWithReuseIdentifier:@"HJMySpaceGiftCCell"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    HJMySpaceGiftCCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJMySpaceGiftCCell" forIndexPath:indexPath];
    
    if (self.type == 0) {
        UserGift *model = self.dataArr[indexPath.item];
    ...
    if (self.type == 1) {
        HJMySpaceHeadWearModel *model = self.headArr[indexPath.item];
        [cell.logoImageView qn_setImageImageWithUrl:model.picUrl placeholderImage:placeholder_image_square type:ImageTypeRoomGift];
        cell.numLabel.text = @"";
    ...
    if (self.type == 2) {
        HJMySpaceCarModel *model = self.carArr[indexPath.item];
        [cell.logoImageView qn_setImageImageWithUrl:model.picUrl placeholderImage:placeholder_image_square type:ImageTypeRoomGift];
        cell.numLabel.text = @"";
...
- (void)getData
{
    [HJHttpRequestHelper getReceiveGift:self.userID orderType:2 type:0 success:^(NSArray * userGiftList){
...
- (void)getCarListData
{
    ...
    [HJHttpRequestHelper getUserCarList:self.userID PageNum:@"1" PageSize:@"50" success:^(NSArray *list) {
        
        if (list.count>0) {
            NSMutableArray *tmpArr = [[NSMutableArray alloc] init];
            for (NSDictionary *dict in list) {
                HJMySpaceCarModel *model = [HJMySpaceCarModel yy_modelWithJSON:dict];
...
- (void)getHeadListData
{
    ...
    __weak typeof(self)weakSelf = self;
    [HJHttpRequestHelper getUserHeadList:self.userID PageNum:@"1" PageSize:@"50" success:^(NSArray *list) {
        
        if (list.count>0) {
            NSMutableArray *tmpArr = [[NSMutableArray alloc] init];
            for (NSDictionary *dict in list) {
                HJMySpaceHeadWearModel *model = [HJMySpaceHeadWearModel yy_modelWithJSON:dict];
                [tmpArr addObject:model];
            }
