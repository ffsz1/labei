...
@implementation HJYouthAlterView

+ (void)updateYouthData
{
    NSString *uidKey = [NSString stringWithFormat:@"%@:isOnYouthSet",[GetCore(HJAuthCoreHelp) getUid]];
    
    [HJHttpRequestHelper getUsersTeensMode:^(BOOL hadSet) {
...
+ (void)check
{
    ...
    NSString *dayKey = [NSString stringWithFormat:@"%@:showYouthAlter",[GetCore(HJAuthCoreHelp) getUid]];
...
+ (void)checkHadShowAlter
{
    ...
    if (adView) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [HJYouthAlterView showAnimation];
        });
...
+ (void)showAnimation
{
    HJYouthAlterView *shareView = [[NSBundle mainBundle]loadNibNamed:@"HJYouthAlterView" owner:self options:nil].lastObject;
