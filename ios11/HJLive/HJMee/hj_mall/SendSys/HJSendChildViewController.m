...
@interface HJSendChildViewController ()<
...
@implementation HJSendChildViewController
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"HJFriendListCell" bundle:nil] forCellReuseIdentifier:@"HJFriendListCell"];
...
#pragma mark - Getter
- (NSMutableArray<Attention *> *)attentionList{
    if (!_attentionList) {
...
- (void)pullDownRefresh:(int)page{
    ...
    [GetCore(HJPraiseCore) requestAttentionListState:0 page:page];
...
- (void)pullUpRefresh:(int)page lastPage:(BOOL)isLastPage{
    ...
    [GetCore(HJPraiseCore) requestAttentionListState:1 page:page];
}

- (void)updateView {
    self.friendsList = [[GetCore(HJImFriendCore) getMyFriends] mutableCopy];
...
- (void)configureCell:(HJFriendListCell *)cell
...
- (void)inviteBegin:(UserID)uid name:(NSString *)sendUserName{
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:NSLocalizedString(XCAlertConfirm,nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        @strongify(self);
        if (self.isCarSys) {
            [GetCore(HJCarSysCore) sendCarWithCarId:self.proId targetUid:[NSString stringWithFormat:@"%lld",uid]];
        } else {
            [GetCore(HJHeadwear) sendHeadwearWithHeadwearId:self.proId targetUid:[NSString stringWithFormat:@"%lld",uid]];
        }
    }];
...
- (void)jumpBack {
    for (UIViewController *vc in self.navigationController.viewControllers) {
        if ([vc isKindOfClass:[HJHeadMallViewController class]]) {
            @weakify(self);
