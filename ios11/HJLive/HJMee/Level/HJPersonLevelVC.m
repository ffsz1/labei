...
@interface HJPersonLevelVC ()<ZJScrollPageViewDelegate,HJUserCoreClient>
@property (nonatomic, strong) ZJSegmentStyle *style;
@property (nonatomic, strong) NSMutableArray<NSString *> *titles;
@property (nonatomic, strong) HJPersonalLevelChirldVC *richVC;
@property (nonatomic, strong) HJPersonalLevelChirldVC *chamVC;
...
@property (strong, nonatomic) HJRankConsTopView *chatsConsTopView;

@end

@implementation HJPersonLevelVC

- (HJRankConsTopView *)chatsConsTopView {
    if (!_chatsConsTopView) {
        _chatsConsTopView = [[NSBundle mainBundle] loadNibNamed:@"HJRankConsTopView" owner:nil options:nil][0];
        ...
        [_chatsConsTopView.backBtn setImage:[UIImage imageNamed:@"hj_nav_bar_back"] forState:UIControlStateNormal];
        @weakify(self);
...
//    shareInfo.imgUrl = @"https://pic.hnyueqiang.com/logo.png";
    shareInfo.imgUrl = [NSString stringWithFormat:@"%@/home/images/logo.png",[HJHttpRequestHelper getHostUrl]];
...
//    http://test.haijiaoxingqiu.cn:80/home/images/logo.png
    ...
    shareInfo.title = @"小熊语音";//NSLocalizedString(ShareCoreTitle2, nil);
...
- (void)setupSegmentView {
    ...
    ZJScrollSegmentView *segment = [[ZJScrollSegmentView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, getRectNavAndStatusHight) segmentStyle:style delegate:self titles:self.titles titleDidClick:^(ZJTitleView *titleView, NSInteger index) {
...
- (void)setupContentView {
    ZJContentView *content = [[ZJContentView alloc] initWithFrame:CGRectMake(0, getRectNavAndStatusHight, kScreenWidth, kScreenHeight-getRectNavAndStatusHight) segmentView:self.segmentView parentViewController:self delegate:self];
...
- (UIViewController<ZJScrollPageViewChildVcDelegate> *)childViewController:(UIViewController<ZJScrollPageViewChildVcDelegate> *)reuseViewController forIndex:(NSInteger)index {
    NSLog(@"%ld---------", index);
    if (index == 0) {
        HJPersonalLevelChirldVC *vc = (HJPersonalLevelChirldVC *)reuseViewController;
        if (vc == nil) {
            vc = self.richVC;
        }
        return vc;
...
- (HJPersonalLevelChirldVC *)richVC {
    if (!_richVC) {
        _richVC = [[HJPersonalLevelChirldVC alloc]init];
        _richVC.type = 0;
        //        _richVC.tipLabel.text = @"财富值每消耗1开心收到1点财富值";
    }
    return _richVC;
}

- (HJPersonalLevelChirldVC *)chamVC {
    if (!_chamVC) {
        _chamVC = [[HJPersonalLevelChirldVC alloc]init];
        _chamVC.type = 1;
        //        _chamVC.tipLabel.text = @"魅力值每收到1开心收到1点魅力值";
