...
@interface HJNotiFriendTableViewCell ()
...
@implementation HJNotiFriendTableViewCell
...
//    {10,25,25,25}
    UIImage *image = [[UIImage imageNamed:@"hj_noti_icon_tuzi"] resizableImageWithCapInsets:UIEdgeInsetsFromString(@"{10,15,25,35}") resizingMode:UIImageResizingModeStretch];
...
//            }
            if (dic) {
                ...
                if ([dic containsObjectForKey:@"params"]) {
                    ...
                    if (uid == miamiaoUid) {
                        // hj官方
                        self.LvImg.image = [UIImage imageNamed:@"hj_guanfang"];
                        self.lvWidCons.constant = 30;
                        self.LvImg.hidden = false;
                        self.miImgBetweenUserHead.constant = 45;
                        ...
                        self.MlImg.hidden = YES;
                        if (txtColor.length) {
                            self.deautLabel.textColor = [[YYTheme defaultTheme] colorWithHexString:txtColor alpha:1.f];
                        }
                        else {
