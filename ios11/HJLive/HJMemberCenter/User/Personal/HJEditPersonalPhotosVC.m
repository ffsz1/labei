...
@interface HJEditPersonalPhotosVC ()<UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, UIImagePickerControllerDelegate, UINavigationControllerDelegate, HJFileCoreClient, HJPersonalPhotoCellDelegate, HJUserCoreClient, SDPhotoBrowserDelegate>
@property (weak, nonatomic) IBOutlet UICollectionView *collectionView;
@property (strong, nonatomic) NSMutableArray<UserPhoto *> *photos;
...
@implementation HJEditPersonalPhotosVC
...
//    self.userinfo = [GetCore(UserCore) getUserInfo:[GetCore(AuthCore) getUid].userIDValue refresh:YES];
    ...
    [self.collectionView registerNib:[UINib nibWithNibName:@"HJPersonalPhotoCell" bundle:nil] forCellWithReuseIdentifier:@"HJPersonalPhotoCell"];
    [self.collectionView registerNib:[UINib nibWithNibName:@"HJPersonalPhotoCell" bundle:nil] forCellWithReuseIdentifier:@"HJPersonalPhotoCell2"];
    [self initView];
...
- (BOOL)isOwenr {
    return [GetCore(HJAuthCoreHelp) getUid].userIDValue == self.uid;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self isOwenr]) {
        ...
        if (indexPath.row == 0) {
            HJPersonalPhotoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJPersonalPhotoCell2" forIndexPath:indexPath];
            cell.personalPhotoImagwView.image = [UIImage imageNamed:@"hj_add_photo_logo"];
            [cell.deleteBtn setHidden:YES];
            return cell;
        } else if (indexPath.item > 0) {
            
            HJPersonalPhotoCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HJPersonalPhotoCell" forIndexPath:indexPath];
...
//            [GetCore(FileCore) uploadImage:selectedPhoto];
            [GetCore(HJFileCore) qiNiuUploadImage:selectedPhoto uploadType:UploadImageTypeLibary];
...
- (void)didUploadPhotoImageSuccessUseQiNiu:(NSString *)key{
    ...
    [GetCore(HJUserCoreHelp) uploadImageUrlToServer:url];
}
...
- (void)deletePhoto:(NSIndexPath *)indexPath {
    YYAlertView *alertView = [[YYAlertView alloc]initWithTitle:NSLocalizedString(XCPhotosDeleteTitle, nil) message:NSLocalizedString(XCPhotosDeleteMesseage, nil)];
...
- (void)deletePhotoWithIndexPath:(NSIndexPath *)indexPath {
    ...
    [GetCore(HJUserCoreHelp) deleteImageUrlToServerWithPid:pid];
