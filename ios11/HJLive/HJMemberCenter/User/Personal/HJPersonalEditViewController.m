//
//  HJPersonalEditViewController.m
//  HJLive
//
...
#import "HJPersonalVoiceEditVC.h"
...
@interface HJPersonalEditViewController () <HJFileCoreClient, UINavigationControllerDelegate, UIImagePickerControllerDelegate,HJUserCoreClient>
...
@implementation HJPersonalEditViewController
...
{
    HJSingleLineTextModifyVC *vc = (HJSingleLineTextModifyVC *)[[HJUserViewControllerFactory sharedFactory] instantiateSingleLineTextModifyViewController];
...
- (void) showSexController
{
    ...
    [sheet addButtonWithTitle:NSLocalizedString(XCEditChangeBoy, nil) block:^(YYActionSheetViewController *controller) {
        __strong __typeof (wself) sSelf = wself;
        sSelf.sexLabel.text = NSLocalizedString(XCEditChangeBoy, nil);
        [self updateUserInfo:@"gender" value:@"1"];
    }];
    
    [sheet addButtonWithTitle:NSLocalizedString(XCEditChangeGirl, nil) block:^(YYActionSheetViewController *controller) {
        __strong __typeof (wself) sSelf = wself;
...
{
    HJPersonalVoiceEditVC *vc = (HJPersonalVoiceEditVC *)[[HJUserViewControllerFactory sharedFactory] instantiatePersonalVoiceEditViewController];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)updateUserInfo:(NSString *)key value:(NSString *)value;
{
    ...
    [[GetCore(HJUserCoreHelp) saveUserInfoWithUserID:self.userInfo.uid userInfos:userinfos] subscribeNext:^(id x) {
