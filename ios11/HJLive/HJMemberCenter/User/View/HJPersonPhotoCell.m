...
#import "HJPersonPhotoCell.h"

#define HJPersonPhotoCellLineItemCount (4.f)

@interface HJPersonPhotoCell ()
...
@implementation HJPersonPhotoCell

+ (HJPersonPhotoCell *)cellWithTableview:(UITableView *)tableView {
    
    HJPersonPhotoCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HJPersonPhotoCell"];
    
    if (!cell) {
        cell = [[HJPersonPhotoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"HJPersonPhotoCell"];
    }
...
- (UIButton *)addBtn {
    if (!_addBtn) {
        ...
        [_addBtn setBackgroundImage:[UIImage imageNamed:@"hj_me_person_addPhoto"] forState:UIControlStateNormal];
        [_addBtn setBackgroundImage:[UIImage imageNamed:@"hj_me_person_addPhoto"] forState:UIControlStateHighlighted];
