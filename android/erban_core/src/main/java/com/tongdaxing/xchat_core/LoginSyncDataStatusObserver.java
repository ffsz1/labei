...
public class LoginSyncDataStatusObserver {
    ...
    public void registerLoginSyncDataStatus(boolean register) {
        MLog.info(TAG, "observe login sync data completed event on Application create");
    ...
    Observer<LoginSyncStatus> loginSyncStatusObserver = new Observer<LoginSyncStatus>() {
        ...
        public void onEvent(LoginSyncStatus status) {
            ...
            if (status == LoginSyncStatus.BEGIN_SYNC) {
                MLog.info(TAG, "login sync data begin");
            } else if (status == LoginSyncStatus.SYNC_COMPLETED) {
                MLog.info(TAG, "login sync data completed");
    ...
    public boolean observeSyncDataCompletedEvent(Observer<Void> observer) {
        ...
        // 超时定时器
        if (uiHandler == null) {
            uiHandler = new Handler(BasicConfig.INSTANCE.getAppContext().getMainLooper());
        }
    ...
    private void onLoginSyncDataCompleted(boolean timeout) {
        MLog.info(TAG, "onLoginSyncDataCompleted, timeout=" + timeout);
