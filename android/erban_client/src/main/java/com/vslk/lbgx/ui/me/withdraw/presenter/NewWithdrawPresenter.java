...
import com.vslk.lbgx.ui.me.withdraw.model.NewWithDrawModel;
 ...
 */
public class NewWithdrawPresenter extends AbstractMvpPresenter<INewWithdrawView> {

    private NewWithDrawModel mWithDrawModel;

    public NewWithdrawPresenter() {
        if (mWithDrawModel == null) {
            mWithDrawModel = new NewWithDrawModel();
        }
    }
     ...
     * 微信提现绑定
     */
    public void bindWithdrawWeixin(String accessToken, String openId, String unionId, int type) {

        mWithDrawModel.bindWithdrawWeixin(accessToken, openId, unionId, type, new OkHttpManager.MyCallBack<ServiceResult>() {
            @Override
            public void onError(Exception e) {
                if (getMvpView() != null) {
            ...
            public void onResponse(ServiceResult response) {
                if (null != response) {
                    if (response.isSuccess()) {
                        if (getMvpView() != null) {
                            getMvpView().onRemindBindWeixinSucToast(type);
                        }
                    } else {
                        if (getMvpView() != null) {
