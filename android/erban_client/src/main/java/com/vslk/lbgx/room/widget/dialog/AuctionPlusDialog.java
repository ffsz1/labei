        ...
        if (info != null) {
            ...
            if (info.getRivals() != null && info.getRivals().size() > 0) {
                ...
                if (userInfo != null) {
                    ...
                    tvCurrentMaxPrice.setText(String.valueOf(info.getRivals().get(0).getAuctMoney() + "金币"));
                }
                if (myAuctionUser != null) {
                    tvMyPrice.setText(String.valueOf(myAuctionUser.getAuctMoney() + "金币"));
                    price = maxAuctionUser.getAuctMoney() - myAuctionUser.getAuctMoney() + middlePrice;
                } else {
                    tvMyPrice.setText("0金币");
                    price = maxAuctionUser.getAuctMoney() + middlePrice;
                }
                tvPriceNum.setText(String.valueOf(price));
                tvPriceNeed.setText(StringUtils.splic(String.valueOf(price), "金币以上"));
            } else {
                price = info.getAuctMoney();
                tv_name.setText(String.valueOf("当前起拍价" + info.getAuctMoney() + "金币"));
                tvCurrentMaxPrice.setText("");
                tvPriceNum.setText(String.valueOf(price));
                tvPriceNeed.setText(String.valueOf(info.getAuctMoney() + "金币以上"));
            }
        }
    }
