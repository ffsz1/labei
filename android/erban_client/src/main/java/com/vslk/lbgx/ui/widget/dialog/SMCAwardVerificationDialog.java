...
public class SMCAwardVerificationDialog extends BaseDialogFragment {
    ...
    InviteRedPacketDrawPresenter inviteRedPacketDrawPresenter;
    ...
    public static SMCAwardVerificationDialog newInstance(InviteRedPacketDrawPresenter inviteRedPacketDrawPresenter) {
        SMCAwardVerificationDialog smcAwardVerificationDialog = new SMCAwardVerificationDialog(inviteRedPacketDrawPresenter);
        return smcAwardVerificationDialog;
    }

    private SMCAwardVerificationDialog(InviteRedPacketDrawPresenter inviteRedPacketDrawPresenter) {

        this.inviteRedPacketDrawPresenter = inviteRedPacketDrawPresenter;
    }
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        View view = inflater.inflate(R.layout.verify_smc_qq_vx_dialog, window.findViewById(android.R.id.content), false);
...
//            CoreManager.getCore(IAuthCore.class).requestSMSCode(phone, 3);
            ...
            inviteRedPacketDrawPresenter.getInviteCode();

            buExchangeVerificationGetSms.setEnabled(false);
        });

        buExchangeVerificationSubmit.setOnClickListener(v -> {
            if (iOnSubmit != null && !(TextUtils.isEmpty(edtExchangeVerification.getText().toString().trim()))) {
                iOnSubmit.onSubmit(edtExchangeVerification.getText().toString().trim());
            } else {
                inviteRedPacketDrawPresenter.getMvpView().onRemindToastError("请输入验证码");
            }
        });
        return view;
    }
