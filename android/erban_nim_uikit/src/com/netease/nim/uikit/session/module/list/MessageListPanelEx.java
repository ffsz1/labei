...
public class MessageListPanelEx {
    ...
    Observer<RevokeMsgNotification> revokeMessageObserver = new Observer<RevokeMsgNotification>() {
        ...
        public void onEvent(RevokeMsgNotification message) {
            if (message == null || !container.account.equals(message.getMessage().getSessionId())) {
                return;
            }

            deleteItem(message.getMessage(), false);
        }
    };
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        private void prepareDialogItems(final IMMessage selectedItem, CustomAlertDialog alertDialog) {
            ...
            if (NimUIKit.getMsgForwardFilter() != null && !NimUIKit.getMsgForwardFilter().shouldIgnore(selectedItem) && !recordOnly) {
                // 6 forward to person
