...
@interface YPSingleLineTextModifyVC ()
@property (weak, nonatomic) IBOutlet UITextField *editText;
@end

@implementation YPSingleLineTextModifyVC
...
- (void)onCompleteBtnClicked {
    ...
    if (text.length > 0) {
        ...
        @weakify(self)
        [[GetCore(YPUserCoreHelp) saveUserInfoWithUserID:self.userID userInfos:userinfos] subscribeNext:^(id x) {
