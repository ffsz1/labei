...
#import "YPPersonLabelCell.h"

@interface YPPersonLabelCell ()

@property (nonatomic, strong) NSMutableArray *labelArr;

@end

@implementation YPPersonLabelCell

+ (YPPersonLabelCell *)cellWithTableView:(UITableView *)tableView {
    
    YPPersonLabelCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPPersonLabelCell"];
    
    if (!cell) {
        cell = [[YPPersonLabelCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"YPPersonLabelCell"];
    }
...
- (void)setLabelStrArr:(NSArray *)labelStrArr {
    ...
    for (NSString *str in labelStrArr) {
        ...
        label.text = str;
        CGFloat w = [YPPersonLabelCell getWidthWithStr:str font:label.font] + textPadding * 2;
        if (x + w > maxtWidth) {
            x = 15.f;
            y += (10.f + h);
        }
...
+ (CGFloat)getHeightWithStrArr:(NSArray *)labelStrArr {
    ...
    for (NSString *str in labelStrArr) {
        CGFloat w = [YPPersonLabelCell getWidthWithStr:str font:[UIFont boldSystemFontOfSize:13.f]] + textPadding * 2;
        if (x + w > maxtWidth) {
            x = 15.f;
            y += (10.f + h);
        }
