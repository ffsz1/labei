...
@interface YPBindingPhoneVC ()<HJAuthCoreClient,HJPurseCoreClient>
...
//    bgView.backgroundColor = [[YPYYDefaultTheme defaultTheme] colorWithHexString:@"#F5F5F5" alpha:1.0];
...
//        [GetCore(AuthCore) requestSmsCode:self.phoneField.text type:@3];
        ...
        [GetCore(YPAuthCoreHelp) requestSmsCode:self.phoneField.text type:@2];
    }
}
...
- (IBAction)onRegisterBtnClicked:(id)sender {
    ...
    if (phone.length > 0 || smsCode.length > 0) {
        ...
        if (self.isBindPhone) {
            if (self.isConfirm) {
                [GetCore(YPAuthCoreHelp) requestConfirmCode:self.phoneField.text smsCode:self.smsCodeField.text success:nil failure:nil];
            } else {
                [GetCore(YPAuthCoreHelp) requestReplacePhone:phone smsCode:smsCode];
            }
        } else {
            [GetCore(YPPurseCore) bindingPhoneNum:phone verifyCode:smsCode];
        }
