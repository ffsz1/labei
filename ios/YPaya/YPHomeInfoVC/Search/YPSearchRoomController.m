...
#import "YPSearchHistoryFooterView.h"
#import "YPSearchHistoryTableCell.h"

@interface YPSearchRoomController ()
<
    UITextFieldDelegate,
    HJSearchCoreClient,
...
@property (copy, nonatomic) NSArray<YPSearchResultInfo *> *resultList;
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (nonatomic, assign) int inputCount;
@property (weak, nonatomic) IBOutlet YPSearchTagView *tagView;
...
@implementation YPSearchRoomController
...
- (void)handleSelectTag:(NSInteger)keyWordTag {
    ...
    [GetCore(YPSearchCoreHelp) searchWithKey:self.searchTextField.text];
}
...
- (void)initView {
    ...
    self.tableView.backgroundColor = [[YPYYDefaultTheme defaultTheme]colorWithHexString:@"#F5F5F5" alpha:1];
...
{
    if (self.isShowHistory) {
        YPSearchHistoryTableCell *cell = [YPSearchHistoryTableCell cellWithTableView:tableView];
        cell.textLabel.text = self.calculateTagArr[indexPath.row];
        return cell;
    }
    else {
        
        YPSearchResultCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPSearchResultCell" forIndexPath:indexPath];
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (self.isShowHistory) {
        YPSearchHistoryHeaderView *headerView = [[YPSearchHistoryHeaderView alloc] init];
        return headerView;
    }
    else {
        return [UIView new];
...
- (nullable UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    if (self.isShowHistory) {
        YPSearchHistoryFooterView *footerView = [YPSearchHistoryFooterView new];
...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isShowHistory) {
        ...
        YPSearchResultInfo *info = self.resultList[indexPath.row];
...
- (void)requestKeyWorld:(NSDictionary *)info{
    if (self.inputCount == [info[@"count"] integerValue]) {
        ...
        [GetCore(YPSearchCoreHelp) searchWithKey:info[@"key"]];
    }
}

#pragma mark - private method
- (void)configureCell:(YPSearchResultCell *)cell
...
//        imageAttachment.image = [UIImage imageNamed:@"yp_main_home_party_tag"];
//        NSAttributedString *imageString = [NSAttributedString attributedStringWithAttachment:imageAttachment];
