...
#import "YPRankCharmListVC.h"

#import "YPRankCell.h"
#import "YPMarqueeLabel.h"

#import "YPHttpRequestHelper+rank.h"



@interface YPRankListVC ()<UITableViewDelegate,UITableViewDataSource>
...
@property (weak, nonatomic) IBOutlet YPMarqueeLabel *nameLabel1;
@property (weak, nonatomic) IBOutlet YPMarqueeLabel *nameLabel2;
@property (weak, nonatomic) IBOutlet YPMarqueeLabel *nameLabel3;
...
@property (nonatomic,strong) YPChartsModel *myCharmModel;
@property (nonatomic,strong) YPChartsModel *myRichModel;
...
- (void)initCharmView
{
    YPRankCharmListVC *vc = YPHomeStoryBoard(@"YPRankCharmListVC");
...
{
    YPRankCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPRankCell" forIndexPath:indexPath];
    
    if (
        indexPath.row<self.dataArr.count-3) {
        
        YPChartsModel *model = self.dataArr[indexPath.row+3];
...
- (IBAction)richBtnAction:(id)sender {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.topLine.image = [UIImage imageNamed:@"yp_room_rank_wealth_bottom"];
...
- (IBAction)charmBtnAction:(id)sender {
    ...
    [self.view layoutIfNeeded];
    [UIView animateWithDuration:0.3 animations:^{
        self.topLine.image = [UIImage imageNamed:@"yp_room_rank_charm_bottom"];
        self.center_topLine.constant = 70;
        [self.view layoutIfNeeded];
    }];
...
- (IBAction)firstHeaderAction:(id)sender {
    if (self.dataArr.count>0) {
        YPChartsModel *model = self.dataArr[0];
        YPMySpaceVC *vc = YPMeStoryBoard(@"YPMySpaceVC");
        vc.userID = model.uid;
        [self.navigationController pushViewController:vc animated:YES];
    }
...
- (IBAction)secondHeaderAction:(id)sender {
    ...
    if (self.dataArr.count>1) {
        YPChartsModel *model = self.dataArr[1];
        YPMySpaceVC *vc = YPMeStoryBoard(@"YPMySpaceVC");
        vc.userID = model.uid;
        [self.navigationController pushViewController:vc animated:YES];
    }
...
- (IBAction)thirdHeaderAction:(id)sender {
    ...
    if (self.dataArr.count>2) {
        YPChartsModel *model = self.dataArr[2];
        YPMySpaceVC *vc = YPMeStoryBoard(@"YPMySpaceVC");
        vc.userID = model.uid;
        [self.navigationController pushViewController:vc animated:YES];
    }
...
//    self.avatarImageView3.hidden = !hasThird;
    self.avatarImageView1.image = [UIImage imageNamed:@"yp_room_rank_emptywealth"];
    self.avatarImageView2.image = [UIImage imageNamed:@"yp_room_rank_emptywealth"];
    self.avatarImageView3.image = [UIImage imageNamed:@"yp_room_rank_emptywealth"];
    ...
    if (hasFirst) {
        YPChartsModel *model1 = self.dataArr[0];
        ...
        self.sexImageView1.image = [UIImage imageNamed:model1.gender==1?@"yp_rank_man":@"yp_rank_women"];
        self.levelImageView1.image = [UIImage imageNamed:[NSString getMoneyLevelImageName:model1.experLevel]];
    }
    
    if (hasSecond) {
        YPChartsModel *model1 = self.dataArr[1];
        ...
        self.sexImageView2.image = [UIImage imageNamed:model1.gender==1?@"yp_rank_man":@"yp_rank_women"];
    ...
    if (hasThird) {
        YPChartsModel *model1 = self.dataArr[2];
        ...
        self.sexImageView3.image = [UIImage imageNamed:model1.gender==1?@"yp_rank_man":@"yp_rank_women"];
...
- (void)updateMyCharmData
{
    if (self.richBtn.selected) {
        ...
        self.coinImageView.image = [UIImage imageNamed:@"yp_rank_coin"];
        ...
        self.coinImageView.image = [UIImage imageNamed:@"yp_rank_love"];
...
- (void)getData
{
    ...
    __weak typeof(self)weakSelf = self;
    [YPHttpRequestHelper getRichRankData:self.daySelType+1 success:^(NSArray * _Nonnull list) {
...
{
    __weak typeof(self)weakSelf = self;
    [YPHttpRequestHelper getMyRankData:type datetype:datetype + 1 success:^(id model) {
        
        YPChartsModel *myModel = model;
