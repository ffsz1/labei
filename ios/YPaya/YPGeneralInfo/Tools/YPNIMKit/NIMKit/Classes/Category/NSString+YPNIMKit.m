...
@implementation NSString (YPNIMKit)
...
- (NSString *)cutAvatarImageSize {
    NSString *avatar = self;
    if ([avatar containsString:@"?"]) {
        NSRange range = [avatar rangeOfString:@"?"];
        avatar = [avatar substringToIndex:range.location+1];
        avatar = [avatar stringByAppendingString:@"imageView2/1/w/128/h/128"];
    }
    return avatar;
}
...
- (NSString *)cutAvatarImageSize:(NSString *)width height:(NSString *)height{
    ...
    if ([avatar containsString:@"?"]) {
        NSRange range = [avatar rangeOfString:@"?"];
        avatar = [avatar substringToIndex:range.location+1];
        avatar = [avatar stringByAppendingString:[NSString stringWithFormat:@"imageView2/1/w/%@/h/%@",width,height]];
    }
    return avatar;
}
