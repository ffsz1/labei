...
@interface YPNIMInputMoreContainerView() <NIMPageViewDataSource,NIMPageViewDelegate>
{
    NSArray                 *_mediaButtons;
    NSArray                 *_mediaItems;
}

@property (nonatomic, strong) YPNIMPageView *pageView;

@end

@implementation YPNIMInputMoreContainerView

- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
        _pageView = [[YPNIMPageView alloc] initWithFrame:CGRectZero];
...
- (void)genMediaButtons
{
    ...
    if (!self.config)
    {
        items = [YPNIMKit sharedKit].config.defaultMediaItems;
    }
    else if([self.config respondsToSelector:@selector(mediaItems)])
    {
        items = [self.config mediaItems];
    }
    [items enumerateObjectsUsingBlock:^(YPNIMMediaItem *item, NSUInteger idx, BOOL *stop) {
...
- (UIView*)mediaPageView:(YPNIMPageView*)pageView beginItem:(NSInteger)begin endItem:(NSInteger)end
{
...
- (UIView*)oneLineMediaInPageView:(YPNIMPageView *)pageView
...
- (void)onTouchButton:(id)sender
{
    ...
    YPNIMMediaItem *item = _mediaItems[index];
