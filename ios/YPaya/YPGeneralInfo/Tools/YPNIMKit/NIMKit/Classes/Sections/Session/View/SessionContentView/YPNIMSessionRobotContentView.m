//
//  YPNIMSessionRobotContentView.m
//  YPNIMKit
//
...
#import "YPNIMSessionRobotContentView.h"
#import "UIView+NIM.h"
...
@interface YPNIMSessionRobotContentView()
...
@property (nonatomic,strong) YPNIMKitRobotDefaultTemplateParser *parser;

@end

@implementation YPNIMSessionRobotContentView
...
- (instancetype)initSessionMessageContentView
{
    ...
    if (self)
    {
        ...
        _parser     = [[YPNIMKitRobotDefaultTemplateParser alloc] init];
...
- (void)refreshContineButton:(YPNIMMessageModel *)data
{
...
- (void)setupRobot:(YPNIMMessageModel *)data
{
    ...
    YPNIMKitRobotTemplate *template = [[YPNIMKit sharedKit].robotTemplateParser robotTemplate:data.message];
    if (![template.version isEqualToString:@"0.1"])
    {
        NSLog(@"robot template version incompatible!");
    }
    for (YPNIMKitRobotTemplateLayout *layout in template.items)
    {
...
{
    CGFloat height = [YPNIMSessionRobotContentView itemSpacing];
    CGFloat width = 0;;
    for (UIView *view in self.subviews) {
        ...
        {
            height += [YPNIMSessionRobotContentView continueItemSpacing];
        }
        else
        {
            height += [YPNIMSessionRobotContentView itemSpacing];
        }
    }
...
- (void)layoutSubviews
{
    ...
    CGFloat top = [YPNIMSessionRobotContentView itemSpacing];
    for (UIView *view in self.subviews)
    {
        if (view == self.bubbleImageView)
        {
        ...
        view.nim_top = top;
        top = view.nim_bottom + [YPNIMSessionRobotContentView itemSpacing];
    }
}
