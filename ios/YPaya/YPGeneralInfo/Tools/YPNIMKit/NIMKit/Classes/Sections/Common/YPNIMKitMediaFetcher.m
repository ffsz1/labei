...
@interface YPNIMKitMediaFetcher()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
@implementation YPNIMKitMediaFetcher
...
- (void)pickImageAction
{
    ...
    } title:nil view:[UIApplication sharedApplication].keyWindow.viewController.view tailor:YES delegate:self];
    [actionSheet show];
}
...
- (void)fetchPhotoFromLibrary:(NIMKitLibraryFetchResult)result
{
    ...
    self.libraryResultHandler = result;
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        if (selectedPhoto) {
            ...
            if (self.libraryResultHandler) {
                self.libraryResultHandler(@[image], nil, PHAssetMediaTypeImage);
            }
...
- (void)requestAsset:(PHAsset *)asset handler:(void(^)(NSString *,PHAssetMediaType)) handler
{
    if (asset.mediaType == PHAssetMediaTypeVideo) {
        ...
        [[PHImageManager defaultManager] requestExportSessionForVideo:asset options:options exportPreset:AVAssetExportPresetMediumQuality resultHandler:^(AVAssetExportSession * _Nullable exportSession, NSDictionary * _Nullable info) {
            ...
            NSString *outputFileName = [YPNIMKitFileLocationHelper genFilenameWithExt:@"mp4"];
            NSString *outputPath = [YPNIMKitFileLocationHelper filepathForVideo:outputFileName];
