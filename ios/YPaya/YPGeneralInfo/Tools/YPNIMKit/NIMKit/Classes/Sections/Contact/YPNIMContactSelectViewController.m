//
//  YPNIMContactSelectViewController.m
//  YPNIMKit
//
...
#import "YPNIMGroupedData.h"
#import "YPNIMContactDataCell.h"
#import "UIView+NIM.h"
#import "YPNIMKit.h"
#import "NIMKitDependency.h"
#import "NIMGlobalMacro.h"

@interface YPNIMContactSelectViewController ()<UITableViewDataSource, UITableViewDelegate, NIMContactPickedViewDelegate>{
    NSMutableArray *_selectecContacts;
}
@property (strong, nonatomic) UITableView *tableView;

@property (strong, nonatomic) YPNIMContactSelectTabView *selectIndicatorView;
...
@implementation YPNIMContactSelectViewController
...
- (instancetype)initWithConfig:(id<YPNIMContactSelectConfig>) config{
...
- (void)setUpNav
{
    self.navigationItem.title = [self.config respondsToSelector:@selector(title)] ? [self.config title] : NSLocalizedString(NIMKitChoosePerson, nil);
...
- (NSString *)detailTitle
{
    ...
    {
        detail = [NSString stringWithFormat:@"%@%zd人",NSLocalizedString(NIMKitDidChoosed, nil),_selectecContacts.count];
    }
    return detail;
}
...
- (void)setConfig:(id<YPNIMContactSelectConfig>)config{
    _config = config;
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    if ([self.sectionTitles[0] isEqualToString:@"$"] && section == 0) {
        return NSLocalizedString(NIMKitRoboite, nil);
    }else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    YPNIMContactDataCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SelectContactCellID"];
    if (cell == nil) {
        cell = [[YPNIMContactDataCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SelectContactCellID"];
    }
    cell.accessoryBtn.hidden = NO;
    cell.accessoryBtn.selected = [_selectecContacts containsObject:[contactItem memberId]];
    YPNIMKitInfo *info = [self.config getInfoById:[contactItem memberId]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    YPNIMContactDataCell *cell = (YPNIMContactDataCell *)[tableView cellForRowAtIndexPath:indexPath];
    YPNIMKitInfo *info;
...
- (YPNIMContactSelectTabView *)selectIndicatorView{
    if (_selectIndicatorView) {
        return _selectIndicatorView;
    }
    CGFloat tabHeight = 50.f;
    CGFloat tabWidth  = 320.f;
    _selectIndicatorView = [[YPNIMContactSelectTabView alloc] initWithFrame:CGRectMake(0, 0, tabWidth, tabHeight)];
    return _selectIndicatorView;
}
