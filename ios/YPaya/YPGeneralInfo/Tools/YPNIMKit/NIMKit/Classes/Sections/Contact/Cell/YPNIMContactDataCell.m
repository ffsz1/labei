...
@interface YPNIMContactDataCell()

@end

@implementation YPNIMContactDataCell

- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    self = [super initWithStyle:style reuseIdentifier:reuseIdentifier];
    if (self) {
        _avatarImageView = [[YPNIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 30, 30)];
...
- (void)refreshUser:(id<NIMGroupMemberProtocol>)member{
    [self refreshTitle:member.showName];
    self.memberId = [member memberId];
    YPNIMKitInfo *info = [[YPNIMKit sharedKit] infoByUser:self.memberId option:nil];
    [self refreshAvatar:info];
}
...
- (void)refreshTeam:(id<NIMGroupMemberProtocol>)member{
    [self refreshTitle:member.showName];
    self.memberId = [member memberId];
    YPNIMKitInfo *info = [[YPNIMKit sharedKit] infoByTeam:self.memberId option:nil];
    [self refreshAvatar:info];
}
...
- (void)refreshAvatar:(YPNIMKitInfo *)info{
