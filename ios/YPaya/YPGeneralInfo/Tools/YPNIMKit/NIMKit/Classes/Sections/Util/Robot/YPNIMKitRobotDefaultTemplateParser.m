...
#import "YPNIMKitRobotDefaultTemplateParser.h"
#import "NSDictionary+YPNIMKit.h"
...
@interface YPNIMKitRobotDefaultTemplateParser()<NSXMLParserDelegate>
{
...
@implementation YPNIMKitRobotDefaultTemplateParser
...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        _containerClass = [NSSet setWithObjects:@"YPNIMKitRobotTemplate",@"YPNIMKitRobotTemplateLayout",@"NIMKitRobotTemplateLinkItem",nil];
...
{
    YPNIMKitRobotTemplate *template = [self.robotTemplates objectForKey:message.messageId];
    if (template)
    {
        return template;
    }
...
- (YPNIMKitRobotTemplate *)resolveBotTemplate:(NIMRobotObject *)object
{
    YPNIMKitRobotTemplate *template = [[YPNIMKitRobotTemplate alloc] init];
    ...
    for (NSDictionary *dict in [object.response nimkit_jsonArray:@"message"])
    {
        if ([dict isKindOfClass:[NSDictionary class]])
        {
            ...
            {
                YPNIMKitRobotTemplateLayout *layout = [self genTextRobotLayout:templateText];
                [template.items addObject:layout];
            }
            if ([type isEqualToString:NNIMKitRobotResponseTypeImage])
            {
                YPNIMKitRobotTemplateLayout *layout =  [self genImageRobotLayout:templateText];
                [template.items addObject:layout];
            }
        }
    }
...
- (YPNIMKitRobotTemplate *)resolveFaqTemplate:(NIMRobotObject *)object
{
    YPNIMKitRobotTemplate *template = [[YPNIMKitRobotTemplate alloc] init];
    template.version = @"0.1"; //默认版本
    ...
    if ([match isKindOfClass:[NSDictionary class]])
    {
        ...
        YPNIMKitRobotTemplateLayout *layout = [self genTextRobotLayout:answer];
        [template.items addObject:layout];
    }
    return template;
}
...
- (YPNIMKitRobotTemplateLayout *)genTextRobotLayout:(NSString *)templateText
{
    YPNIMKitRobotTemplateLayout *layout  = [[YPNIMKitRobotTemplateLayout alloc] init];
...
- (YPNIMKitRobotTemplateLayout *)genImageRobotLayout:(NSString *)templateText
{
    YPNIMKitRobotTemplateLayout *layout  = [[YPNIMKitRobotTemplateLayout alloc] init];
    NIMKitRobotTemplateItem *templateItem = [[NIMKitRobotTemplateItem alloc] init];
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName attributes:(NSDictionary *)attributeDict
{
    ...
    {
        YPNIMKitRobotTemplate *robotTemplate = [self.robotTemplates objectForKey:_currentMessageId];
        item = robotTemplate;
    }
