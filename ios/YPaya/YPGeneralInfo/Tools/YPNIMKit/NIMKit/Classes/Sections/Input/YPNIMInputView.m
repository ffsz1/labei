...
#import "YPNIMInputMoreContainerView.h"
#import "YPNIMInputEmoticonContainerView.h"
#import "YPNIMInputAudioRecordIndicatorView.h"
#import "UIView+NIM.h"
...
@interface YPNIMInputView()<NIMInputToolBarDelegate,NIMInputEmoticonProtocol,NIMContactSelectDelegate>
{
    UIView  *_emoticonView;
}

@property (nonatomic, strong) YPNIMInputAudioRecordIndicatorView *audioRecordIndicator;
                       ...
                       config:(id<NIMSessionConfig>)config
{
    ...
    if (self)
    {
        ...
        _atCache = [[YPNIMInputAtCache alloc] init];
        _inputConfig = config;
...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    switch (self.status)
    {
        ...
        default:
        {
            ...
            CGFloat keyboardDelta = [YPNIMKitKeyboardInfo instance].keyboardHeight - safeArea.bottom;
...
- (YPNIMInputAudioRecordIndicatorView *)audioRecordIndicator {
    if(!_audioRecordIndicator) {
        _audioRecordIndicator = [[YPNIMInputAudioRecordIndicatorView alloc] init];
    }
...
- (void)setup
{
    if (!_toolBar)
    {
        _toolBar = [[YPNIMInputToolBar alloc] initWithFrame:CGRectMake(0, 0, self.nim_width, 0)];
        
        [self addSubview:_toolBar];
        
        //设置placeholder
        NSString *placeholder = [YPNIMKit sharedKit].config.placeholder;
        ...
        //设置最大输入字数
        NSInteger textInputLength = [YPNIMKit sharedKit].config.inputMaxLength;
...
{
    if (!_moreContainer) {
        YPNIMInputMoreContainerView *moreContainer = [[YPNIMInputMoreContainerView alloc] initWithFrame:CGRectZero];
...
- (void)checkEmoticonContainer
{
    if (!_emoticonContainer) {
        YPNIMInputEmoticonContainerView *emoticonContainer = [[YPNIMInputEmoticonContainerView alloc] initWithFrame:CGRectZero];
...
- (void)didFinishedSelect:(NSArray *)selectedContacts
{
    ...
    for (NSString *uid in selectedContacts) {
        NSString *nick = [[YPNIMKit sharedKit].provider infoByUser:uid option:option].showName;
...
- (NSRange)delRangeForEmoticon
{
    ...
    if (range.length > 1)
    {
        ...
        NIMInputEmoticon *icon = [[YPNIMInputEmoticonManager sharedManager] emoticonByTag:name];
