//
//  YPNIMAdvancedTeamCardViewController.m
//  NIM
//
...
#import "YPNIMAdvancedTeamCardViewController.h"
#import "YPNIMTeamCardRowItem.h"
#import "UIView+NIM.h"
#import "UIImage+YPNIMKit.h"
#import "YPNIMKitColorButtonCell.h"
#import "YPNIMAdvancedTeamMemberCell.h"
#import "NIMKitDependency.h"
#import "YPNIMTeamMemberCardViewController.h"
#import "YPNIMCardMemberItem.h"
#import "YPNIMContactSelectViewController.h"
#import "YPNIMGroupedUsrInfo.h"
#import "YPNIMTeamMemberListViewController.h"
#import "YPNIMTeamAnnouncementListViewController.h"
#import "YPNIMKitUtil.h"
#import "YPNIMTeamSwitchTableViewCell.h"
#import "YPNIMAvatarImageView.h"
#import "YPNIMKitProgressHUD.h"
#import "YPNIMTeamNotifyUpdateViewController.h"
...
@property (nonatomic,strong) YPNIMAvatarImageView *avatar;
...
- (instancetype)initWithTeam:(NIMTeam*)team{
    ...
    if (self) {
        ...
        _avatar  = [[YPNIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 50, 50)];
...
@interface YPNIMAdvancedTeamCardViewController ()<NIMAdvancedTeamMemberCellActionDelegate,NIMContactSelectDelegate,NIMTeamSwitchProtocol,NIMAdvancedTeamCardHeaderViewDelegate,NIMTeamManagerDelegate,UIActionSheetDelegate,UITableViewDataSource,UITableViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>{
...
@implementation YPNIMAdvancedTeamCardViewController
...
- (void)buildBodyData{
    BOOL canEdit = [YPNIMKitUtil canEditTeamInfo:self.myTeamInfo];
    BOOL isOwner    = self.myTeamInfo.type == NIMTeamMemberTypeOwner;
    BOOL isManager  = self.myTeamInfo.type == NIMTeamMemberTypeManager;
    
    YPNIMTeamCardRowItem *teamMember = [[YPNIMTeamCardRowItem alloc] init];
    ...
    YPNIMTeamCardRowItem *teamName = [[YPNIMTeamCardRowItem alloc] init];
    teamName.title = @"群名称";
    ...
    YPNIMTeamCardRowItem *teamNick = [[YPNIMTeamCardRowItem alloc] init];
    teamNick.title = @"群昵称";
    ...
    YPNIMTeamCardRowItem *teamIntro = [[YPNIMTeamCardRowItem alloc] init];
    teamIntro.title = @"群介绍";
    ...
    YPNIMTeamCardRowItem *teamAnnouncement = [[YPNIMTeamCardRowItem alloc] init];
    ...
    YPNIMTeamCardRowItem *teamNotify = [[YPNIMTeamCardRowItem alloc] init];
    ...
    YPNIMTeamCardRowItem *itemQuit = [[YPNIMTeamCardRowItem alloc] init];
    itemQuit.title = @"退出高级群";
    itemQuit.action = @selector(quitTeam);
    itemQuit.rowHeight = 60.f;
    itemQuit.type   = TeamCardRowItemTypeRedButton;
    
    YPNIMTeamCardRowItem *itemDismiss = [[YPNIMTeamCardRowItem alloc] init];
    ...
    YPNIMTeamCardRowItem *itemAuth = [[YPNIMTeamCardRowItem alloc] init];
    itemAuth.title  = @"身份验证";
    ...
    YPNIMTeamCardRowItem *itemInvite = [[YPNIMTeamCardRowItem alloc] init];
    ...
    YPNIMTeamCardRowItem *itemUpdateInfo = [[YPNIMTeamCardRowItem alloc] init];
    ...
    YPNIMTeamCardRowItem *itemBeInvite = [[YPNIMTeamCardRowItem alloc] init];
...
- (NSIndexPath *)cellIndexPathByTitle:(NSString *)title {
    ...
    [self.bodyData enumerateObjectsUsingBlock:^(NSArray *rows, NSUInteger s, BOOL *stop) {
        ...
        [rows enumerateObjectsUsingBlock:^(YPNIMTeamCardRowItem *item, NSUInteger r, BOOL *stop) {
...
- (UITableViewCell*)builidRedButtonCell:(id<NTESCardBodyData>) bodyData{
    YPNIMKitColorButtonCell * cell = [self.tableView dequeueReusableCellWithIdentifier:TableButtonCellReuseId];
    if (!cell) {
        cell = [[YPNIMKitColorButtonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:TableButtonCellReuseId];
    }
...
- (UITableViewCell*)builidTeamMemberCell:(id<NTESCardBodyData>) bodyData{
    YPNIMAdvancedTeamMemberCell * cell = [self.tableView dequeueReusableCellWithIdentifier:TableMemberCellReuseId];
    if (!cell) {
        cell = [[YPNIMAdvancedTeamMemberCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:TableMemberCellReuseId];
        cell.delegate = self;
    }
...
{
    YPNIMTeamSwitchTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:TableSwitchReuseId];
    if (!cell) {
        cell = [[YPNIMTeamSwitchTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"YPNIMTeamSwitchTableViewCell"];
    }
    cell.textLabel.text = bodyData.title;
...
- (void)onMore:(id)sender{
    _moreActionSheet = [[UIActionSheet alloc] initWithTitle:@"请操作" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles:@"转让群",@"转让群并退出",nil];
    [_moreActionSheet showInView:self.view];
}

- (void)onTouchAvatar:(id)sender{
    if([YPNIMKitUtil canEditTeamInfo:self.myTeamInfo])
    {
        _avatarActionSheet = [[UIActionSheet alloc] initWithTitle:@"设置群头像" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles:@"拍照",@"从相册", nil];
        [_avatarActionSheet showInView:self.view];
    }
}

- (void)updateTeamName{
    _updateTeamNameAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"修改群名称" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
    _updateTeamNameAlertView.alertViewStyle = UIAlertViewStylePlainTextInput;
    [_updateTeamNameAlertView show];
}

- (void)updateTeamNick{
    _updateTeamNickAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"修改群昵称" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
...
{
    YPNIMTeamNotifyUpdateViewController *vc = [[YPNIMTeamNotifyUpdateViewController alloc] initTeam:self.team];
    [self.navigationController pushViewController:vc animated:YES];
}

- (void)updateTeamIntro{
    _updateTeamIntroAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"修改群介绍" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
...
- (void)updateTeamAnnouncement{
    YPNIMTeamAnnouncementListViewController *vc = [[YPNIMTeamAnnouncementListViewController alloc] initWithNibName:nil bundle:nil];
    vc.team = self.team;
    vc.canCreateAnnouncement = [YPNIMKitUtil canEditTeamInfo:self.myTeamInfo];
    [self.navigationController pushViewController:vc animated:YES];
}


- (void)quitTeam{
    _quitTeamAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"确认退出群聊?" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
    [_quitTeamAlertView show];
}

- (void)dismissTeam{
    _dismissTeamAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"确认解散群聊?" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
...
- (void)changeAuthMode{
    ...
    _authActionSheet = [[UIActionSheet alloc] initWithTitle:@"更改验证方式" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles:@"允许任何人",@"需要验证",@"拒绝任何人", nil];
    [_authActionSheet showInView:self.view];
}

- (void)changeInviteMode{
    _inviteActionSheet = [[UIActionSheet alloc] initWithTitle:@"邀请他人入群" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles:@"管理员邀请",@"所有人邀请", nil];
    [_inviteActionSheet showInView:self.view];
}

- (void)changeUpdateInfoMode{
    _updateInfoActionSheet = [[UIActionSheet alloc] initWithTitle:@"群资料修改权限" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles:@"管理员修改",@"所有人修改", nil];
    [_updateInfoActionSheet showInView:self.view];
}

- (void)changeBeInviteMode{
    _beInviteActionSheet = [[UIActionSheet alloc] initWithTitle:@"被邀请人身份验证" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles:@"需要验证",@"不需要验证", nil];
...
- (void)enterMemberCard{
    YPNIMTeamMemberListViewController *vc = [[YPNIMTeamMemberListViewController alloc] initTeam:self.team members:self.memberData];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (void)didSelectAddOpeartor{
    ...
    YPNIMContactSelectViewController *vc = [[YPNIMContactSelectViewController alloc] initWithConfig:config];
    vc.delegate = self;
    [vc show];
}
