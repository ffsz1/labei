...
#import "YPNIMCommonTableData.h"
#import "YPNIMCommonTableDelegate.h"
...
#import "YPNIMKitSwitcherCell.h"

@interface YPNIMTeamMemberCardViewController () <UIActionSheetDelegate>{
...
@property (nonatomic,strong) YPNIMCommonTableDelegate *delegator;

@property (nonatomic,strong) NSArray *data;

@end

@implementation YPNIMTeamMemberCardViewController
...
- (void)viewDidLoad {
    ...
    user.info = [[YPNIMKit sharedKit] infoByUser:self.member.memberId option:nil];
    self.usrInfo = user;

    [self buildData];
    __weak typeof(self) wself = self;
    self.delegator = [[YPNIMCommonTableDelegate alloc] initWithTableData:^NSArray *{
        return wself.data;
    }];
...
- (void)buildData{
    ...
    NSArray *data = @[
                      @{
                          ...
                          RowContent :@[
                                  @{
                                      CellClass     : @"YPNIMTeamMemberCardHeaderCell",
                                      RowHeight     : @(222),
                                  ...
                                  @{
                                      Title         : @"设置禁言",
                                      CellClass     : @"YPNIMKitSwitcherCell",
                                  ...
                                  @{
                                      Title         : @"移出本群",
                                      CellClass     : @"YPNIMKitColorButtonCell",
...
- (void)onKickBtnClick:(id)sender {
    _kickAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"移出本群" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
    [_kickAlertView show];
}

- (void)updateTeamNick
{
    _updateNickAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"修改群昵称" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) otherButtonTitles:NSLocalizedString(XCRoomConfirm, nil), nil];
...
- (void)updateTeamRole
{
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"管理员操作" delegate:self cancelButtonTitle:NSLocalizedString(XCRoomCancel, nil) destructiveButtonTitle:nil otherButtonTitles: self.member.type == NIMTeamMemberTypeManager ? @"取消管理员" : @"设为管理员", nil];
...
{
    return [YPNIMKitUtil canEditTeamInfo:self.viewer.member];
}
