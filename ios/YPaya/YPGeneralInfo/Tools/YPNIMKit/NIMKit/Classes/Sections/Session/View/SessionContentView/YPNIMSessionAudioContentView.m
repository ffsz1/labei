...
@interface YPNIMSessionAudioContentView()<NIMMediaManagerDelegate>
...
@implementation YPNIMSessionAudioContentView
...
- (BOOL)isPlaying
{
    return [YPNIMKitAudioCenter instance].currentPlayingMessage == self.model.message; //对比是否是同一条消息，严格同一条，不能是相同ID，防止进了会话又进云端消息界面，导致同一个ID的云消息也在动画
}
