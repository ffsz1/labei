...
@interface YPNIMMessageCell()<NIMPlayAudioUIDelegate,NIMMessageContentViewDelegate>
{
    UILongPressGestureRecognizer *_longPressGesture;
    UIMenuController             *_menuController;
}

@property (nonatomic,strong) YPNIMMessageModel *model;
...
@implementation YPNIMMessageCell
...
- (void)makeComponents
{
    ...
    //audioPlayedIcon
    _audioPlayedIcon = [YPNIMBadgeView viewWithBadgeTip:@""];
    ...
    //headerView
    _headImageView = [[YPNIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    ...
    _nameLabel.font   = [YPNIMKit sharedKit].config.nickFont;
    _nameLabel.textColor = [YPNIMKit sharedKit].config.nickColor;
    ...
    _readLabel.font   = [YPNIMKit sharedKit].config.receiptFont;
    _readLabel.textColor = [YPNIMKit sharedKit].config.receiptColor;
...
- (BOOL)checkData{
    return [self.model isKindOfClass:[YPNIMMessageModel class]];
}
...
- (void)refresh
{
    ...
    {
        NSString *nick = [YPNIMKitUtil showNick:self.model.message.from inMessage:self.model.message];
...
- (void)addContentViewIfNotExist
{
    if (_bubbleView == nil)
    {
        ...
        YPNIMSessionMessageContentView *contentView =  [[clazz alloc] initSessionMessageContentView];
        ...
        if (messageType == NIMMessageTypeAudio) {
            ((YPNIMSessionAudioContentView *)_bubbleView).audioUIDelegate = self;
        }
...
- (void)onCatchEvent:(YPNIMKitEvent *)event{
