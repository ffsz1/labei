...
#import "YPNIMKitTitleView.h"
#import "YPNIMKitKeyboardInfo.h"
#import "YPUserCoreHelp.h"
#import "UIView+XCToast.h"

@interface YPNIMSessionViewController ()<NIMMediaManagerDelegate,NIMInputDelegate>
...
@property (nonatomic,strong)  YPNIMSessionConfigurator *configurator;

@property (nonatomic,weak)    id<NIMSessionInteractor> interactor;

@end

@implementation YPNIMSessionViewController
...
{
    [self removeListener];
    [[YPNIMKit sharedKit].robotTemplateParser clean];
...
{
    [self setUpTitleView];
//    YPNIMCustomLeftBarView *leftBarView = [[YPNIMCustomLeftBarView alloc] init];
...
- (void)setupInputView
{
    ...
    {
        self.sessionInputView = [[YPNIMInputView alloc] initWithFrame:CGRectMake(0, 0, self.view.nim_width,0) config:self.sessionConfig];
...
{
    _configurator = [[YPNIMSessionConfigurator alloc] init];
...
- (NSMutableAttributedString *)sessionTitle
{
    ...
    switch (type) {
        case NIMSessionTypeTeam:{
            ...
            title = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"%@(%zd)",[team teamName],[team memberNumber]]];
        }
            break;
        case NIMSessionTypeP2P:{
            title = [[NSMutableAttributedString alloc]initWithString:[YPNIMKitUtil showNick:self.session.sessionId inSession:self.session]];
        }
            break;
        default:
            break;
...
- (void)fetchMessageAttachment:(NIMMessage *)message didCompleteWithError:(NSError *)error
{
    if ([message.session isEqual:_session])
    {
        YPNIMMessageModel *model = [self.interactor findMessageModel:message];
...
- (void)recordAudio:(NSString *)filePath didCompletedWithError:(NSError *)error {
    if(!error) {
        if ([self recordFileCanBeSend:filePath]) {
            [self sendMessage:[YPNIMMessageMaker msgWithAudio:filePath]];
        }else{
...
- (void)onSendText:(NSString *)text atUsers:(NSArray *)atUsers
{
    ...
    {
        message = [YPNIMMessageMaker msgWithRobotQuery:text toRobot:robotsToSend];
    }
    else
    {
        message = [YPNIMMessageMaker msgWithText:text];
    }
    ...
    if (atUsers.count)
    {
        ...
        NSString *me = [[YPNIMKit sharedKit].provider infoByUser:[NIMSDK sharedSDK].loginManager.currentAccount option:option].showName;
...
- (void)onStartRecording
{
    ...
    NSTimeInterval duration = [YPNIMKit sharedKit].config.recordMaxDuration;
...
- (BOOL)onTapCell:(YPNIMKitEvent *)event{
    ...
    if ([eventName isEqualToString:NIMKitEventNameTapRobotBlock]) {
        ...
        NIMMessage *message = [YPNIMMessageMaker msgWithRobotSelect:param[@"text"] target:param[@"target"] params:param[@"param"] toRobot:param[@"robotId"]];
        [self sendMessage:message];
        handle = YES;
    }
...
- (YPNIMMessageModel *)uiDeleteMessage:(NIMMessage *)message{
    YPNIMMessageModel *model = [self.interactor deleteMessage:message];
...
- (void)onTapMediaItemPicture:(YPNIMMediaItem *)item
{
    if ([GetCore(YPUserCoreHelp) checkUserHasBanned:0]) {
        [UIView showToastInKeyWindow:@"亲，由于您的发言违反平台规定，现已被禁言，若有疑问可联系海角星球客服。" duration:1 position:YYToastPositionCenter];
        return;
    }
    
    NSInteger sendPicLeftLevel = 0;
    NSUserDefaults *ur = [NSUserDefaults standardUserDefaults];
    if ([ur objectForKey:@"sendPicLeftLevel"]) {
        NSString *tag = [ur objectForKey:@"sendPicLeftLevel"];
        sendPicLeftLevel = [tag integerValue];
    }
    
    NSString *userId = [GetCore(YPAuthCoreHelp) getUid];
    UserInfo *info = [GetCore(YPUserCoreHelp) getUserInfoInDB:[userId intValue]];

    if (info.experLevel < sendPicLeftLevel) {
        NSString *tip = [NSString stringWithFormat:@"还没达到%ld等级，无法发送哦",(long)sendPicLeftLevel];
        [UIView showToastInKeyWindow:tip duration:1 position:YYToastPositionCenter];
        return;
    }
...
- (void)onTapMediaItemShoot:(YPNIMMediaItem *)item
{
    [self.interactor mediaShootPressed];
}

- (void)onTapMediaItemLocation:(YPNIMMediaItem *)item
{
...
{
    YPNIMCustomLeftBarView *leftBarView = (YPNIMCustomLeftBarView *)self.navigationItem.leftBarButtonItem.customView;
...
- (void)setUpTitleView
{
    YPNIMKitTitleView *titleView = (YPNIMKitTitleView *)self.navigationItem.titleView;
    if (!titleView || [titleView isKindOfClass:[YPNIMKitTitleView class]])
    {
        titleView = [[YPNIMKitTitleView alloc] initWithFrame:CGRectZero];
        self.navigationItem.titleView = titleView;
    }
    titleView.titleLabel.attributedText = self.sessionTitle;
