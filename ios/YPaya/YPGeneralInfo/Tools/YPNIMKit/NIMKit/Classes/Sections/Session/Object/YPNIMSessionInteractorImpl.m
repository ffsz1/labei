...
@interface YPNIMSessionInteractorImpl()<NIMMediaManagerDelegate>

@property (nonatomic,strong) NIMSession  *session;

@property (nonatomic,strong) id<NIMSessionConfig> sessionConfig;

@property (nonatomic,strong) YPNIMKitMediaFetcher *mediaFetcher;
...
@implementation YPNIMSessionInteractorImpl
...
- (YPNIMMessageModel *)deleteMessage:(NIMMessage *)message
{
    YPNIMMessageModel *model = [self findMessageModel:message];
    if (model) {
...
- (YPNIMMessageModel *)updateMessage:(NIMMessage *)message
{
    YPNIMMessageModel *model = [self findMessageModel:message];
    if (model) {
...
- (YPNIMMessageModel *)findMessageModel:(NIMMessage *)message
{
...
{
  BOOL ddd =  [[[NIMSDK sharedSDK] chatManager] sendMessage:message toSession:_session error:nil];
    NSLog(@"ddd");
}
- (void)sendMessage:(NIMMessage *)message
...
- (void)mediaAudioPressed:(YPNIMMessageModel *)messageModel
{
    if (![[NIMSDK sharedSDK].mediaManager isPlaying]) {
        [[NIMSDK sharedSDK].mediaManager switchAudioOutputDevice:NIMAudioOutputDeviceSpeaker];
        self.pendingAudioMessages = [self findRemainAudioMessages:messageModel.message];
        [[YPNIMKitAudioCenter instance] play:messageModel.message];
        ...
        switch (type) {
            case PHAssetMediaTypeImage:
            {
                for (UIImage *image in images) {
                    NIMMessage *message = [YPNIMMessageMaker msgWithImage:image];
                ...
                if (path) {
                    ...
                    if ([path.pathExtension isEqualToString:@"HEIC"])
                    {
                        ...
                        message = [YPNIMMessageMaker msgWithImage:image];
                    }
                    else
                    {
                        message = [YPNIMMessageMaker msgWithImagePath:path];
                    }
                    NIMAntiSpamOption *antiSpamOption = [[NIMAntiSpamOption alloc] init];
                    antiSpamOption.yidunEnabled = NO;
                    message.antiSpamOption = antiSpamOption;
            ...
            {
                NIMMessage *message = [YPNIMMessageMaker msgWithVideo:path];
                NIMAntiSpamOption *antiSpamOption = [[NIMAntiSpamOption alloc] init];
                antiSpamOption.yidunEnabled = NO;
                message.antiSpamOption = antiSpamOption;
                [[NIMSDK sharedSDK].chatManager sendMessage:message toSession:weakSelf.session error:nil];
...
- (void)mediaShootPressed
{
    ...
    [self.mediaFetcher fetchMediaFromCamera:^(NSString *path, UIImage *image) {
        ...
        if (image) {
            ...
            message = [YPNIMMessageMaker msgWithVideo:path];
        }
        NIMAntiSpamOption *antiSpamOption = [[NIMAntiSpamOption alloc] init];
        antiSpamOption.yidunEnabled = NO;
        message.antiSpamOption = antiSpamOption;
...
- (void)onSendLocation:(YPNIMKitLocationPoint *)locationPoint{ 
    NIMMessage *message = [YPNIMMessageMaker msgWithLocation:locationPoint];
    NIMAntiSpamOption *antiSpamOption = [[NIMAntiSpamOption alloc] init];
    antiSpamOption.yidunEnabled = NO;
    message.antiSpamOption = antiSpamOption;
...
- (void)playNextAudio
{
    ...
    if (self.pendingAudioMessages.count) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [[YPNIMKitAudioCenter instance] play:message];
        });
    }
}
...
#pragma mark - Private
- (YPNIMKitMediaFetcher *)mediaFetcher
{
    if (!_mediaFetcher) {
        _mediaFetcher = [[YPNIMKitMediaFetcher alloc] init];
    }
...
- (NSMutableArray *)findRemainAudioMessages:(NIMMessage *)message
{
    ...
    [self.dataSource.items enumerateObjectsWithOptions:NSEnumerationReverse usingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[YPNIMMessageModel class]]) {
            YPNIMMessageModel *model = (YPNIMMessageModel *)obj;
...
- (void)processChatroomMessageModels
{
    ...
    if ([self.layout canInsertChatroomMessages])
    {
        ...
        static NSInteger NTESMaXBDroomMessageCount = 200;
        NSInteger count = self.dataSource.items.count;
        if (count > NTESMaXBDroomMessageCount) {
