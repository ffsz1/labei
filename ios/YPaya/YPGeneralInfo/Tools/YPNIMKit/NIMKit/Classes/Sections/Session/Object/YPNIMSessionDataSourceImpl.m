...
@interface YPNIMSessionDataSourceImpl()

@property (nonatomic,strong) NIMSession *session;

@property (nonatomic,strong) YPNIMSessionMsgDatasource *dataSource;
...
@implementation YPNIMSessionDataSourceImpl
                         ...
                         config:(id<NIMSessionConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        _dataSource = [[YPNIMSessionMsgDatasource alloc] initWithSession:_session config:_sessionConfig];
    }
    return self;
}
...
- (YPNIMMessageModel *)findModel:(NIMMessage *)message{
    ...
    for (YPNIMMessageModel *item in self.dataSource.items.reverseObjectEnumerator.allObjects) {
        if ([item isKindOfClass:[YPNIMMessageModel class]] && [item.message.messageId isEqual:message.messageId]) {
            model = item;
...
- (void)checkAttachmentState:(NSArray *)messages{
    ...
    for (id item in items) {
        ...
        if ([item isKindOfClass:[YPNIMMessageModel class]]) {
            message = [(YPNIMMessageModel *)item message];
        }
...
- (void)sendMessageReceipt:(NSArray *)messages
{
    ...
    if ([[UIApplication sharedApplication] applicationState] == UIApplicationStateActive)
    {
        ...
        for (NSInteger i = [messages count] - 1; i >= 0; i--) {
            ...
            else if ([item isKindOfClass:[YPNIMMessageModel class]])
            {
                message = [(YPNIMMessageModel *)item message];
            }
            if (message)
