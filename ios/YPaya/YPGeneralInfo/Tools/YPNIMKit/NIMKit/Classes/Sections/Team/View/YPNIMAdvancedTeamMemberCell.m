...
@property(nonatomic,strong) YPNIMKitInfo *member;
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _imageView   = [[YPNIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 37, 37)];
        [self addSubview:_imageView];
    }
    return self;
}

- (void)setMember:(YPNIMKitInfo *)member{
    _member = member;
    NSURL *avatarURL;
...
@interface YPNIMAdvancedTeamMemberCell()
...
@implementation YPNIMAdvancedTeamMemberCell
...
- (void)rereshWithTeam:(NIMTeam*)team
    ...
    NSInteger count = 0;
    if ([YPNIMKitUtil canInviteMember:myTeamInfo]) {
    ...
    for (NSInteger i = 0; i < iconCount; i++) {
        ...
        if (!count || i != 0) {
            ...
            view.member                 = info;
            view.titleLabel.text        = [YPNIMKitUtil showNick:member.userId inSession:session];
        }
