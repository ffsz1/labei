...
@interface YPNIMKitProgressHUD()

@property (nonatomic, strong) CAShapeLayer *indefiniteAnimatedLayer;

@end

@implementation YPNIMKitProgressHUD

+ (instancetype)sharedView
{
    static YPNIMKitProgressHUD *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[YPNIMKitProgressHUD alloc] initWithFrame:CGRectMake(0, 0, 84, 84)];
    });
    return instance;
}
...
+ (void)show
{
    [[YPNIMKitProgressHUD sharedView] showInView:[UIApplication sharedApplication].keyWindow];
}

+ (void)dismiss
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[YPNIMKitProgressHUD sharedView] removeFromSuperview];
        [[YPNIMKitProgressHUD sharedView].indefiniteAnimatedLayer removeFromSuperlayer];
    });
}
...
- (CAShapeLayer*)indefiniteAnimatedLayer {
    if(!_indefiniteAnimatedLayer) {
        ...
        NSBundle *bundle = [NSBundle bundleForClass:[YPNIMKitProgressHUD class]];
        NSURL *url = [bundle URLForResource:[YPNIMKit sharedKit].resourceBundleName withExtension:nil];
