...
- (void)setNeedsNavigationBackground:(CGFloat)alpha {
    ...
    [self getBackView:self.navigationBar with:alpha];
}

- (void)setNeedsNavigationBackgroundForColor:(UIColor*)color {
    // 导航栏背景透明度设置
    
    [self getBackViewForColor:self.navigationBar with:color];
}

-(void)getBackView:(UIView*)superView with:(CGFloat)alpha {
    
    UIView *barBackgroundView = [[self.navigationBar subviews] safeObjectAtIndex:0];// _UIBarBackground
    UIImageView *backgroundImageView = [[barBackgroundView subviews] safeObjectAtIndex:0];// UIImageView
    if (self.navigationBar.isTranslucent && [backgroundImageView isKindOfClass:[UIImageView class]]) {
        if (backgroundImageView != nil && backgroundImageView.image != nil) {
            ...
            if ([barBackgroundView subviews].count > 1) {
                ...
                UIView *backgroundEffectView = [[barBackgroundView subviews] safeObjectAtIndex:1];// UIVisualEffectView
    ...
    for (UIView *view in superView.subviews) {
        [self getBackView:view with:alpha];
    }
...
-(void)getBackViewForColor:(UIView*)superView with:(UIColor*)color {
...
//        superView.alpha = alpha;
    ...
    for (UIView *view in superView.subviews) {
        [self getBackViewForColor:view with:color];
    }
    
    // 对导航栏下面那条线做处理
    self.navigationBar.clipsToBounds = 0 == 0.0;
}
