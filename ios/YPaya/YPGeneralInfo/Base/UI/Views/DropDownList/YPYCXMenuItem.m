...
#import "YPYCXMenuItem.h"

@interface YPYCXMenuItem ()

@property (nonatomic, assign) BOOL showSelected;
@end

@implementation YPYCXMenuItem
...
+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image target:(id)target action:(SEL)action {
    YPYCXMenuItem *item = [[YPYCXMenuItem alloc] init:title image:image tag:0 userInfo:nil target:target action:action];
...
+ (instancetype)menuItem:(NSString *)title image:(UIImage *)image tag:(NSInteger)tag userInfo:(NSDictionary *)userInfo {
    YPYCXMenuItem *item = [[YPYCXMenuItem alloc] init:title image:image tag:tag userInfo:userInfo target:nil action:NULL];
    item.alignment = NSTextAlignmentCenter;
...
+ (instancetype)menuTitle:(NSString *)title WithIcon:(UIImage *)icon {
    YPYCXMenuItem *item = [[YPYCXMenuItem alloc] init:title image:icon tag:0 userInfo:nil target:nil action:NULL];
