...
#import "YPZJContentView.h"
//#import "HJHomeCoreClient.h"

@interface YPZJContentView ()<UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource> {
    CGFloat   _oldOffSetX;
...
// 用于处理重用和内容的显示
@property (strong, nonatomic) YPZJCollectionView *collectionView;
// collectionView的布局
...
@implementation YPZJContentView
...
- (void)commonInit {
    ...
    if ([navi isKindOfClass:[UINavigationController class]]) {
        ...
        if (navi.interactivePopGestureRecognizer) {
            ...
            [_collectionView setupScrollViewShouldBeginPanGestureHandler:^BOOL(YPZJCollectionView *collectionView, UIPanGestureRecognizer *panGesture) {
    ...
    [_childVcsDic enumerateKeysAndObjectsUsingBlock:^(NSString * _Nonnull key, UIViewController<ZJScrollPageViewChildVcDelegate> * _Nonnull childVc, BOOL * _Nonnull stop) {
        ...
        if (strongSelf) {
            if (childVc != strongSelf.currentChildVc) {
                ...
                [YPZJContentView removeChildVc:childVc];
            }
        }
...
//    NotifyCoreClient(HJHomeCoreClient, @selector(notiIndex:), notiIndex:currentIndex);
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ZJContentViewScrollViewDidScrollNotification" object:@{@"index" : @(currentIndex)}];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.segmentView.segmentStyle.scrollContentViewBgColor) {
        cell.contentView.backgroundColor = self.segmentView.segmentStyle.scrollContentViewBgColor;
    }
...
- (YPZJCollectionView *)collectionView {
    if (_collectionView == nil) {
        YPZJCollectionView *collectionView = [[YPZJCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:self.collectionViewLayout];
        ...
        if (self.segmentView.segmentStyle.scrollContentViewBgColor) {
            collectionView.backgroundColor = self.segmentView.segmentStyle.scrollContentViewBgColor;
