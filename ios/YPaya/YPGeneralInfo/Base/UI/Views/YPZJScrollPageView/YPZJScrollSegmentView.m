...
#import "YPZJTitleView.h"
#import "UIView+ZJFrame.h"

@interface YPZJScrollSegmentView ()<UIScrollViewDelegate> {
...
@implementation YPZJScrollSegmentView
...
- (void)addScrollLineOrCoverOrExtraBtn {
    ...
    if (self.segmentStyle.leftView) {
        [self addSubview:self.segmentStyle.leftView];
    }
    
    if (self.segmentStyle.rightView) {
        [self addSubview:self.segmentStyle.rightView];
    }
...
{
#if DEBUG
    NSLog(@"YPZJScrollSegmentView ---- 销毁");
...
- (void)titleLabelOnClick:(UITapGestureRecognizer *)tapGes {
    ...
    YPZJTitleView *currentLabel = (YPZJTitleView *)tapGes.view;
...
- (void)setupTitles {
    ...
    for (NSString *title in self.titles) {
        ...
        YPZJTitleView *titleView = [[YPZJTitleView alloc] initWithFrame:CGRectZero];
        titleView.tag = index;
...
- (void)setupUI {
    ...
    if (self.segmentStyle.isScrollTitle) { // 设置滚动区域
        YPZJTitleView *lastTitleView = (YPZJTitleView *)self.titleViews.lastObject;
...
- (void)setupScrollViewAndExtraBtn {
    ...
    CGFloat x = 0;
    if (self.segmentStyle.leftView) {
        x = self.segmentStyle.leftView.width;
        scrollW -= self.segmentStyle.leftView.width;
        self.segmentStyle.leftView.frame = CGRectMake(0,
                                                      (self.zj_height-self.segmentStyle.leftView.height + self.segmentStyle.titleViewTopSpace)/2 ,
                                                      self.segmentStyle.leftView.width,
                                                      self.segmentStyle.leftView.height);
    }
    if (self.segmentStyle.rightView) {
        scrollW -= self.segmentStyle.rightView.width;
        self.segmentStyle.rightView.frame = CGRectMake(self.zj_width - self.segmentStyle.rightView.width,
                                                       (self.zj_height-self.segmentStyle.rightView.height + self.segmentStyle.titleViewTopSpace)/2 ,
                                                       self.segmentStyle.rightView.width,
                                                       self.segmentStyle.rightView.height);
    }
    
    self.scrollView.frame = CGRectMake(x, 0.0, scrollW, self.zj_height);
...
- (void)setUpTitleViewsPosition {
    CGFloat titleX = 0.0;
    CGFloat titleY = self.segmentStyle.titleViewTopSpace;//0.0;
    CGFloat titleW = 0.0;
    CGFloat titleH = self.zj_height - self.segmentStyle.scrollLineHeight - self.segmentStyle.titleViewTopSpace;
    ...
    if (!self.segmentStyle.isScrollTitle) {// 标题不能滚动, 平分宽度
        ...
        NSInteger index = 0;
        for (YPZJTitleView *titleView in self.titleViews) {
    ...
    YPZJTitleView *currentTitleView = (YPZJTitleView *)self.titleViews[_currentIndex];
...
- (void)setupScrollLineAndCover {
    ...
    YPZJTitleView *firstLabel = (YPZJTitleView *)self.titleViews[0];
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.titleViewTopSpace > 0) {
//                self.scrollLine.frame = CGRectMake(coverX , self.titleViewHeight *4 / 5 + self.segmentStyle.titleViewTopSpace, coverW , self.segmentStyle.scrollLineHeight);
                if (self.segmentStyle.scrollLineIsBackground) {
                    //背景位置修改后
                    self.scrollLine.frame = CGRectMake(coverX , self.segmentStyle.titleViewTopSpace, coverW , self.segmentStyle.scrollLineHeight);
                }
                
            } else {
                self.scrollLine.frame = CGRectMake(coverX , 3*self.zj_height / 4, coverW , self.segmentStyle.scrollLineHeight);
                if (self.segmentStyle.scrollLineIsBackground) {
                    //背景位置修改后
                    self.scrollLine.frame = CGRectMake(coverX , 0, coverW , self.segmentStyle.scrollLineHeight);
                }
    
            }
            self.scrollLineImageView.center = CGPointMake(coverW/2, 0);
            
        } else {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                coverW = [self.titleWidths[_currentIndex] floatValue] + wGap;
                
                if (self.segmentStyle.setSpLineWidth) {
                    coverW = 15;
                }
            ...
            if (self.segmentStyle.scrollLineIsBackground) {
                //背景位置修改后
                self.scrollLine.frame = CGRectMake(coverX ,17, coverW , self.segmentStyle.scrollLineHeight);
                self.scrollLineImageView.center = CGPointMake(coverW/2, 0);
            }
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    YPZJTitleView *oldTitleView = (YPZJTitleView *)self.titleViews[_oldIndex];
    YPZJTitleView *currentTitleView = (YPZJTitleView *)self.titleViews[_currentIndex];
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    YPZJTitleView *oldTitleView = (YPZJTitleView *)self.titleViews[oldIndex];
    YPZJTitleView *currentTitleView = (YPZJTitleView *)self.titleViews[currentIndex];
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                if (self.segmentStyle.setSpLineWidth) {
                    self.scrollLine.zj_width = 15 + wDistance * progress;
                }
            } else {
                self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress;
                self.scrollLine.zj_width = oldTitleView.zj_width + wDistance * progress;
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    int index = 0;
    for (YPZJTitleView *titleView in _titleViews) {
...
//
    ...
    if (self.scrollView.contentSize.width != self.scrollView.bounds.size.width + contentSizeXOff) {// 需要滚动
        YPZJTitleView *currentTitleView = (YPZJTitleView *)_titleViews[currentIndex];
...
//            }
//        }
        [self.scrollView setContentOffset:CGPointMake(offSetx, 0.0) animated:NO];
    }
...
- (UIView *)scrollLine {
    ...
    if (!_scrollLine) {
        ...
        if (self.segmentStyle.scrollLineImage != nil) {
            self.scrollLineImageView = [[UIImageView alloc]initWithImage:self.segmentStyle.scrollLineImage];
            [_scrollLine addSubview:self.scrollLineImageView];
        }
    }
...
- (UIScrollView *)scrollView {
    ...
    if (!_scrollView) {
        ...
        if (self.segmentStyle.scrollSegmentViewBgColor) {
            scrollView.backgroundColor = self.segmentStyle.scrollSegmentViewBgColor;
        }
