...
#import "YPAppDelegate+HJJpush.h"
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [[YPYYTheme defaultTheme] apply];//theme
    [self initControllerCenter];
    
    self.window = [[YPYYWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    self.window.backgroundColor = [UIColor colorWithHexString:@"#f5f5f5"];
    
    [GetCore(YPLinkedMeCore) initLinkedMEWithLaunchOptions:launchOptions];
    ...
    if (launchOptionsRemoteNotification != nil) {
        [GetCore(YPImLoginCore) handleRemoteNotification:launchOptionsRemoteNotification];
    }
    ...
    [GetCore(YPAuthCoreHelp) autoLogin];//主页中判断是否登陆
...
- (void)applicationDidBecomeActive:(UIApplication *)application {
    ...
    [GetCore(YPImLoginCore) markNotificationRead];
...
- (void)showGuidView {
    ...
    if (version.length <= 0 || ![version isEqualToString:currentVersion]) {
        YPGuidView *guidView = [[YPGuidView alloc] init];
...
- (void)initControllerCenter{
    [YPAuthViewControllerCenter defaultCenter];
    [YPRoomViewControllerCenter defaultCenter];
    [YPPurseViewControllerCenter defaultCenter];
}
...
{
    [GetCore(YPImLoginCore) updateApnsToken:deviceToken];
    // Jpush
...
- (BOOL)application:(UIApplication *)app openURL:(NSURL *)url options:(NSDictionary<UIApplicationOpenURLOptionsKey,id> *)options
{
    ...
    if ([[url description] rangeOfString:@"click_id"].location != NSNotFound) {
        return [GetCore(YPLinkedMeCore)judgeDeepLinkWith:url];
    }
    else if ([url.scheme isEqualToString:JX_QQ_SEXADECIMAL]) {
        [QQApiInterface handleOpenURL:url delegate:GetCore(YPAuthCoreHelp).qqTool];
    }
        ...
        return [WXApi handleOpenURL:url delegate:GetCore(YPAuthCoreHelp).wechtTool];
    }
...
- (BOOL)application:(UIApplication*)application continueUserActivity:(NSUserActivity*)userActivity restorationHandler:(void (^)(NSArray*))restorationHandler{
    ...
    if ([[userActivity.webpageURL description] rangeOfString:@"lkme.cc"].location != NSNotFound) {
        return  [GetCore(YPLinkedMeCore)judgeDeepLinkWithUniversal:userActivity];
    }
...
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo
{
    ...
    if (self.needHandlerNotification) {
        [GetCore(YPImLoginCore) handleRemoteNotification:userInfo];
    }
}
...
//    [GetCore(YPRoomCoreV2Help) closeRoom:GetCore(AuthCore).getUid.userIDValue];
