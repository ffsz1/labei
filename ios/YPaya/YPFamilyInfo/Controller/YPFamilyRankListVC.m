...
@interface YPFamilyRankListVC ()<UITableViewDataSource, UITableViewDelegate, HJFamilyCoreClient>

@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray<YPFamilyModel *> *records;
...
@implementation YPFamilyRankListVC
...
- (void)loadRecords {
    [GetCore(YPFamilyCore) familyGetListWithFamilyId:nil page:0];
}
...
#pragma mark - Event
- (void)skipToFamily:(YPFamilyModel *)info {
    YPMyFamilyVC *vc = [[UIStoryboard storyboardWithName:@"HJFamily" bundle:[NSBundle mainBundle]] instantiateViewControllerWithIdentifier:@"YPMyFamilyVC"];
    YPFamilyInfoDetail *detail = [YPFamilyInfoDetail new];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    YPDiscoveryFamilyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:XCFamilyRankListCellID forIndexPath:indexPath];
    [self configureCell:cell atIndexPath:indexPath];
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    YPFamilyModel *info = [self.records objectOrNilAtIndex:indexPath.row];
...
- (void)configureCell:(YPDiscoveryFamilyTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    YPFamilyModel *info = [self.records objectOrNilAtIndex:indexPath.row];
...
- (NSMutableArray<YPFamilyModel *> *)records {
    if (!_records) {
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[YPDiscoveryFamilyTableViewCell class] forCellReuseIdentifier:XCFamilyRankListCellID];
