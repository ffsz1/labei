...
#import "YPFamilyManagerSettingCell.h"
...
@interface YPFamilyManagersVC ()<HJFamilyCoreClient, UITableViewDataSource, UITableViewDelegate>
...
@implementation YPFamilyManagersVC
...
- (void)setupManagers {
    ...
    if (self.selectedUids.count == 1) {
        ...
        [GetCore(YPFamilyCore) setManagerWithFamilyId:self.familyInfo.familyId uid:uid];
        return;
    }
    ...
    [MBProgressHUD showMessage:@"处理中"];
    [GetCore(YPFamilyCore) setManagerWithFamilyId:self.familyInfo.familyId uids:uids];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    YPFamilyManagerSettingCell *cell = [tableView dequeueReusableCellWithIdentifier:HJFamilyManagerSettingCellID forIndexPath:indexPath];
...
- (void)configureCell:(YPFamilyManagerSettingCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.didTapDeleteHandler = ^{
        ...
        YPFamilyInfoDetail *buffer = [self.members objectOrNilAtIndex:currentIndexPath.row];
        [MBProgressHUD showMessage:@""];
        [GetCore(YPFamilyCore) removeManagerWithFamilyId:self.familyInfo.familyId userID:[NSString stringWithFormat:@"%ld", (long)buffer.uid]];
    };
}
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[YPFamilyManagerSettingCell class] forCellReuseIdentifier:HJFamilyManagerSettingCellID];
