...
HJAuthCoreClient
>
@property (nonatomic, strong) YPNotiFriendToolView *notiFriendToolView;
...
- (void)onSendPublicMessageDidSuccess:(NSString *)roomId {
    if (![roomId isEqualToString:GetCore(YPNotiFriendCore).getRoomid]) return;
    
    [GetCore(YPMyTaskCore) dutyFreshPublic];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    YPNotiFriendTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPNotiFriendTableViewCell" forIndexPath:indexPath];
...
//        if ([GetCore(YPImRoomCoreV2) isInRoom]) {
            if (textField.text.length > 45) {
                ...
                [GetCore(YPImMessageCore) sensitiveWordRegexWithText:textField.text requestId:NSStringFromClass([self class]) finishBlock:^(BOOL isCanSend, NSString *msg) {
                    @strongify(self);
                    if (isCanSend) {
                        @weakify(self);
                        [GetCore(YPNotiFriendCore) sendChatMessage:self.notiFriendToolView.chatTextField.text withUserInfo:self.info success:^{
                            [GetCore(YPMyTaskCore) dutyFreshPublic];
                        } failure:^(NSInteger code, NSString *message) {
                            @strongify(self);
                            @weakify(self);
                            [YPJXAuthorizationAlertHelper showAuthorizationAlertWithViewController:self code:code message:message didTapActionHandler:^(UIViewController * _Nullable toViewController) {
                                @strongify(self);
                                if (toViewController) {
...
//    [UIView showToastInKeyWindow:@"亲，由于您的发言违反平台规定，现已被禁言，若有疑问可联系喜翻鸭客服。" duration:1 position:YYToastPositionCenter];
    ...
    [UIView showToastInKeyWindow:@"亲，由于您的发言违反平台规定，现已被禁言，若有疑问可联系喜翻鸭客服。" duration:1.0 position:(YYToastPosition)YYToastPositionAboveKeyboard];

}

- (void)checkQuestion {
    YPWKWebViewController *vc = [[YPWKWebViewController alloc]init];
    NSString *urlSting = [NSString stringWithFormat:@"%@/front/friends/index.html",[YPHttpRequestHelper getHostUrl]];
...
- (NSMutableArray<YPIMMessage *> *)chatMessage {
    if (!_chatMessage) {
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerNib:[UINib nibWithNibName:@"YPNotiFriendTableViewCell" bundle:nil] forCellReuseIdentifier:@"YPNotiFriendTableViewCell"];
    }
    return _tableView;
}

- (YPNotiFriendToolView *)notiFriendToolView {
    if (!_notiFriendToolView) {
        _notiFriendToolView = [[NSBundle mainBundle] loadNibNamed:@"YPNotiFriendToolView" owner:nil options:nil][0];
