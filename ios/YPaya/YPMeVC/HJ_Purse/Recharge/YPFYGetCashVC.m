...
#import "YPFYSetBankInfoVC.h"
#import "YPFYRealNameAuthView.h"
#import "YPWKWebViewController.h"
#import "YPFYBankInfoListModel.h"
#import "YPFYBankBindInfoModel.h"
#import "YPFYZfbBindInfoModel.h"
#import "YPZhiFuBaoAndBankInfoVC.h"
#import "YPGetCachSuccessView.h"
#import "YPAlerVerificationCodeView.h"
#import "HJAuthCoreClient.h"
#import "YPHttpRequestHelper+Purse.h"
#import "UINavigationController+Cloudox.h"

@interface YPFYGetCashVC ()<UICollectionViewDelegate, UICollectionViewDataSource, HJPurseCoreClient, HJRechargeViewCellDelegate,UIGestureRecognizerDelegate>
...
//钻石模型
@property (strong, nonatomic)  YPFYGetCashInfoModel *selectModel;
...
- (void)viewDidLoad {
    ...
    self.balanceInfo = GetCore(YPPurseCore).drawExchangeModel;
    ...
    self.selectModel = [[YPFYGetCashInfoModel alloc] init];
//    [self getWXInfo];
    _desView.hidden = NO;
    _wxView.hidden = YES;
...
//    [GetCore(YPPurseCore) requestBalanceInfo:uid.userIDValue];
    //MARK: - 提现用户信息
    [GetCore(YPPurseCore) requestBalanceInfoWithDrawExchange:uid.userIDValue];
    ...
    //判断实名认证
        [GetCore(YPPurseCore) requestGetUserRealNameStatus:GetCore(YPAuthCoreHelp).getUid.userIDValue type:@"cash"];
}
- (void)dealloc
{
...
//    @weakify(self);
    ...
    [GetCore(YPPurseCore) requestGetCashList:myUid];
    [self.collectionView registerNib:[UINib nibWithNibName:@"YPRechargeViewCollectionViewCell" bundle:nil] forCellWithReuseIdentifier:@"RechargeViewCollectionViewCell"];
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if (kind == UICollectionElementKindSectionFooter) {
        ...
        label3.text = @"逾期未到账请联系客服微信号：haijiaoxingqiu6";
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    YPRechargeViewCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"RechargeViewCollectionViewCell" forIndexPath:indexPath];
    YPFYGetCashInfoModel *cashInfo = [self.productList safeObjectAtIndex:indexPath.row];
    cell.titleLable.text = cashInfo.cashProdName;
    cell.rmbLabel.text = [NSString stringWithFormat:@"¥ %ld元", (long)cashInfo.cashNum];
    cell.goldlabel.text =  [NSString stringWithFormat:@"%ld钻石", (long)cashInfo.diamondNum];
    
//    cell.iconImage.image = [UIImage imageNamed:@"fy_qianbao_zhuanshi"];
    
//    [self.navigationController setNeedsNavigationBackgroundForColor:[UIColor redColor]];
...
//    cell.titleLable.hidden = YES;
    ...
    if (indexPath.row == _index) {
        cell.bgImageView.image = [UIImage imageNamed:@"yp_recharge_icon_yes"];
...
//         cell.bgView.layer.borderColor = [UIColor colorWithHexString:@"#FFFFFF"].CGColor;
        cell.bgImageView.image = [UIImage imageNamed:@"yp_recharge_icon_no"];
...
//    return CGSizeMake((XC_SCREE_W-(15*4))/3, 75);
//    return CGSizeMake((XC_SCREE_W-(22+12))/3, 75);
    return CGSizeMake((XC_SCREE_W-(15*4)-24)/3,XC_SCREE_W/375.0 *95);
}
...
- (void)onRequestcheckBindAliPaySuccess:(id )data{
    ...
    YPFYZfbBindInfoModel* model = [YPFYZfbBindInfoModel yy_modelWithJSON:data];
    ...
    if (self.isBindAlipay) {
        ...
        YPFYBindingWeixinVC* vc =   (YPFYBindingWeixinVC *)[[YPPurseViewControllerFactory sharedFactory] instantiateFYBindingWeixinVC];
...
- (void)onRequestWithDrawMyBankCardSuccess:(NSArray *)list{
    if (list.count>0) {
         YPFYBankBindInfoModel* model = list[0];
...
- (void)onRequestWithDrawMyBankCardFailth:(NSNumber *)resCode{
    ...
    if ( [resCode integerValue] == 16003) {
         YPFYSetBankInfoVC* vc =   (YPFYSetBankInfoVC *)[[YPPurseViewControllerFactory sharedFactory] instantiateFYSetBankInfoVC];
        vc.takeBankCashWayBlock = ^(YPFYBankInfoListModel * _Nonnull model) {
            _desView.hidden = YES;
            _wxView.hidden = NO;
...
-(void)showCachSuccessView:(NSString*)cashNum{
    [YPGetCachSuccessView show:^(HJCachSuccessType type) {
...
//    [self showSelectPayway];
   YPZhiFuBaoAndBankInfoVC* vc = [[YPZhiFuBaoAndBankInfoVC alloc] init];
    vc.cashPayWayBlock = ^(NSInteger accountType, NSString * _Nonnull account, NSString * _Nonnull accountName) {
        self.accountType = accountType;
        self.account = account;
        self.accountName=accountName;
        if (accountType ==1) {
            [self.cashWayButton setImage:[UIImage imageNamed:@"yp_my_zhifu_icon"] forState:UIControlStateNormal];
            [self.cashWayButton setTitle:@"" forState:UIControlStateNormal];
        }else if (accountType ==2){
            [self.cashWayButton setImage:[UIImage imageNamed:@"yp_my_bank_icon"] forState:UIControlStateNormal];
             [self.cashWayButton setTitle:@"" forState:UIControlStateNormal];
        }
...
-(void)showSelectPayway{
    [YPFYCashBindingView show:^(FYCachType type) {
        switch (type) {
                   case FYCashType:
                   {
                       //支付宝
...
-(void)jumpSetBankInfo{
    ...
    [GetCore(YPPurseCore) requestWithDrawMyBankCardWith:[GetCore(YPAuthCoreHelp).getUid userIDValue]];
    
}
//MARK: -选择支付宝支付
-(void)jumpBindingzhifubao{
    
    [GetCore(YPPurseCore) requestPostCheckBindAliPay:[GetCore(YPAuthCoreHelp).getUid userIDValue]];
    
   
}

-(void)showRealNameAuth{
    [YPFYRealNameAuthView show:^(FYRealNameAuthType type) {
               switch (type) {
                          case FYConfirmType:
                          {
                            //jump 认证页面H5
...
- (IBAction)immediatelyCashAction:(id)sender {
   ...
   //         [self showRealNameAuth];
           ...
           [YPAlerVerificationCodeView show:^(HJAlerVerificationCodeType type,NSString* code) {
                        switch (type) {
                                case HJAlerVerificationCodeFirmType:
                                //兑换请求
                              
                                [GetCore(YPPurseCore) requestWithDrawCash:[GetCore(YPAuthCoreHelp).getUid userIDValue] pid:self.selectModel.cashProdId type:self.accountType account:self.account accountName:self.accountName smsCode:code];
                                break;
   ...
   //        [GetCore(YPPurseCore) requestWithDrawCash:[GetCore(YPAuthCoreHelp).getUid userIDValue] pid:self.selectModel.cashProdId type:_accountType account:_account accountName:_accountName smsCode:@"smscode"];
...
-(void)getValidationCode{
        ...
        [YPHttpRequestHelper getMsmWithType:0 Success:^(BOOL succeed) {
