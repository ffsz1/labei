...
@interface YPFYExchangeGoldVC ()<UITextFieldDelegate>

@property (weak, nonatomic) IBOutlet UILabel *goldLabel;//金币余额
...
@property (strong, nonatomic) YPBalanceInfo *balanceInfo2;
...
@implementation YPFYExchangeGoldVC
...
//    [GetCore(YPPurseCore)  requestCheckWhiteListWithUid:uid.userIDValue]; //life
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    self.balanceInfo = GetCore(YPPurseCore).drawExchangeModel;
       self.balanceInfo2 = GetCore(YPPurseCore).balanceInfo;
...
- (IBAction)qucklyChangeAction:(id)sender {
    ...
    //            [GetCore(YPPurseCore) requestWithDrawCash:[GetCore(YPAuthCoreHelp).getUid userIDValue] pid:self.selectModel.cashProdId type:_accountType account:_account accountName:_accountName];
...
-(void)requestChangeGoldWithCode:(NSString*)code{
     NSString *uid = GetCore(YPAuthCoreHelp).getUid;
    [GetCore(YPPurseCore) requestChangeGold:uid.userIDValue smsCode:code diamondNum:[self.changLabel.text doubleValue] roomId:self.idTextField.text];
