...
#import "YPMySpaceHeaderView.h"
#import "YPMySpaceGiftWallVC.h"
#import "YPBaseGestureTableView.h"
#import "YPUserViewControllerFactory.h"
...
@interface YPMySpaceVC ()<UITableViewDelegate,UITableViewDataSource,SDPhotoBrowserDelegate,HJMediaCoreClient,HJFileCoreClient,HJPraiseCoreClient,UIScrollViewDelegate>
...
@property (weak, nonatomic) IBOutlet YPBaseGestureTableView *bgTableView;
...
@property (strong,nonatomic) YPMySpaceHeaderView *sectionView;
...
@property (nonatomic, strong) YPChatRoomInfo *myRoomInfo;
@property (nonatomic, strong) NSString *filePath;

@property (nonatomic, assign) BOOL isAttentioned;

@property (nonatomic,strong) YPMySpaceGiftWallVC *wallVC;
@property (nonatomic,strong) YPMySpaceGiftWallVC *carVC;
@property (nonatomic,strong) YPMySpaceGiftWallVC *headerVC;
...
@property (nonatomic, strong) YPSDCycleScrollView *banaCommonView;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.isThis = YES;
    @weakify(self);
    [[GetCore(YPRoomCoreV2Help) requestRoomInfo:self.userID] subscribeNext:^(id x) {
        @strongify(self)
        self.myRoomInfo = (YPChatRoomInfo *)x;
    }];
...
- (IBAction)moreAction:(id)sender {
    ...
    if (self.isAttentioned) {
        ...
        [YPUserHandler showReport:self.userID cancelFollowBlock:^{
            weakSelf.isAttentioned = NO;
            weakSelf.attentBtn.userInteractionEnabled = YES;
        }];
    }else{
        [YPUserHandler showReport:self.userID cancelFollowBlock:nil];
    }
...
- (IBAction)findHimAction:(id)sender {
    ...
    [YPRoomPusher pushUserInRoomByID:self.userID];
}
...
- (IBAction)hisRoomAction:(id)sender {
    ...
    if (self.myRoomInfo.valid) {
        [YPRoomPusher pushRoomByID:self.userID];
    }
    else {
...
- (IBAction)attendAction:(id)sender {
    if (!self.isAttentioned) {
        ...
        [YPUserHandler follow:self.userID followSucceed:^{
            [MBProgressHUD showSuccess:@"关注成功"];
            wealSelf.isAttentioned = YES;
        }];
        
//        [MBProgressHUD showMessage:NSLocalizedString(XCHudLoadingTip1, nil)];
//        [GetCore(YPPraiseCore) praise:mine.userIDValue bePraisedUid:self.userID];
    }else{
...
//    self.bgTableView.hidden = YES;
    ...
    __weak typeof(self)weakSelf = self;
    [GetCore(YPUserCoreHelp) getUserInfo:self.userID refresh:YES success:^(UserInfo *info) {
...
- (void)checkAttend
{
    [YPHttpRequestHelper isLike:[GetCore(YPAuthCoreHelp).getUid userIDValue] isLikeUid:self.userID success:^(BOOL isLike) {
...
- (void)updatePhotoView
{
    ...
    for (int i = 0; i<self.userInfo.privatePhoto.count; i++) {
        ...
        YPUserPhoto *model = self.userInfo.privatePhoto[i];
...
- (void)updataFanBtn
{
    ...
    self.fansNumLabel.text = [NSString stringWithFormat:@"%ld",_userInfo.fansNum];
...
- (void)updateSexView
{
    ...
    self.sexImageView.image = [UIImage imageNamed:isMale?@"yp_home_attend_man":@"yp_home_attend_woman"];
...
//        NSString* mine = [GetCore(YPAuthCoreHelp) getUid];
//        [MBProgressHUD showMessage:NSLocalizedString(XCHudLoadingTip1, nil)];
//        [GetCore(YPPraiseCore) cancel:mine.userIDValue beCanceledUid:self.userID];
        
        [YPUserHandler cancelFollow:self.userID cancelFollowBlock:^{
...
- (void)setUserID:(UserID)userID
{
    ...
    //判断是否自己的空间
    if ([GetCore(YPAuthCoreHelp).getUid userIDValue] == _userID) {
        self.isMySpace = YES;
    }
...
- (YPMySpaceHeaderView *)sectionView
{
    if (!_sectionView) {
        _sectionView = [[NSBundle mainBundle] loadNibNamed:@"YPMySpaceHeaderView" owner:self options:nil][0];
...
- (YPMySpaceGiftWallVC *)wallVC
{
    if (!_wallVC) {
        ...
        _wallVC = [[YPMySpaceGiftWallVC alloc] initWithCollectionViewLayout:layout];
...
- (YPMySpaceGiftWallVC *)carVC
{
    if (!_carVC) {
        ...
        _carVC = [[YPMySpaceGiftWallVC alloc] initWithCollectionViewLayout:layout];
...
- (YPMySpaceGiftWallVC *)headerVC
{
    if (!_headerVC) {
        ...
        _headerVC = [[YPMySpaceGiftWallVC alloc] initWithCollectionViewLayout:layout];
...
//        _banaCommonView.localizationImageNamesGroup = @[placeholder_image_rectangle];
        ...
        if (self.userInfo.privatePhoto != nil && self.userInfo.privatePhoto.count > 0) {
               ...
               for (YPUserPhoto *info in self.userInfo.privatePhoto) {
