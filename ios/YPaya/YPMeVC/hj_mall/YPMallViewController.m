...
#import "YPCountPopViewController.h"
...
#define XCPropsScrollViewH  (XC_SCREE_H - 210) - XCPropsBottomViewH-50+50

@interface YPMallViewController ()<
...
@implementation YPMallViewController
...
- (void)viewDidLoad {
    ...
    if (_isZuojia) {
        self.title = @"座驾";
        [self.bgScrollView setContentOffset:CGPointMake(0, 0.0) animated:YES];
    }else{
        self.title = @"头饰";
        [self.bgScrollView setContentOffset:CGPointMake(XC_SCREE_W, 0.0) animated:YES];
    }
...
- (void)initView {
    ...
    [self.popularityCollectionView registerNib:[UINib nibWithNibName:@"YPCarCardCell" bundle:nil] forCellWithReuseIdentifier:@"YPCarCardCell"];
...
//                make.right.equalTo(self.view).offset(-30);
    ...
    [self.selectAvatarImgView mas_makeConstraints:^(MASConstraintMaker *make) {
                   make.top.equalTo(self.view).offset(90-3);
                make.centerX.equalTo(self.view);
...
-(void)backPackBtnAction{
    YPPackViewController *pack = [[YPPackViewController alloc] init];
...
- (void)showPopView {
    ...
    YPCountPopViewController *contentVC = [[YPCountPopViewController alloc] init];
...
- (YPZJScrollSegmentView *)segmentView {
    if (!_segmentView) {
        ...
        YPZJScrollSegmentView *segment = [[YPZJScrollSegmentView alloc] initWithFrame:CGRectMake(0, 190, 260, XCPropsSegementH) segmentStyle:style delegate:nil titles:@[@"座驾",@"头饰"] titleDidClick:^(YPZJTitleView *titleView, NSInteger index) {//人气票
...
//        _titleMallLabel.text = @"商城";
        if (_isZuojia) {
            _titleMallLabel.text = @"座驾";
        }else{
            _titleMallLabel.text = @"头饰";
