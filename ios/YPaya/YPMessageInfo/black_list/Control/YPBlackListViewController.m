...
@interface YPBlackListViewController ()<HJImFriendCoreClient>
...
@implementation YPBlackListViewController
...
{
    YPBlackListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPBlackListCell" forIndexPath:indexPath];
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewRowAction *deleteAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDestructive title:NSLocalizedString(XCBlackDelete, nil) handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        UserInfo *info = self.fansArr[indexPath.row];
        [GetCore(YPImFriendCore) removeFromBlackList:[NSString stringWithFormat:@"%lld",info.uid]];
...
- (void)getData
{
    ...
    if (blackList.count > 0) {
        ...
        __weak typeof(self)weakSelf = self;
        [GetCore(YPUserCoreHelp)getUserInfos:idsArr refresh:YES success:^(NSArray *infoArr) {
