...
#import "YPMICRecordTransition.h"
#import "UIVIew+Toast.h"
#import "YPUserCoreHelp.h"
#import "HJUserCoreClient.h"
#import "YPMICRecordVC+Private.h"
#import "YPNavgationTopView.h"

@interface YPMICRecordVC () <HJFileCoreClient, HJMediaCoreClient, HJUserCoreClient, UIViewControllerTransitioningDelegate>

@property (nonatomic, strong) YPMICRecordView *recordView;

@property (nonatomic, strong) YPMICRecordInfoModel *recordInfo;

@property (nonatomic, strong) YPNavgationTopView *navgationTopView;
...
- (UIImageView *)bgImg {
    if (!_bgImg) {
        ...
        _bgImg.image = [UIImage imageNamed:@"yp_match_recordBg"];
    }
    return _bgImg;
}

- (YPNavgationTopView *)navgationTopView {
    if (!_navgationTopView) {
        _navgationTopView = [[NSBundle mainBundle] loadNibNamed:@"YPNavgationTopView" owner:nil options:nil][0];
...
- (void)stopPlay {
    if (self.recordInfo.filePath.length) {
        [GetCore(YPFileCore) cancelVoiceTask:self.recordInfo.filePath];
    }
...
- (YPMICRecordInfoModel *)recordInfo {
    if (!_recordInfo) {
        _recordInfo = [YPMICRecordInfoModel new];
...
- (YPMICRecordView *)recordView {
    if (!_recordView) {
        @weakify(self);
        _recordView = [YPMICRecordView new];
...
//            [self getVoiceText];
        ...
        _recordView.didTapDoneHandler = ^{
            ...
            if (self.recordInfo.localPath.length && self.recordInfo.state == MICRecordStateFinished) {
                ...
                [GetCore(YPFileCore) uploadVoice:self.recordInfo.localPath];
            } else {
                [self.navigationController popViewControllerAnimated:YES];
            }
        };
        ...
        _recordView.didTapPlayHandler = ^{
            ...
            if (self.recordInfo.localPath) {
                if (self.recordInfo.state == MICRecordStatePlaying) {
                    ...
                    if (![GetCore(YPMediaCore) isPlaying]) {
                        [GetCore(YPMediaCore) play:self.recordInfo.localPath];
                    }
                }
            } else {
                if (self.recordInfo.filePath.length) {
                    if (![GetCore(YPMediaCore) isPlaying]) {
                        ...
                        [GetCore(YPFileCore) downloadVoice:self.recordInfo.filePath];
                    }
                }
            }
        };
        ...
        _recordView.saveBlock = ^{
            ...
            [weakSelf stopPlay];
            [MBProgressHUD showMessage:@"上传中..."];
            [GetCore(YPFileCore) uploadVoice:weakSelf.recordInfo.localPath];
