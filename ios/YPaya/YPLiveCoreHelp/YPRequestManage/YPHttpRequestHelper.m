...
//        }
        ...
        NSURL *baseURL = [NSURL URLWithString:[YPHttpRequestHelper getHostUrl]];
        client = [[YYHttpClient alloc] initWithBaseURL:baseURL];
    });
    return client;
}

static YPHTTPRequestConfiguration *HTTPConfiguration() {
    static YPHTTPRequestConfiguration *buffer = nil;
    static dispatch_once_t configurationOnceToken;
    dispatch_once(&configurationOnceToken, ^{
        buffer = [[YPHTTPRequestConfiguration alloc] init];
    });
    return buffer;
}
...
@implementation YPHttpRequestHelper
+ (NSString *)getHostUrl {
...
//        [YYLogger info:@"get请求成功" message:@"method:%@--params:%@--responseObject:%@",method,params,responseObject];
        ...
        if ([resCode integerValue] == HttpResponseSuccess) {
            ...
            [YPHttpRequestHelper logWithTask:task code:resCode.integerValue message:message error:nil];
            if (failure) {
                if (![method isEqualToString:@"user/isBindPhone"] && ![method isEqualToString:@"gift/sendV3"] && ![method isEqualToString:@"room/rcmd/get"] && ![method isEqualToString:@"room/pkvote/get"]&&![method isEqualToString:@"version/get"]&& ![method isEqualToString:@"users/teens/mode/getUsersTeensMode"]) {
        ...
        YPErrorInfo *errorInfo = [YPErrorInfo yy_modelWithJSON:info];
        [YPHttpRequestHelper logWithTask:task code:errorInfo.code.integerValue message:errorInfo.message error:error];
        if (errorInfo != nil) {
...
+ (void)logWithTask:(NSURLSessionDataTask *)task code:(NSInteger)code message:(NSString *)message error:(NSError *)error {
    [YYLogger info:@"request-error--->" message:@"uid:%@ host:%@ body:%@ code:%ld message:%@ error:%@", JX_STR_AVOID_nil(GetCore(YPAuthCoreHelp).getUid), JX_STR_AVOID_nil(task.originalRequest.URL.absoluteString), JX_STR_AVOID_nil(task.originalRequest.HTTPBody.utf8String), code,  JX_STR_AVOID_nil(message), JX_STR_AVOID_nil(error.description)];
}
