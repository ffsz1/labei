...
@interface YPPurseCore()<HJAuthCoreClient, HJNotificationCoreClient>

@end
@implementation YPPurseCore
- (instancetype)init
{
    self = [super init];
    if (self) {
        ...
        _balanceInfo = [YPBalanceInfoStorage getCurrentBalanceInfo];
        if (_balanceInfo == nil) {
            _balanceInfo = [[YPBalanceInfo alloc] init];
        }
    }
    return self;
}
...
- (void)requestInAppPurseProductAndBuy:(NSString *)productID {
    ...
    [YPHttpRequestHelper requestInAppRechargeWithChargeProdId:productID success:^(BOOL orderStatus, NSString *orderID) {
        if (orderStatus) {
                    ...
                    [[IAPShare sharedHelper].iap buyProduct:selProduct onCompletion:^(SKPaymentTransaction *transcation) {
                        ...
                        switch(transcation.transactionState) {
                            case SKPaymentTransactionStatePurchased: {
                                ...
                                [YPHttpRequestHelper checkReceiptWithData:receipt orderID:orderID tranID:transcation.transactionIdentifier success:^(NSString *orderStatus) {
...
- (void)requestRechargeList {
    [YPHttpRequestHelper requestChargeListWith:@(5) success:^(NSArray *rechargeInfo) {
...
{
    @weakify(self)
    [YPHttpRequestHelper requestBalanceInfo:uid success:^(YPBalanceInfo *balance) {
        @strongify(self)
        self.balanceInfo = balance;
        [YPBalanceInfoStorage saveBalanceInfo:balance];
...
- (void)requestWithDrawMyBankCardWith:(UserID)uid {
    [YPHttpRequestHelper requestPostWithDrawMyBankCardWith:uid success:^(NSArray *bankInfoList) {
...
- (void)requestWithDrawBankCardBindWith:(NSMutableDictionary*)dict {
    [YPHttpRequestHelper requestPostWithDrawBankCardBindWith:dict success:^(NSArray *bankInfoList) {
...
- (void)requestBankList {
    [YPHttpRequestHelper requestBankListSuccess:^(NSArray *bankInfoList) {
...
- (void)requestRechargeListForAppStore {
    ...
    [YPHttpRequestHelper requestVerifySetiapChargeListWith:@(1) success:^(NSArray *rechargeInfo) {
        NotifyCoreClient(HJPurseCoreClient, @selector(onRequestRechargeListSuccess:), onRequestRechargeListSuccess:rechargeInfo);
    } failure:^(NSNumber *resCode, NSString *message) {
        NotifyCoreClient(HJPurseCoreClient, @selector(onRequestRechargeListFailth), onRequestRechargeListFailth);
    }];
...
- (void)requestStatpacketGet:(UserID)uid {
    [YPHttpRequestHelper requestGetStatpacketGetWith:uid success:^(NSArray *rechargeInfo) {
...
- (void)requestGetCashList:(UserID)uid {
    [YPHttpRequestHelper requestGetCashListWith:uid success:^(NSArray *rechargeInfo) {
...
- (void)requestRedPacketGetCashList:(UserID)uid {
    [YPHttpRequestHelper requestRedPacketGetCashListWith:uid success:^(NSArray *rechargeInfo) {
...
{
    @weakify(self)
    [YPHttpRequestHelper requestChargeChannelsSuccess:^(id balance) {
        @strongify(self)
...
{
    @weakify(self)
    [YPHttpRequestHelper requestChargeChannelsForWxapiTpye:prod_id pay4id:pay4id path:path success:^(id data) {
          @strongify(self)
...
- (void)requestBalanceInfoWithDrawExchange:(UserID)uid{
    @weakify(self)
       [YPHttpRequestHelper requestBalanceInfoWithDrawExchange:uid success:^(DrawExchangeModel *balance) {
           @strongify(self)
           self.drawExchangeModel = balance;
           [YPBalanceInfoStorage saveDrawExchangeModel:balance];
...
-(void)requestCheckWhiteListWithUid:(UserID)uid{
    ...
    [YPHttpRequestHelper requestCheckWhiteListWithUid:uid success:^(BOOL isSuccess) {
...
- (void)requestChangeGold:(UserID)uid smsCode:(NSString*)smsCode diamondNum:(long)diamondNum roomId:(NSString*)roomId{
    @weakify(self)
    [YPHttpRequestHelper requestChangeGold:uid smsCode:smsCode diamondNum:diamondNum roomId:roomId success:^(ChargeGoldModel *balance) {
           @strongify(self)
           
           DrawExchangeModel* exchangeModel = GetCore(YPPurseCore).drawExchangeModel;
                 YPBalanceInfo* info = GetCore(YPPurseCore).balanceInfo;
           ...
           [YPBalanceInfoStorage saveDrawExchangeModel:exchangeModel];
            [YPBalanceInfoStorage saveBalanceInfo:info];
...
- (void)requestPostWithDrawBound:(UserID)uid  code:(NSString*)code aliPayAccount:(NSString*)aliPayAccount aliPayAccountName:(NSString*)aliPayAccountName {
    @weakify(self)
    [YPHttpRequestHelper requestPostWithDrawBound:uid code:code aliPayAccount:aliPayAccount aliPayAccountName:aliPayAccountName success:^(id data) {
         @strongify(self)
...
- (void)requestPostCheckBindAliPay:(UserID)uid{
    @weakify(self)
       [YPHttpRequestHelper requestPostCheckBindAliPay:uid success:^(id data) {
           @strongify(self)
...
- (void)requestGetUserRealNameStatus:(UserID)uid  type:(NSString*)type{
    @weakify(self)
       [YPHttpRequestHelper requestGetUserRealNameStatus:uid type:type success:^(DrawExchangeModel *balance) {
           @strongify(self)
...
//    @weakify(self)
       [YPHttpRequestHelper requestWithDrawCash:uid  pid:pid  type:type account:account accountName:accountName smsCode:smsCode success:^(DrawExchangeModel *balance) {
//           @strongify(self)
//           self.drawExchangeModel = balance;
//           [YPBalanceInfoStorage saveDrawExchangeModel:balance];
...
- (void)requestRedPacketWithDrawCash:(UserID)uid pid:(long)pid type:(NSInteger)type{
    @weakify(self)
       [YPHttpRequestHelper requestRedPacketWithDrawCash:uid  pid:pid  type:type success:^(redPacketcashModel *balance) {
           @strongify(self)
...
- (void)getSmsWithType:(NSInteger)type {
    [YPHttpRequestHelper getMsmWithType:type Success:^(BOOL isSuccess) {
...
- (void)getCodeWithPhoneNum:(NSString *)phoneNum {
    [YPHttpRequestHelper getCodeWithPhoneNum:phoneNum Success:^(BOOL success) {
        NotifyCoreClient(HJPurseCoreClient, @selector(getSmsSuccess), getSmsSuccess);
    } failure:^(NSNumber *resCode, NSString *message) {
        NotifyCoreClient(HJPurseCoreClient, @selector(getSmsFaildWithMessage:), getSmsFaildWithMessage:message);
    }];
...
- (void)bindingPhoneNum:(NSString *)phoneNum verifyCode:(NSString *)verifyCode {
    [YPHttpRequestHelper bindingPhoneNumber:phoneNum verifyCode:verifyCode Success:^(BOOL success) {
...
- (void)queryFirst {
    ...
    [YPHttpRequestHelper queryFirstWithSuccess:^(BOOL first) {
...
- (void)onKicked
{
    self.balanceInfo = [[YPBalanceInfo alloc] init];
    [YPBalanceInfoStorage saveBalanceInfo:nil];
}
...
- (void)onRecvCustomP2PNoti:(NIMCustomSystemNotification *)notification
{
    ...
    if (attachment.first == Custom_Noti_Header_Account){
        YPBalanceInfo *info = [YPBalanceInfo yy_modelWithDictionary:attachment.data];
