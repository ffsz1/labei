...
@interface YPPhoneCallCore()<HJImMessageCoreClient, AgoraRtcEngineDelegate, HJImMessageCoreClient>
@property(nonatomic, strong) AgoraRtcEngineKit *engine;
@end

@implementation YPPhoneCallCore
{
...
//        
//        YPCallInfo *callInfo = [[YPCallInfo alloc]init];
...
//    [[NIMAVChatSDK sharedSDK].netCallManager hangup:currentCallID];
    currentCallID = 0;
    NotifyCoreClient(HJPhoneCallCoreClient, @selector(onHangup:), onHangup:nil);
//    [self.engine leaveChannel:^(AgoraRtcStats *stat) {
//        if (stat == 0) {
//            YPCallInfo *callInfo = [[YPCallInfo alloc]init];
//            callInfo.uid = [GetCore(AuthCore)getUid].userIDValue;
...
//        YPCallInfo *info = [YPCallInfo yy_modelWithDictionary:attachment.data];
...
- (void)onResponse:(UInt64)callID from:(NSString *)callee accepted:(BOOL)accepted
{
    if (callID == currentCallID) {
        NotifyCoreClient(HJPhoneCallCoreClient, @selector(onResponsePhoneCall:accept:), onResponsePhoneCall:callee accept:accepted);
    }
    if (!accepted) {
        currentCallID = 0;
    }
...
{
    if (callID == currentCallID) {
        NotifyCoreClient(HJPhoneCallCoreClient, @selector(onHangup:), onHangup:user);
    }
    currentCallID = 0;
}
...
{
    if (callID == currentCallID) {
        NotifyCoreClient(HJPhoneCallCoreClient, @selector(onCallEstablished), onCallEstablished);
    }
}
...
{
    if (callID == currentCallID) {
        NotifyCoreClient(HJPhoneCallCoreClient, @selector(onCallDisconnected), onCallDisconnected);
