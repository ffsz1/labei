...
@interface YPImRoomCoreV2()<NIMChatroomManagerDelegate, HJImMessageCoreClient, HJImLoginCoreClient, HJPhoneCallCoreClient,HJWebSocketReceiveCoreClient>
@property (nonatomic, assign) int state;//记录刷新的方式

@end

@implementation YPImRoomCoreV2
...
- (void)onEnterRoomNotifyMessage:(YPChatRoomMember *)member {
...
- (void)onRecvChatRoomNotiMsg:(NIMMessage *)msg{
    if ([msg.session.sessionId isEqualToString:GetCore(YPNotiFriendCore).getRoomid]) {return;}
...
- (void)enterChatRoom:(NSInteger )roomId{
    ...
    [YPIMRequestManager enterRoomWithRoomId:[NSString stringWithFormat:@"%ld",(long)roomId] success:^(YPChatRoomInfo *roomInfo, YPChatRoomMember *member, NSArray<YPIMQueueItem *> *queueItem) {
...
- (void)exitChatRoom:(NSInteger)roomId{
    ...
    //更改切换房间，即构还在上个房间的问题
    [GetCore(YPMeetingCore) leaveMeeting:[NSString stringWithFormat:@"%ld", (long)roomId]];
    
    if (self.currentRoomInfo.uid == GetCore(YPAuthCoreHelp).getUid.userIDValue) {
        self.roomOwner = nil;
    }
    ...
    @weakify(self);
    [YPIMRequestManager exitRoomWithRoomId:[NSString stringWithFormat:@"%ld",(long)roomId] success:^{
        @strongify(self);
        self.currentRoomInfo = nil;
        [self resetAllQueue];
        NotifyCoreClient(HJImRoomCoreClientV2, @selector(onMeExitChatRoomSuccessV2), onMeExitChatRoomSuccessV2);
...
//    [GetCore(RoomQueueCoreV2) removeInMicQueUserArrWith:[NSString stringWithFormat:@"%lld",beKickedUid] roomid:nil islossNet:YES];
    [GetCore(YPRoomQueueCoreV2Help) removeUserPositionLocation:[NSString stringWithFormat:@"%lld",beKickedUid]];
    @weakify(self);
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [YPIMRequestManager kickRoomMemberWithRoomId:[NSString stringWithFormat:@"%ld", (long)self.currentRoomInfo.roomId] account:[NSString stringWithFormat:@"%lld",beKickedUid] uid:[GetCore(YPAuthCoreHelp) getUid] ticket:[GetCore(YPAuthCoreHelp) getTicket] success:^{
            if (didFinish) {
                didFinish();
            }
        } failure:^(NSInteger code, NSString * _Nonnull errorMessage) {
...
- (RACSignal *)rac_queryQueueWithRoomId:(NSString *)roomId{
    return [RACSignal createSignal:^RACDisposable *(id<RACSubscriber> subscriber) {
        [YPIMRequestManager fetchRoomQueueSuccess:^(NSMutableArray<YPIMQueueItem *> * _Nonnull queueList) {
...
- (void)queryQueueWithRoomId:(NSString *)roomId{
    ...
    [YPIMRequestManager fetchRoomQueueSuccess:^(NSMutableArray<YPIMQueueItem *> * _Nonnull queueList) {
        ...
        YPIMQueueItem *micSequence = [self.micQueue safeObjectAtIndex:0];
        [self.micQueue replaceObjectAtIndex:0 withObject:micSequence];
        
        YPIMQueueItem *mySeq = nil;
        for (YPIMQueueItem *item in queueList) {
            YPRoomQueueInfo *sequence = ((YPIMQueueItem *)[self.micQueue safeObjectAtIndex:item.position+1]).queueInfo;
            if (sequence == nil) {
                sequence = [YPRoomQueueInfo new];
            }
            if ([sequence.chatRoomMember.account longLongValue] == [item.queueInfo.chatRoomMember.account longLongValue]) {
                ...
                if ([sequence.chatRoomMember.account longLongValue] != 0 && [sequence.chatRoomMember.account longLongValue] == [GetCore(YPAuthCoreHelp) getUid].integerValue) {
                    mySeq = item;
                }
                break;
            }
...
- (void)queryChartRoomMembersWithUids:(NSString *)uid {
    ...
    [YPIMRequestManager getRoomMembersByIdsWithRoomId:[NSString stringWithFormat:@"%ld",(long)self.currentRoomInfo.roomId] accounts:uid uid:GetCore(YPAuthCoreHelp).getUid ticket:GetCore(YPAuthCoreHelp).getTicket success:^(NSArray<YPChatRoomMember *> *roomMembers) {
        //找出自己的
...
- (void)markManagerList:(UserID)userID enable:(BOOL)enable{
    ...
    [YPIMRequestManager markRoomMamagerWithActionType:type roomId:[NSString stringWithFormat:@"%ld",(long)self.currentRoomInfo.roomId] account:[NSString stringWithFormat:@"%lld",userID] uid:GetCore(YPAuthCoreHelp).getUid ticket:GetCore(YPAuthCoreHelp).getTicket success:^{
        if (enable) {
...
//    [GetCore(RoomQueueCoreV2) removeInMicQueUserArrWith:[NSString stringWithFormat:@"%lld",userID] roomid:nil islossNet:YES];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [YPIMRequestManager markRoomBlackListWithActionType:type roomId:[NSString stringWithFormat:@"%ld",(long)self.currentRoomInfo.roomId] account:[NSString stringWithFormat:@"%lld",userID] uid:GetCore(YPAuthCoreHelp).getUid ticket:GetCore(YPAuthCoreHelp).getTicket success:^{
...
- (void)sendMessage:(NSString *)message success:(void (^)())success
    ...
    [GetCore(YPImMessageCore) sendRoomTextMessage:message roomId:roomId member:member success:success failure:failure];
}
...
- (void)getBackListWithPage:(NSInteger)page {
    ...
    @weakify(self);
    [YPIMRequestManager getRoomBlackMembersWithStart:self.lastBlackPosition count:PageCount roomId:[NSString stringWithFormat:@"%ld", GetCore(YPImRoomCoreV2).currentRoomInfo.roomId] success:^(NSArray<YPChatRoomMember *> *roomMembers) {
        @strongify(self);
...
- (void)fetchRoomRegularMembers {
    @weakify(self);
    [YPIMRequestManager getRoomMembersWithStart:self.lastPosition count:PageCount success:^(NSArray<YPChatRoomMember *> *roomMembers) {
        @strongify(self);
...
//        NSArray *noMicArr = [self removeMicMember:[roomMembers mutableCopy]];
        ...
        for (YPChatRoomMember *item in roomMembers) {
            if (item.is_manager) {
...
- (NSArray *)removeMicMember:(NSMutableArray *)array{
    ...
    [temp enumerateObjectsUsingBlock:^(YPChatRoomMember *member, NSUInteger idx, BOOL * _Nonnull stop) {
        for (YPChatRoomMember *filterMember in arrayFilter) {
...
- (void)resetMicQueueChatRoomMember {
    for (int i = 0 ; i < MIC_COUNT; i++) {
        YPRoomQueueInfo *micSequence = ((YPIMQueueItem *)[self.micQueue safeObjectAtIndex:i]).queueInfo;
        if (micSequence != nil) {
...
- (void)setCurrentRoomInfo:(YPChatRoomInfo *)currentRoomInfo{
...
- (NSMutableArray<YPChatRoomMember *> *)displayMembers{
    if (!_displayMembers) {
        _displayMembers = [NSMutableArray array];
    }
    return _displayMembers;
}
- (NSMutableArray<YPChatRoomMember *> *)onLineManagerMembers{
...
- (NSMutableArray<YPChatRoomMember *> *)allManagers{
    if (!_allManagers) {
        _allManagers = [NSMutableArray array];
    }
    return _allManagers;
}
- (NSMutableArray<YPChatRoomMember *> *)backLists{
    if (!_backLists) {
...
- (NSMutableArray<YPChatRoomMember *> *)micMembers{
    if (!_micMembers) {
...
- (NSMutableArray<YPChatRoomMember *> *)micMembersNoRoomOwner{
    ...
    [_micMembersNoRoomOwner enumerateObjectsUsingBlock:^(YPChatRoomMember * _Nonnull member, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (NSMutableArray<YPChatRoomMember *> *)noMicMembers{
    if (!_noMicMembers) {
...
- (NSMutableArray<YPChatRoomMember *> *)normalMembers{
    if (!_normalMembers) {
