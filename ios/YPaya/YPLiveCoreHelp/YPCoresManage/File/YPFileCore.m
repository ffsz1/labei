...
#import "YPHttpRequestHelper+File.h"
#import "YPUserCoreHelp.h"
#import "GTMBase64.h"
#include <CommonCrypto/CommonDigest.h>
#include <CommonCrypto/CommonHMAC.h>

@implementation YPFileCore
...
//    UIImage *resizedImage = [image resizedImageWithRestrictSize:CGSizeMake(720, 1280)];
    ...
    [[NIMSDK sharedSDK].resourceManager upload:imageFilePath progress:^(float progress) {
        ...
        if (error == nil) {
            [GetCore(YPUserCoreHelp)uploadImageUrlToServer:urlString];
...
- (void)qiNiuUploadImage:(UIImage *)image  uploadType:(UploadImageType)uploadType{
    ...
    [YPHttpRequestHelper uploadImage:image named:nil token:token success:^(NSString *key, NSDictionary *resp) {
...
- (void)uploadFileWithPath:(NSString *)path
    ...
    [YPHttpRequestHelper uploadFileWithPath:path fileName:fileName token:[self token] success:^(NSString *key, NSDictionary *resp) {
