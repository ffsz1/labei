//
//  YPIMRequestManager+RoomMessage.m
//  HJLive
//
...
@implementation YPIMRequestManager (RoomMessage)
...
+ (void)sendMessageWithRoomId:(NSString *)roomId
    ...
    [GetCore(YPWebSocketCore) send:JX_IM_ROUTE_SEND_MESSAGE content:params success:^(id data) {
        !success ?: success();
...
+ (void)sendTextWithRoomId:(NSString *)roomId
    ...
    [GetCore(YPWebSocketCore) send:JX_IM_ROUTE_SEND_TEXT content:params success:^(id data) {
        !success ?: success();
    } failure:^(NSInteger code, NSString *errmsg) {
        !failure ?: failure(code, errmsg);
    }];
