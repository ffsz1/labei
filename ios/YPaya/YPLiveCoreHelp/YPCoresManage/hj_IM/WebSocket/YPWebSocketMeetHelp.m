...
@interface YPWebSocketMeetHelp()<SRWebSocketDelegate>
/**
 *  socket连接标记
 */
...
@implementation YPWebSocketMeetHelp
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _configuration = [YPWebSocketConfiguration new];
    }
    return self;
}
...
- (NSString *)getSocketURL {
    if (GetCore(YPVersionCoreHelp).isReleaseEnv) return JX_IM_WSS_SOCKET_URL_RELEASE;
...
//    NSLog(@"\n\n\n==================webSocketDidClose==================\n%@\n\n\n",webSocket);
    if (self.isDestroy) {return;}
//    if (!wasClean) {
//        [GetCore(YPWebSocketCore).noDeallocSocketList addObject:self];
