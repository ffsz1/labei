...
@interface YPWebSocketCore()<HJWebSocketCoreClient,HJImLoginCoreClient>

@end

@implementation YPWebSocketCore
...
- (void)onConnectSuccess {
    [YPIMRequestManager autoLoginWithSuccess:^{
        NSLog(@"自动登录成功");
...
//    for (YPWebSocketMeetHelp *mi in self.noDeallocSocketList) {
//        [mi ondisconnet];
//    }
 ...
 */
- (void)destroy {
    if (self.YPWebSocketMeetHelp) {
        self.YPWebSocketMeetHelp.isDestroy = true;
        [self.YPWebSocketMeetHelp ondisconnet];
        self.YPWebSocketMeetHelp = nil;
    }
}
 ...
 *  是否在clean中
 */
//- (BOOL)isInNoCleanList:(YPWebSocketMeetHelp *)YPWebSocketMeetHelp {
//    if ([self.noDeallocSocketList containsObject:YPWebSocketMeetHelp]) {
//        return YES;
//    }
//        return false;
//}
...
//    if (![self isInNoCleanList:self.YPWebSocketMeetHelp]) {[self destroy];};
    [self destroy];
    self.YPWebSocketMeetHelp = [[YPWebSocketMeetHelp alloc] init];
    [self.YPWebSocketMeetHelp openScoket];
}
