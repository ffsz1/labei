...
@interface YPWechatTool ()

@property (nonatomic, strong) YPWeChatUserInfo *wechtInfo;

@end

@implementation YPWechatTool
...
- (void)wechatLoginDidSuccessWithCode:(NSString *)code {
    ...
    @weakify(self);
    [YPHttpRequestHelper getWechtAccessTokenWithCode:code appId:JX_WE_CHAT_APP_ID appSecret:JX_WE_CHAT_APP_SECRET success:^(id data) {
        ...
        if (errorCode == 40029) {
            ...
            @weakify(self);
            [YPHttpRequestHelper getWechtUserInfoWithAccessToken:access_token openId:openid success:^(id userInfo) {
                ...
                if (userInfoErrorCode == 40003) {
                    ...
                    self.wechtInfo = [YPWeChatUserInfo yy_modelWithDictionary:userInfoDic];
                    
                    
                    GetCore(YPAuthCoreHelp).qqInfo = nil;
                    GetCore(YPAuthCoreHelp).info = self.wechtInfo;
                    
                    [GetCore(YPAuthCoreHelp) loginWithOpenID:openid andUnionID:unionid access_token:access_token andType:XCThirdPartLoginWc];
                }
