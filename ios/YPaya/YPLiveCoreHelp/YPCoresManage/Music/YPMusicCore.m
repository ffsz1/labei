...
@interface YPMusicCore ()
@property (nonatomic, strong) NSMutableArray<YPMusicInfoModel *> *musicInfoItems;//音乐的基本i信息
@property (nonatomic, strong) YPMusicInfoModel *currentMusicInfo;//音乐的基本i信息
@end

@implementation YPMusicCore

- (void)changeMusicVolume:(CGFloat)volume {
    

    
    [GetCore(YPMeetingCore).engine adjustAudioMixingVolume:volume * 100];
...
- (void)changeManVolume:(CGFloat)volume
{
    ...
    [GetCore(YPMeetingCore).engine adjustRecordingSignalVolume:GetCore(YPMusicCore).manSounds*255];
}
...
- (void)stopMusic {
    ...
    [GetCore(YPMeetingCore).engine stopAudioMixing];
    self.isPlaying = false;
    self.hasPlayer = false;
    self.indexUrl = @"";
    self.indexSongTitle = @"";
    NotifyCoreClient(YPMusicCoreClient, @selector(stopMusicNoti), stopMusicNoti);
}
...
//    [self.backgroundMusicPlayer pause];
    [GetCore(YPMeetingCore).engine resumeAudioMixing];
    self.isPlaying = YES;
}
...
- (void)pauseMusic {
    ...
    [GetCore(YPMeetingCore).engine pauseAudioMixing];
    self.isPlaying = false;
...
- (void)playBackgroundMusicList {
    ...
    NotifyCoreClient(YPMusicCoreClient, @selector(updateList), updateList);
}
...
//    }
    ...
    int result = [GetCore(YPMeetingCore).engine startAudioMixing:url loopback:false replace:false cycle:1];
    ...
    NotifyCoreClient(YPMusicCoreClient, @selector(changeIndexTitle), changeIndexTitle);
...
- (void)getMusicInfoWithUrl:(NSString *)url title:(NSString *)title{
    //获取基本信息
    YPMusicInfoModel *infoModel = [[YPMusicInfoModel alloc] init];
...
- (long)getCurrentDuration {
        ...
        return GetCore(YPMeetingCore).engine.getAudioMixingCurrentPosition;
}
/**
 获取整个文件的播放时长
 @return 文件的播放时长，单位毫秒
...
- (long)getDuration {
        ...
        return GetCore(YPMeetingCore).engine.getAudioMixingDuration;
}
/**
 设置指定的进度进行播放
...
- (void)seekTo:(long)millisecond {
        ...
        [GetCore(YPMeetingCore).engine setAudioMixingPosition:millisecond];
}
...
//- (BOOL)isPlayMusic {
//    return false;
//    if (GetCore(YPMusicCore).backgroundMusicPlayer) {
//        if ([GetCore(YPMusicCore).backgroundMusicPlayer isPlaying]) {
//            return YES;
//        } else {
//            return false;
//        }
...
- (NSMutableArray<YPMusicInfoModel *> *)musicInfoItems{
