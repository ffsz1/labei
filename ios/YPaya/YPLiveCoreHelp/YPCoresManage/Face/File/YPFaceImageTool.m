...
#define SPACES 15
@interface YPFaceImageTool () <CAAnimationDelegate>

@end


@implementation YPFaceImageTool {
...
- (void)addAnimateInImageView:(UIImageView *)imageView receiveInfo:(YPFaceReceiveInfo *)receiveInfo {
    YPFaceConfigInfo *configInfo = [GetCore(YPFaceCore)findFaceInfoById:receiveInfo.faceId];
    if (receiveInfo.resultIndexes.count > 0) {
        /*==================== 动画数组 ================= */
        ...
        for (int i = (short)configInfo.animStartPos; i <= (short)configInfo.animEndPos; i ++) {
            UIImage *image = [GetCore(YPFaceCore) findFaceImageByConfig:configInfo index:i];
            if (image) {
...
//            }else { //普通表情
////                YPFaceConfigInfo *configInfo = [GetCore(FaceCore)findFaceInfoById:item.faceId];
...
- (UIImage *)combineImageInOne:(YPFaceReceiveInfo *)faceInfo size:(CGSize)size {
...
//            height =
    ...
    for (int i = 0; i < faceCount; i++) {
        ...
        YPFaceConfigInfo *configInfo = [GetCore(YPFaceCore)findFaceInfoById:faceInfo.faceId];
        UIImage *singleImage = [GetCore(YPFaceCore)findFaceImageById:faceInfo.faceId index:index];
