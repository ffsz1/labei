...
@implementation YPSignnAlterView
...
//                [YPSignnAlterView showAnimation:data];
            });
...
+ (void)showAnimation:(YPMMHomeInfoModel *)model
{
    YPSignnAlterView *shareView = [[NSBundle mainBundle]loadNibNamed:@"YPSignnAlterView" owner:self options:nil].lastObject;
...
- (void)setModel:(YPMMHomeInfoModel *)model
{
    _model = model;
    ...
    if (model) {
        ...
        if (model.weeklyMissions.count==7) {
            for (int i = 0; i<model.weeklyMissions.count; i++) {
                YPMMHomeItemModel *dayModel = model.weeklyMissions[i];
