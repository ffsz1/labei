...
@interface YPManagerSettingController ()
...
@property (strong, nonatomic) NSMutableArray<YPChatRoomMember *> *list;
@property (weak, nonatomic) IBOutlet UILabel *titleLabel;
@property (assign, nonatomic) NSInteger page;

@end

@implementation YPManagerSettingController
...
- (void)requestForNewData {
    ...
    if (self.type == RoomBlackList) {
        self.page = 1;
        [GetCore(YPImRoomCoreV2) queryManagerorBackList];
        self.tableView.mj_footer.hidden = YES;
    }else if (self.type == RoomManager) {
        [GetCore(YPImRoomCoreV2) getManagerList];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    YPManagerSettingCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPManagerSettingCell" forIndexPath:indexPath];
...
- (void)removeBy:(NSIndexPath *)indexPath {
    if (self.type == RoomBlackList) {
        YPChatRoomMember *user = [self.list safeObjectAtIndex:indexPath.row];
...
- (void)dealListWithMember:(NSArray<YPChatRoomMember *> *)member page:(NSInteger)page {
    if (page == 1) {
...
- (void)configureCell:(YPManagerSettingCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    cell.delegate = self;
    YPChatRoomMember *member = self.list[indexPath.row];
    ...
    [[GetCore(YPImRoomCoreV2) rac_fetchMemberUserInfoByUid:member.account] subscribeNext:^(id x) {
