...
#import "YPPlayRulesView.h"
#import "HJPurseCoreClient.h"
//关闭捡海螺的回调代理要导入
#import "YPRoomlotteryView.h"

@interface YPCallView()

@end

@implementation YPCallView
- (void)awakeFromNib
{
    [super awakeFromNib];
    ...
    _btnSet = [NSMutableSet set];
       self.ownAvetarView.image = [UIImage imageNamed:@"yp_room_call_touxiang_default"];
    //设置刷新数据
    [self setDataAndUpdate];
   
     self.jinbiNumLabel.text =  GetCore(YPPurseCore).balanceInfo.goldNum;
    self.numRemainLabel.text =  [NSString stringWithFormat:@"我的许愿星：%@",GetCore(YPPurseCore).balanceInfo.conchNum];//GetCore(YPPurseCore).balanceInfo.conchNum;
...
-(void)notificationUpdateConchNumAction:(NSNotification*)noti {
    if (noti) {
        if (noti.object) {
            self.numRemainLabel.text = [NSString stringWithFormat:@"我的许愿星：%@",(NSString*)noti.object];//(NSString*)noti.object;
        }
...
- (void)onBalanceInfoUpdate:(YPBalanceInfo *)balanceInfo
{
    ...
    self.numRemainLabel.text =  [NSString stringWithFormat:@"我的许愿星：%@",balanceInfo.conchNum];//balanceInfo.conchNum;
...
-(void)notificationCallModelAction:(NSNotification*)noti {
    if (noti) {
        if (noti.object) {
            YPDaCallModel* model = (YPDaCallModel*)noti.object;
            //打call剩余次数
            self.numRemainLabel.text = [NSString stringWithFormat:@"我的许愿星：%ld",model.conchNum];
            //更新金币数量
...
-(void)setClickBtn{
//    UserInfo *ownerInfo = GetCore(YPImRoomCoreV2).roomOwnerInfo;//房主信息
//          UserInfo *myInfo = [GetCore(YPUserCoreHelp) getUserInfoInDB:GetCore(YPAuthCoreHelp).getUid.userIDValue];//个人信息
       //判断是否是自己的房间
...
- (NSMutableArray *)getArr
{
    ...
    YPChatRoomInfo *roomInfo = GetCore(YPImRoomCoreV2).currentRoomInfo;//房间信息
    UserInfo *ownerInfo = GetCore(YPImRoomCoreV2).roomOwnerInfo;//房主信息
    UserInfo *myInfo = [GetCore(YPUserCoreHelp) getUserInfoInDB:GetCore(YPAuthCoreHelp).getUid.userIDValue];//个人信息
...
+ (void)showCall:(TapCallTypeBlock)tapCallTypeBlock list:(nonnull NSMutableArray *)list
{
    ...
    YPCallView *shareView = [[NSBundle mainBundle]loadNibNamed:@"YPCallView" owner:self options:nil][0];
    shareView.frame = CGRectMake(0, 0, XC_SCREE_W, XC_SCREE_H);
    
    CGFloat height = UIApplication.sharedApplication.statusBarFrame.size.height;
    if (height >= 44.0) {
          // 是机型iPhoneX/iPhoneXR/iPhoneXS/iPhoneXSMax
        shareView.mangerViewHeight.constant = 260;//280
        ...
        shareView.mangerViewHeight.constant = 232;//252
        shareView.bottom_item_layout.constant = 70;
    ...
    if (list.count>0) {
        if (list.count >2) {
          YPGiftInfo* giftModel =  list[0];
            YPGiftInfo* giftModel2 =  list[1];
            YPGiftInfo* giftModel3 =  list[2];
...
-(void)setImgForSelectState:(UIImageView*)selImageView{
    if (_btnSet.count>0) {
        ...
        if ([selImageView isEqual:temp]) {
           temp.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
        }else{
            temp.image = [UIImage imageNamed:@"yp_room_call_touxiang_select_no"];
        }
...
//        self.ownAvetarView.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
//    });
 self.ownAvetarView.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai1Action:(id)sender {
    self.mai1Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai2Action:(id)sender {
    ...
    self.mai2Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
   [self setImgForSelectState:self.mai2Img];
  
}

- (IBAction)mai3Action:(id)sender {
      self.mai3Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai4Action:(id)sender {
    ...
    self.mai4Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai5Action:(id)sender {
     ...
     self.mai5Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai6Action:(id)sender {
    ...
    self.mai6Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai7Action:(id)sender {
    ...
    self.mai7Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)mai8Action:(id)sender {
    ...
    self.mai8Img.image = [UIImage imageNamed:@"yp_room_call_touxiang_select"];
...
- (IBAction)guizeAction:(id)sender {
    [YPPlayRulesView showPlayRulesView];
...
- (IBAction)mufaAction:(id)sender {
    YPGiftInfo* giftModel = self.giftList[0];
    ...
    [_mufaBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select"] forState:UIControlStateNormal];
    [_xiaochuanBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select_no"] forState:UIControlStateNormal];
    [_youtingBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select_no"] forState:UIControlStateNormal];
    
}
//小船
- (IBAction)xiaochuanAction:(id)sender {
    YPGiftInfo* giftModel = self.giftList[1];
       _intoModel.giftId = giftModel.giftId;
     _intoModel.giftNum = 1;
    _intoModel.callName = giftModel.giftName;
     _intoModel.giftUrl = giftModel.giftUrl;
     [_xiaochuanBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select"] forState:UIControlStateNormal];
    [_youtingBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select_no"] forState:UIControlStateNormal];
    [_mufaBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select_no"] forState:UIControlStateNormal];
}
//游艇
- (IBAction)youtingAction:(id)sender {
    YPGiftInfo* giftModel = self.giftList[2];
       _intoModel.giftId = giftModel.giftId;
     _intoModel.giftNum = 1;
    _intoModel.callName = giftModel.giftName;
    _intoModel.giftUrl = giftModel.giftUrl;
     [_youtingBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select"] forState:UIControlStateNormal];
    [_xiaochuanBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select_no"] forState:UIControlStateNormal];
    [_mufaBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_mufa_select_no"] forState:UIControlStateNormal];
}
//捡海螺入口
...
-(YPDaCallIntoModel*)intoModel{
    if (!_intoModel) {
        _intoModel = [[YPDaCallIntoModel alloc] init];
    }
