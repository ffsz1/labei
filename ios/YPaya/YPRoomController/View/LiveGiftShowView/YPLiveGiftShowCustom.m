...
#import "YPLiveGiftShowView.h"

static CGFloat const kGiftViewMargin = 5.0;/**< 两个弹幕之间的高度差 */
...
@interface YPLiveGiftShowCustom ()

@property (nonatomic ,strong) NSMutableDictionary * showViewDict;/**< key([self getDictKey]):value(YPLiveGiftShowView*) */
//用来记录模型的顺序
@property (nonatomic ,strong) NSMutableArray * showViewArr;/**< [YPLiveGiftShowView] */

@end

@implementation YPLiveGiftShowCustom
...
+ (instancetype)addToView:(UIView *)superView{
    YPLiveGiftShowCustom * v = [[YPLiveGiftShowCustom alloc]init];
    [superView addSubview:v];
    //布局
    [v mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.equalTo(@244);//这个改动之后要注意修改HJLiveGiftShowView.h的kViewWidth
        make.height.equalTo(@0.01);
        make.left.equalTo(superView.mas_left);//这个可以任意修改
        make.top.equalTo(superView.mas_bottom).offset(-43);//这个参数在的设定应该注意最大礼物数量时不要超出屏幕边界
    }];
...
- (void)addHJLiveGiftShowModel:(YPLiveGiftShowModel *)showModel{
    [self addHJLiveGiftShowModel:showModel showNumber:0];
}

- (void)addHJLiveGiftShowModel:(YPLiveGiftShowModel *)showModel showNumber:(NSInteger)showNumber{
    if (!showModel || ![showModel isKindOfClass:[YPLiveGiftShowModel class]]) {
        return;
    }
    
    YPLiveGiftShowView * oldShowView = [self.showViewDict objectForKey:[self getDictKey:showModel]];
    
    //判断是否强制修改显示的数字
    BOOL isResetNumber = showNumber > 0 ? YES : NO;

    //如果不存在旧模型
    if (!oldShowView || ![oldShowView isKindOfClass:[YPLiveGiftShowView class]]) {
        //如果当前弹幕数量大于最大限制
        if (self.showViewArr.count >= live_maxGiftShowCount) {
            ...
            if (![self.showViewArr containsObject:kGiftViewRemoved]) {
                //排序 最小的时间在第一个
                NSArray * sortArr = [self.showViewArr sortedArrayUsingComparator:^NSComparisonResult(YPLiveGiftShowView * obj1, YPLiveGiftShowView * obj2) {
                    return [obj1.creatDate compare:obj2.creatDate];
                }];
                YPLiveGiftShowView * oldestView = sortArr.firstObject;
                //重置模型
        ...
        //创建新模型
        YPLiveGiftShowView * newShowView = [[YPLiveGiftShowView alloc]initWithFrame:CGRectMake(0, showViewY, 0, 0)];
        //赋值
        ...
        newShowView.HJLiveGiftShowViewTimeOut = ^(YPLiveGiftShowView * willReMoveShowView){
            //从数组移除
...
- (void)resetY{
    for (int i = 0; i < self.showViewArr.count; i++) {
        YPLiveGiftShowView * show = self.showViewArr[i];
        if ([show isKindOfClass:[YPLiveGiftShowView class]]) {
...
- (void)sortShowArr{
    //如果当前数组包含kGiftViewRemoved 则将kGiftViewRemoved替换到HJLiveGiftShowView之后
    for (int i = 0; i < self.showViewArr.count; i++) {
    ...
    for (int i = 0; i < self.showViewArr.count; i++) {
        for (int j = i; j < self.showViewArr.count; j++) {
            YPLiveGiftShowView * showViewI = self.showViewArr[i];
            YPLiveGiftShowView * showViewJ = self.showViewArr[j];
            if ([showViewI isKindOfClass:[YPLiveGiftShowView class]] && [showViewJ isKindOfClass:[YPLiveGiftShowView class]]) {
...
- (void)searchLiveShowViewFrom:(int)i{
    for (int j = i; j < self.showViewArr.count; j++) {
        YPLiveGiftShowView * next = self.showViewArr[j];
        if ([next isKindOfClass:[YPLiveGiftShowView class]]) {
...
#pragma mark - Private
- (void)resetView:(YPLiveGiftShowView *)view nowModel:(YPLiveGiftShowModel *)model isChangeNum:(BOOL)isChange number:(NSInteger)number{
...
- (NSString *)getDictKey:(YPLiveGiftShowModel *)model{
    //默认以 用户名+礼物类型 为key
