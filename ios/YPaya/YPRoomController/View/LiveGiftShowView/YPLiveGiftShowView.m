...
@implementation YPLiveGiftShowView
...
- (void)setModel:(YPLiveGiftShowModel *)model{
    if (!model) {
        return;
    }
    _model = model;
...
//        _backIV.image = [UIImage imageNamed:@"yp_w_liveGiftBack"];
...
- (YPLiveGiftShowNumberView *)numberView{
    if (!_numberView) {
        YPLiveGiftShowNumberView * nv = [[YPLiveGiftShowNumberView alloc]init];
        [self addSubview:nv];
        _numberView = nv;
    }
    return _numberView;
