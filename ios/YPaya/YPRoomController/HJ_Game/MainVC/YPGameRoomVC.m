...
#import "YPGameRoomVC+Intput.h"
#import "YPGameRoomVC+Delegate.h"
#import "YPGameRoomVC+Constraint.h"
#import "YPGameRoomVC+CoreClient.h"
...
@property (weak, nonatomic) IBOutlet YPMarqueeLabel *roomerNameLabel;

@property (strong,nonatomic) YPRoomBounsListInfo *shenHaoTopMemeber;
...
- (IBAction)popClick:(UIButton *)sender {
    [[YPRoomViewControllerCenter defaultCenter] dismissChannelViewWithQuitCurrentRoom:NO];
}
...
//    [GetCore(YPImRoomCoreV2) checkRoomerIsInRoom];
...
//    [self.navigationController setNavigationBarHidden:YES animated:YES];
    ...
    //????
//    [[GetCore(YPUserCoreHelp) getUserInfoByUidV2:[[GetCore(YPAuthCoreHelp) getUid] intValue]] subscribeNext:^(id x) {}];
...
//      } repeats:YES];
    ...
    NSString* uid = [NSString stringWithFormat:@"%lld",GetCore(YPImRoomCoreV2).currentRoomInfo.uid];
    NSString *charmValue = [[self.charmInfoMap valueForKey:uid] valueForKey:@"value"];
    self.charmFangzhuLabel.text = (charmValue == nil) ?@"0":charmValue;
...
{
    //背景图左上、右上圆角
    CGRect frame = CGRectMake(0, 0, 171, 45);
    if (self.roomOwner.uid == [GetCore(YPAuthCoreHelp) getUid].userIDValue) {
        self.topViewWidth.constant = 171;
        frame = CGRectMake(0, 0, 171, 45);
    }
...
- (void)configPlayImageView {
    ...
    [self.playImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithActionBlock:^(id  _Nonnull sender) {
        ...
        if (GetCore(YPImRoomCoreV2).myMember.is_manager) {
...
- (void)initView {
//    [GetCore(YPActivityCore) getActivity:2];
    ...
    self.messageTableView = [YPMessageTableView loadFromNib];
    ...
    //获取房主信息
    self.roomOwner = GetCore(YPImRoomCoreV2).roomOwnerInfo;
    
    //麦序
    self.positionView = [YPGameRoomPositionView loadFromNib];
...
//    self.discButton.hidden = YES; //默认先隐藏骰子按钮
    ...
    //活动
//    self.activityView = [YPRedPacketActivityView loadFromNib];
...
- (void)roomOwnerUpMic {
    ...
    if(self.roomOwner.uid == [GetCore(YPAuthCoreHelp) getUid].userIDValue) {
        YPIMQueueItem *item = [GetCore(YPImRoomCoreV2).micQueue safeObjectAtIndex:0];
        YPRoomQueueInfo *temp = item.queueInfo;
        ...
        //主坑 设置为房主
        [GetCore(YPRoomQueueCoreV2Help)upMic:-1];
        @weakify(self);
...
- (void)cycleScrollView:(YPSDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index {
    ...
    YPActivityInfo *info = [self.banaCommonArray safeObjectAtIndex:index];
    if (info.skipType == 3) {
        ...
        [[YPRoomViewControllerCenter defaultCenter]presentRoomViewWithRoomOwnerUid:info.skipUrl.userIDValue succ:^(YPChatRoomInfo *roomInfo) {
            if (roomInfo != nil) {
...
- (void)sendText:(NSString *)text {
    ...
    @weakify(self);
    [GetCore(YPImMessageCore) sensitiveWordRegexWithText:text requestId:NSStringFromClass([self class]) finishBlock:^(BOOL isCanSend, NSString *msg) {
        @strongify(self);
        if (isCanSend) {
            ...
            [GetCore(YPImRoomCoreV2) sendMessage:aText success:^{
...
- (void)onSendMessageBanned {
    [UIView showToastInKeyWindow:@"亲，由于您的发言违反平台规定，现已被禁言，若有疑问可联系喜翻鸭客服。" duration:1 position:YYToastPositionCenter];
}
...
- (IBAction)attendBtnAction:(id)sender {
    ...
    NSUInteger myUid = GetCore(YPAuthCoreHelp).getUid.userIDValue;
    
    self.isFollowing = YES;
    if (self.attendBtn.selected) {
        [YPHttpRequestHelper cancel:myUid beCanceledUid:self.roomOwner.uid success:^{
        ...
        [YPHttpRequestHelper praise:myUid bePraisedUid:self.roomOwner.uid success:^{
...
- (void)checkIsFollow:(UserInfo *)info
{
    ...
    [YPHttpRequestHelper isLike:myUid isLikeUid:info.uid success:^(BOOL isLike) {
...
- (void)getShenHaoRankData:(BOOL)isShow
{
    ...
    [YPHttpRequestHelper getRoomShenHaoTop:roomUId Success:^(NSMutableArray *bounsInfoList) {
        if (bounsInfoList.count>0) {
            
            YPRoomBounsListInfo *first = bounsInfoList.firstObject;
            
            if ([first.sumGold floatValue]>=1000) {
                _shenHaoTopMemeber = first;
                [self.shenhaoImageView qn_setImageImageWithUrl:_shenHaoTopMemeber.avatar placeholderImage:@"yp_room_shenhao_avatar" type:ImageTypeUserIcon];
//                [self.roomShenhaoBtn.imageView qn_setImageImageWithUrl:_shenHaoTopMemeber.avatar placeholderImage:@"yp_room_shenhao_avatar" type:ImageTypeUserIcon];
...
//                [self.roomShenhaoBtn setImage:[UIImage imageWithData:imageData] forState:UIControlStateNormal];
        ...
        if (isShow) {
            [YPRoomShenHaoView show:self.shenHaoTopMemeber];
...
- (void)sendManagerMemberRoomIMMsg:(BOOL)isManager uid:(UserID)uid
{
    ...
    __block UserInfo *infoModel = [GetCore(YPUserCoreHelp) getUserInfoInDB:uid];
    if (infoModel == nil) {
...
//    BOOL online = GetCore(YPImRoomCoreV2).roomOwner.is_online;
//    self.userLeaveImg.hidden = online;
...
/** 用户进入退出加数组和动画*/
- (void)onUserEnterChatRoomWith:(YPIMMessage *)message {
...
//    [self showlotteryView];//life-hj
   ...
   NSMutableArray* giftList = [YPGiftInfoStorage getGiftInfosWithtype:GiftTypeNormal];
    for (YPGiftInfo* infoModel in giftList) {
        if (infoModel.giftType ==1) {
            [callGiftList addObject:infoModel];
        }
    }
    
    [callGiftList sortUsingComparator:^NSComparisonResult(YPGiftInfo* _Nonnull obj1, YPGiftInfo* _Nonnull obj2)
    {
    ...
    __weak typeof(self)weakSelf = self;
    [YPCallView showCall:^(HJCallType type,YPDaCallIntoModel* model) {
        switch (type) {
...
-(void)daCallAction:(YPDaCallIntoModel*)model{
   
    //1、请求数据
    
    //MARK: - 打call /gift/callForUser  给单个用户打call
    [YPHttpRequestHelper getRoomCallForUser:model Success:^(YPDaCallModel *callModel) {
...
//打call发送消息
- (void)sendWanfaMessageForChangeInfo:(YPDaCallModel *)callModel{
...
- (void)updateView {
    ...
    if (self.roomInfo) {
        ...
        GetCore(YPGiftCore).isOpenAnimation = !self.roomInfo.giftEffectSwitch;
        
        if (([self.roomInfo.backPicUrl hasPrefix:@"http"] || [self.roomInfo.backPicUrl hasPrefix:@"https"])) {
            [self.bgImg sd_setImageWithURL:[NSURL URLWithString:self.roomInfo.backPicUrl] placeholderImage:[UIImage imageNamed:@"yp_room_bg_default"]];
        } else {
            NSString *imageName = @"yp_room_bg_default";
...
//                self.yellowPoint.hidden = NO;
            ...
            self.onlineNumberLabel.text = [NSString stringWithFormat:@"%ld人在线",GetCore(YPImRoomCoreV2).onlineNumber];
                NSInteger onlineNumber = GetCore(YPImRoomCoreV2).onlineNumber;
            if (onlineNumber<10) {
                [self.onlineNumberBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_onlineBg1"] forState:UIControlStateNormal];
            }else if(onlineNumber<100){
                [self.onlineNumberBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_onlineBg2"] forState:UIControlStateNormal];
            }else{
                [self.onlineNumberBtn setBackgroundImage:[UIImage imageNamed:@"yp_room_onlineBg3"] forState:UIControlStateNormal];
            }
        ...
        NSString *imageName = @"yp_room_bg_default";
        self.bgImg.image = [UIImage imageNamed:imageName];
        GetCore(YPGiftCore).isOpenAnimation = YES;
    }
    
    [self setupHeaderView];
    
//    if (GetCore(YPGiftCore).lastGiftInfo) {
...
#pragma mark - ReceiveGift  YPGameRoomContainerVC 调用
...
- (void)onUserBeAddBlack:(NSString *)uid {
    ...
    if (uid.integerValue == [[GetCore(YPAuthCoreHelp) getUid] integerValue]) {
        // 自己被加入黑名单 退出房间
        [[YPRoomViewControllerCenter defaultCenter] dismissChannelViewWithQuitCurrentRoom:YES];
    }
}
...
- (void)updateOnLineCount{
    self.onlineNumberLabel.text = [NSString stringWithFormat:@"%ld人在线",(long)GetCore(YPImRoomCoreV2).onlineNumber];
    [self.onlineNumberBtn setTitle:[NSString stringWithFormat:@"%ld",(long)GetCore(YPImRoomCoreV2).onlineNumber] forState:UIControlStateNormal];
}
...
{
    if ([self.delegate respondsToSelector:@selector(roomCloseButtonClick)]) {
        [self.delegate roomCloseButtonClick];
    }
}
...
#pragma mark - Getter
- (NSMutableArray<YPGiftInfo *> *)gifQueueArr {
    if (_gifQueueArr == nil) {
...
- (void)talkMessageSelectClick{
    ...
    if (self.messageType == HJMessageTypeAll) {
        ...
        [self.talkMessageSelectBtn setImage:[UIImage imageNamed:@"yp_room_talkType"] forState:UIControlStateNormal];
    }else if(self.messageType == HJMessageTypeTalk){
        self.messageType = HJMessageTypeGift;
        [self.talkMessageSelectBtn setImage:[UIImage imageNamed:@"yp_room_giftType"] forState:UIControlStateNormal];
    }else if(self.messageType == HJMessageTypeGift){
        self.messageType = HJMessageTypeDaCall;
        [self.talkMessageSelectBtn setImage:[UIImage imageNamed:@"yp_room_dacallType"] forState:UIControlStateNormal];
    }else if(self.messageType == HJMessageTypeDaCall){
        self.messageType = HJMessageTypeAll;
        [self.talkMessageSelectBtn setImage:[UIImage imageNamed:@"yp_room_allType"] forState:UIControlStateNormal];
    }
    //通知公屏刷新
...
- (void)setRoomInfo:(YPChatRoomInfo *)roomInfo
{
    _roomInfo = roomInfo;
}
...
- (UIButton *)pkBtn
{
    if (!_pkBtn) {
        ...
        [_pkBtn setImage:[UIImage imageNamed:@"yp_pk_logo"] forState:UIControlStateNormal];
    }
    return _pkBtn;
}
...
- (UIButton *)giftRecordBtn
{
    if (!_giftRecordBtn) {
        ...
        [_giftRecordBtn setImage:[UIImage imageNamed:@"yp_room_giftRecord"] forState:UIControlStateNormal];
...
- (UIButton *)talkMessageSelectBtn{
    if (!_talkMessageSelectBtn) {
        ...
        [_talkMessageSelectBtn setImage:[UIImage imageNamed:@"yp_room_allType"] forState:UIControlStateNormal];
...
- (UIButton *)conchRecordBtn
{
    if (!_conchRecordBtn) {
        ...
        [_conchRecordBtn setImage:[UIImage imageNamed:@"yp_room_jianhailu_record"] forState:UIControlStateNormal];
...
- (YPRoomComeView *)enterTipView
{
    if (!_enterTipView) {
        _enterTipView = [[NSBundle mainBundle] loadNibNamed:@"YPRoomComeView" owner:self options:nil].firstObject;
        ...
        _enterTipView.carBlock = ^(NSString *carUrl) {
            ...
            if (weakSelf != nil) {
                ...
                if (!GetCore(YPImRoomCoreV2).enterRoomInfo.giftCardSwitch) {
