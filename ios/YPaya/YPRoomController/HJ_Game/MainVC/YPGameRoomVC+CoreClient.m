...
#import "YPGameRoomVC+Animation.h"
...
@implementation YPGameRoomVC (CoreClient)

#pragma mark - GiftCoreClient
// 送礼物爆出神秘礼物
- (void)didReceiveSecretGiftWithInfo:(YPGiftSecretInfo *)giftInfo {
...
//            YPFaceConfigInfo *faceInfo = [GetCore(FaceCore)findFaceInfoById:item.faceId];
            [[YPFaceImageTool shareFaceImageTool] queryImage:item imageView:self.playImageView success:^(YPFaceReceiveInfo *info) {
...
- (void)onMicroBeKicked {
    ...
    if ([[YPYYViewControllerCenter currentViewController] isKindOfClass:[YPMusicViewController class]] || [[YPYYViewControllerCenter currentViewController] isKindOfClass:[YPAddMusicViewController class]]) {
...
- (void)getActivityInfoSuccess {
        ...
        self.activityInfo = GetCore(YPActivityCore).activityInfo;
...
- (void)getAllActivityInfoSuccess:(NSArray *)arr {
    ...
    if (arr.count>0) {
        ...
        if (arr != nil && arr.count > 0) {
                     ...
                     for (YPActivityInfo *info in arr) {
...
- (void)onReceiveGift:(YPGiftAllMicroSendInfo *)giftReceiveInfo isALLChannelSend:(BOOL)isALLChannelSend {
    ...
    if (!isALLChannelSend) {
        ...
        YPGiftAllMicroSendInfo *info = giftReceiveInfo;
        BOOL isAll = NO;
