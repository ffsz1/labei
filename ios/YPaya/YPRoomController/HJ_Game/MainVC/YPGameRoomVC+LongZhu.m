...
#import "YPGameRoomPeiDuiChooseView.h"
...
@implementation YPGameRoomVC (LongZhu)

- (void)updateLongZhu {
    
    if (GetCore(YPImRoomCoreV2).currentRoomInfo.roomId != 0) {
        
        if ([GetCore(YPAuthCoreHelp) getUid].integerValue == self.roomInfo.uid) {
            [GetCore(YPLongZhuCore) getStateWithRoomId:GetCore(YPImRoomCoreV2).currentRoomInfo.roomId];
        }
        else {
...
- (void)setupLongZhuView {
    ...
    GetCore(YPLongZhuCore);
    
    YPGameRoomPeiDuiEnterView *longZhuView = [[[NSBundle mainBundle] loadNibNamed:@"YPGameRoomPeiDuiEnterView" owner:self options:nil] lastObject];
...
- (void)showSupeiView {
    YPGameRoomPeiDuiSuPeiView *view = [[NSBundle mainBundle] loadNibNamed:@"YPGameRoomPeiDuiSuPeiView" owner:nil options:nil][0];
...
//    view.backgroundColor = [UIColor redColor];
    ...
    [view setRuleBtnActionBlock:^{
        @strongify(view);
        YPGameRoomPeiDuiRuleView *ruleView = [[NSBundle mainBundle] loadNibNamed:@"YPGameRoomPeiDuiRuleView" owner:nil options:nil][0];
    ...
    [view setSupeiBtnActionBlock:^{
        ...
        [GetCore(YPLongZhuCore) getChooseResultWithRoomId:self.roomInfo.roomId type:1 result:nil];
    }];
    
    [view setCancelBtnActionBlock:^{
        @strongify(self);
        [MBProgressHUD showMessage:@""];
        [GetCore(YPLongZhuCore) cancelChooseResultWithRoomId:self.roomInfo.roomId type:1 result:[@(self.didSeletedNum) description]];
    ...
    [view setShowBtnActionBlock:^{
        @strongify(self);
        [MBProgressHUD showMessage:@""];
        [GetCore(YPLongZhuCore) confirmResult:[@(self.didSeletedNum) description] roomId:self.roomInfo.roomId type:1];
    }];
...
- (void)showChooseView {
    ...
    YPGameRoomPeiDuiChooseView *view = [[NSBundle mainBundle] loadNibNamed:@"YPGameRoomPeiDuiChooseView" owner:nil options:nil][0];
    ...
    [view setChooseBtnActionBlock:^{
        ...
        [MBProgressHUD showMessage:@""];
        [GetCore(YPLongZhuCore) getChooseResultWithRoomId:GetCore(YPImRoomCoreV2).currentRoomInfo.roomId type:2 result:[@(self.didSeletedNum) description]];
    }];
    ...
    [view setShowBtnActionBlock:^{
        ...
        @strongify(self);
        [MBProgressHUD showMessage:@""];
        [GetCore(YPLongZhuCore) confirmResult:[@(self.didSeletedNum) description] roomId:GetCore(YPImRoomCoreV2).currentRoomInfo.roomId type:2];
    }];
...
- (void)sendMsgWithNumArr:(NSArray *)numArr isShowed:(BOOL)isShowed {
    ...
    YPRoomLongZhuMsgModel *model = [YPRoomLongZhuMsgModel new];
    model.uid = [GetCore(YPAuthCoreHelp) getUid];
    model.nick = myMember.nick.length ? myMember.nick : @" ";
    model.isShowd = isShowed;
    model.numArr = numArr == nil ? @[] : numArr;
    model.level = [GetCore(YPUserCoreHelp) getUserInfoInDB:[GetCore(YPAuthCoreHelp) getUid].integerValue].experLevel;
    ...
    [GetCore(YPImMessageCore) sendCustomMessageAttachement:attachement sessionId:[NSString stringWithFormat:@"%ld",(long)GetCore(YPImRoomCoreV2).currentRoomInfo.roomId] type:JXIMSessionTypeChatroom];
}
...
- (void)sendCancelMsg
{
    ...
    attachement.experLevel = [GetCore(YPUserCoreHelp) getUserInfoInDB:[GetCore(YPAuthCoreHelp) getUid].integerValue].experLevel;
