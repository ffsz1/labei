...
@implementation YPGiftCell

- (void)setGiftModel:(YPGiftInfo *)giftModel
{
    ...
    if (giftModel) {
        ...
        if (giftModel.hasTimeLimit) {
            ...
            self.firstImgView.image = [UIImage imageNamed:@"yp_gift_xianshi_icon"];
            if (giftModel.hasLatest) {
                self.secondImgView.hidden = NO;
                self.secondImgView.image = [UIImage imageNamed:@"yp_gift_zuixin_icon"];
                if (giftModel.hasEffect) {
                    self.thirdImgView.hidden = NO;
                     self.thirdImgView.image = [UIImage imageNamed:@"yp_gift_texiao_icon"];
                }
            }
        }else if (giftModel.hasLatest){
            self.firstImgView.hidden = NO;
             self.firstImgView.image = [UIImage imageNamed:@"yp_gift_zuixin_icon"];
            if (giftModel.hasEffect) {
                 self.secondImgView.hidden = NO;
                  self.secondImgView.image = [UIImage imageNamed:@"yp_gift_texiao_icon"];
            }
        }else if (giftModel.hasEffect){
             self.firstImgView.hidden = NO;
             self.firstImgView.image = [UIImage imageNamed:@"yp_gift_texiao_icon"];
        }
...
{
    self.selImageView.image = [UIImage imageNamed:isSel?@"yp_gift_cell_sel":@"yp_gift_cell_normal"];
}
