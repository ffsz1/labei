...
@interface YPRoomBackgroundSettingVC () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) UICollectionView *collectionView;

@property (nonatomic, strong) __block NSMutableArray<YPRoomBgModel *> *items;
@property (nonatomic, assign) NSInteger selectedItemId;

@end

@implementation YPRoomBackgroundSettingVC
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    YPRoomBackgroundSeetingCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:HJRoomBackgroundSeetingCellID forIndexPath:indexPath];
    [self configureCell:cell atIndexPath:indexPath];
    return cell;
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    YPRoomBgModel *roomInfo = [self.items objectOrNilAtIndex:indexPath.row];
...
- (void)loadDatas {
    [self clearDatas];
        ...
        @weakify(self);
        [GetCore(YPRoomCoreV2Help) roomBgListWithRoomId:GetCore(YPImRoomCoreV2).currentRoomInfo.roomId success:^(NSArray *list) {
            @strongify(self);
...
- (YPRoomBgModel *)getDefaultItem {
    YPRoomBgModel *item = [YPRoomBgModel new];
    item.id = 0;
    item.picUrl = @"yp_room_bg_default";
    item.name = @"默认";
    return item;
}
...
- (void)configureCell:(YPRoomBackgroundSeetingCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    YPRoomBgModel *roomInfo = [self.items objectOrNilAtIndex:indexPath.row];
...
- (NSMutableArray<YPRoomBgModel *> *)items {
    if (!_items) {
        _items = @[].mutableCopy;
    }
    return _items;
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[YPRoomBackgroundSeetingCell class] forCellWithReuseIdentifier:HJRoomBackgroundSeetingCellID];
    }
