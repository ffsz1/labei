...
#import "YPRoomSettingPlayInfoVC.h"
#import <IQTextView.h>
...
@interface YPRoomSettingPlayInfoVC ()<
HJRoomCoreClient,
UITextViewDelegate
>
...
@implementation YPRoomSettingPlayInfoVC
...
- (void)viewDidLoad {
    ...
    self.textView.text = GetCore(YPImRoomCoreV2).currentRoomInfo.playInfo;
...
- (void)doneClick {
    ...
    if (GetCore(YPImRoomCoreV2).myMember.is_creator) {
        [GetCore(YPRoomCoreV2Help) updateGameRoomInfo:GetCore(YPImRoomCoreV2).currentRoomInfo.uid backPic:nil title:nil roomTopic:GetCore(YPImRoomCoreV2).currentRoomInfo.roomDesc roomNotice:GetCore(YPImRoomCoreV2).currentRoomInfo.roomNotice roomPassword:nil tag:GetCore(YPImRoomCoreV2).currentRoomInfo.tagId playInfo:self.textView.text giftEffectSwitch:GetCore(YPImRoomCoreV2).currentRoomInfo.giftEffectSwitch giftCardSwitch:GetCore(YPImRoomCoreV2).currentRoomInfo.giftEffectSwitch publicChatSwitch:GetCore(YPImRoomCoreV2).currentRoomInfo.publicChatSwitch];
    } else if (GetCore(YPImRoomCoreV2).myMember.is_manager) {
        [GetCore(YPRoomCoreV2Help) managerUpdateGameRoomInfo:[GetCore(YPAuthCoreHelp)getUid].userIDValue backPic:nil title:nil roomTopic:GetCore(YPImRoomCoreV2).currentRoomInfo.roomDesc roomNotice:GetCore(YPImRoomCoreV2).currentRoomInfo.roomNotice roomPassword:nil tag:GetCore(YPImRoomCoreV2).currentRoomInfo.tagId playInfo:self.textView.text giftEffectSwitch:GetCore(YPImRoomCoreV2).currentRoomInfo.giftEffectSwitch giftCardSwitch:GetCore(YPImRoomCoreV2).currentRoomInfo.giftEffectSwitch publicChatSwitch:GetCore(YPImRoomCoreV2).currentRoomInfo.publicChatSwitch];
    }
}
