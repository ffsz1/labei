...
@interface YPRoomRankView ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,strong) YPRoomRankTopView *topView;
...
@implementation YPRoomRankView

+ (void)show:(XBDRoomRankCardBlock)cardBlock
{
    YPRoomRankView *shareView = [[NSBundle mainBundle]loadNibNamed:@"YPRoomRankView" owner:self options:nil][0];
...
- (void)awakeFromNib
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"YPRoomRankCell" bundle:nil] forCellReuseIdentifier:@"YPRoomRankCell"];
}
...
{
    YPRoomRankCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPRoomRankCell" forIndexPath:indexPath];
    
    if (indexPath.row <self.dataArr.count-3) {
        YPRoomBounsListInfo *model = self.dataArr[indexPath.row+3];
...
- (IBAction)richAction:(id)sender {
    ...
    [self layoutIfNeeded];
    [UIView animateWithDuration:0.3 animations:^{
        self.richBtnBottomImageView.image = [UIImage imageNamed:@"yp_room_rank_wealth_bottom"];
...
- (IBAction)charmAction:(id)sender {
    ...
    [self layoutIfNeeded];
    [UIView animateWithDuration:0.3 animations:^{
        self.richBtnBottomImageView.image = [UIImage imageNamed:@"yp_room_rank_charm_bottom"];
        self.centerX_type.constant = 53;
        [self layoutIfNeeded];
    }];
...
{
    self.typeBgImageView.image = [UIImage imageNamed:isCharm?@"yp_room_rank_charmbkg":@"yp_room_rank_wealthbkg"];
    self.dateTypeImageView.image = [UIImage imageNamed:isCharm?@"yp_room_rank_charmslider":@"yp_room_rank_wealthslider"];
...
//    [self.monthBtn setTitleColor:isCharm?UIColorHex(FF80B3):UIColorHex(C280FF) forState:UIControlStateNormal];
    ...
    if (!isCharm) {
        [self.dayBtn setTitleColor:UIColorHex(8A58FF) forState:UIControlStateNormal];
        [self.weekBtn setTitleColor:UIColorHex(8A58FF) forState:UIControlStateNormal];
        [self.monthBtn setTitleColor:UIColorHex(8A58FF) forState:UIControlStateNormal];
    }else{
        [self.dayBtn setTitleColor:UIColorHex(F25499) forState:UIControlStateNormal];
        [self.weekBtn setTitleColor:UIColorHex(F25499) forState:UIControlStateNormal];
        [self.monthBtn setTitleColor:UIColorHex(F25499) forState:UIControlStateNormal];
    }
...
- (YPRoomRankTopView *)topView
{
    if (!_topView) {
        _topView = [[NSBundle mainBundle] loadNibNamed:@"YPRoomRankTopView" owner:self options:nil].firstObject;
        _topView.frame = CGRectMake(0, 110, kScreenWidth, 230);
