...
@implementation YPEggRecordListView

+ (void)showRecord
{
    [YPEggRecordListView loadXib];
}


+ (void)loadXib
{
    YPEggRecordListView *shareView = [[NSBundle mainBundle]loadNibNamed:@"YPEggRecordListView" owner:self options:nil].lastObject;
...
- (void)awakeFromNib
{
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"YPEggRecordCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"YPEggRecordCell"];
    [self getData];
...
{
    YPEggRecordCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YPEggRecordCell" forIndexPath:indexPath];
...
- (void)getData
{
    ...
    [YPHttpRequestHelper getEggRecord:self.tableView.page+1 success:^(NSArray * _Nonnull arr) {
