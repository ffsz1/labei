...
#import "NSString+JXEmoji.h"

@implementation UIImage (JXBase)

#pragma mark - Base
- (CGSize)jx_sizeInPixel {
    return CGSizeMake(self.size.width * self.scale, self.size.height * self.scale);
}

- (UIColor *)jx_averageColor {
    /*
...
+ (UIImage *)jx_imageWithSize:(CGSize)size drawBlock:(void (^)(CGContextRef context))drawBlock {
    return [self jx_imageWithSize:size opaque:NO scale:0 drawBlock:drawBlock];
}

+ (UIImage *)jx_imageWithSize:(CGSize)size opaque:(BOOL)opaque scale:(CGFloat)scale drawBlock:(void (^)(CGContextRef context))drawBlock {
...
- (BOOL)_jx_compareWithImage:(UIImage *)image tolerance:(CGFloat)tolerance {
    /*
    ...
    if (tolerance == 0) {
        ...
        JXComparePixel *p1 = referenceImagePixels;
        JXComparePixel *p2 = imagePixels;
...
- (BOOL)jx_isEqualToImage:(UIImage *)other {
    if (!other) return NO;
    if (![other isKindOfClass:[UIImage class]]) return NO;
    if (self == other) return YES;
    
    return [self _jx_compareWithImage:other tolerance:0];
}
...
#pragma mark - Modify
- (void)jx_drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = JXCGRectFitWithContentMode(rect, self.size, contentMode);
...
- (UIImage *)jx_imageByAddingImage:(UIImage *)image toPoint:(CGPoint)point {
    return [UIImage jx_imageWithSize:self.size opaque:![self jx_hasAlphaChannel] scale:self.scale drawBlock:^(CGContextRef  _Nonnull context) {
...
#pragma mark - Resize
- (UIImage *)jx_imageByResizeToWidth:(CGFloat)width {
    return [self jx_imageByResizeToWidth:width scale:self.scale];
}

- (UIImage *)jx_imageByResizeToWidth:(CGFloat)width scale:(CGFloat)scale {
    if (self.size.width <= 0 || self.size.height <= 0) return nil;
    CGFloat height = width * self.size.height / self.size.width;
    return [self jx_imageByResizeToSize:CGSizeMake(width, height) scale:scale];
}

- (UIImage *)jx_imageByResizeToSize:(CGSize)size {
    return [self jx_imageByResizeToSize:size scale:self.scale];
}

- (UIImage *)jx_imageByResizeToSize:(CGSize)size scale:(CGFloat)scale {
...
- (UIImage *)jx_imageByResizeToSize:(CGSize)size contentMode:(UIViewContentMode)contentMode {
    if (size.width <= 0 || size.height <= 0) return nil;
    UIGraphicsBeginImageContextWithOptions(size, NO, self.scale);
    [self jx_drawInRect:CGRectMake(0, 0, size.width, size.height) withContentMode:contentMode clipsToBounds:NO];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return image;
}
...
- (UIImage *)jx_imageByFixToOrientationUp {
...
+ (UIImage *)jx_imageWithEmoji:(NSString *)emoji size:(CGFloat)size {
    if (emoji.length == 0 || ![emoji jx_isEmoji]) return nil;
