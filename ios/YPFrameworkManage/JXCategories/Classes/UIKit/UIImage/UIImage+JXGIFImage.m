...
@implementation UIImage (JXGIFImage)
...
static NSTimeInterval JXCGImageSourceGetGIFFrameDelayAtIndex(CGImageSourceRef source, size_t index) {
...
+ (UIImage *)jx_imageWithSmallGIFData:(NSData *)data scale:(CGFloat)scale {
    ...
    for (size_t i = 0; i < count; i++) {
        NSTimeInterval delay = JXCGImageSourceGetGIFFrameDelayAtIndex(source, i);
        totalTime += delay;
...
+ (UIImage *)jx_imageWithSmallGIFNamed:(NSString *)name {
    CGFloat scale = [UIScreen mainScreen].scale;
    NSString *retinaPath = [[NSBundle mainBundle] pathForResource:name ofType:@"gif"];
    NSData *data = [NSData dataWithContentsOfFile:retinaPath];
    return [self jx_imageWithSmallGIFData:data scale:scale];
}
