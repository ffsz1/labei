//
//  UIView+JXBase.h
//  JXCategories
//
//  Created by Colin on 16/12/31.
//  Copyright © 2016年 Colin. All rights reserved.
//

#import <UIKit/UIKit.h>

NS_ASSUME_NONNULL_BEGIN

@interface UIView (JXBase)

#pragma mark - Base
/**
 View可见的Alpha值(综合考虑Subview及Window情况)
 */
@property (nonatomic, readonly) CGFloat jx_visibleAlpha;

/**
 View最顶部的SuperView(无则为self)
 */
@property (nonatomic, readonly) UIView *jx_topSuperView;

/**
 View所属的ViewController(或为nil)
 */
@property (nullable, nonatomic, readonly) UIViewController *jx_viewController;

/**
 将View移动到父View的最前面
 */
- (void)jx_bringToFront;

/**
 将View移动到父View的最后面
 */
- (void)jx_sendToBack;

/**
 移除View内指定的Subview(勿在view的drawRect:方法内调用此方法)

 @param subview 指定的Subview
 */
- (void)jx_removeSubview:(UIView *)subview;

/**
 移除所有Subviews(勿在view的drawRect:方法内调用此方法)
 */
- (void)jx_removeAllSubviews;

/**
 判断View是否包含指定的子视图(递归遍历子视图)

 @param subview 指定的子视图
 @return 包含返回YES, 否则返回NO
 */
- (BOOL)jx_containsSubview:(UIView *)subview;

#pragma mark - Layout
@property (nonatomic, assign) CGFloat jx_x;       ///< self.frame.origin.x
@property (nonatomic, assign) CGFloat jx_y;       ///< self.frame.origin.y
@property (nonatomic, assign) CGPoint jx_origin;  ///< self.frame.origin
@property (nonatomic, assign) CGFloat jx_width;   ///< self.frame.size.width
@property (nonatomic, assign) CGFloat jx_height;  ///< self.frame.size.height
@property (nonatomic, assign) CGSize  jx_size;    ///< self.frame.size
@property (nonatomic, assign) CGFloat jx_centerX; ///< self.center.x
@property (nonatomic, assign) CGFloat jx_centerY; ///< self.center.y
@property (nonatomic, assign) CGFloat jx_left;    ///< self.frame.origin.x
@property (nonatomic, assign) CGFloat jx_top;     ///< self.frame.origin.y
@property (nonatomic, assign) CGFloat jx_right;   ///< self.frame.origin.x + self.frame.size.width
@property (nonatomic, assign) CGFloat jx_bottom;  ///< self.frame.origin.y + self.frame.size.height

- (void)jx_originXEqualToView:(UIView *)view;
- (void)jx_originYEqualToView:(UIView *)view;
- (void)jx_originEqualToView:(UIView *)view;

- (void)jx_widthEqualToView:(UIView *)view;
- (void)jx_heightEqualToView:(UIView *)view;
- (void)jx_sizeEqualToView:(UIView *)view;

- (void)jx_centerXEqualToView:(UIView *)view;
- (void)jx_centerYEqualToView:(UIView *)view;
- (void)jx_centerEqualToView:(UIView *)view;

- (void)jx_leftEqualToView:(UIView *)view;
- (void)jx_topEqualToView:(UIView *)view;
- (void)jx_rightEqualToView:(UIView *)view;
- (void)jx_bottomEqualToView:(UIView *)view;

#pragma mark - Convert
/**
  将Point由Point所在视图转换到目标视图View或Window中

 @param point Point
 @param view 目标视图view或window
 @return 在目标视图view中或window的point
 */
- (CGPoint)jx_convertPoint:(CGPoint)point toViewOrWindow:(UIView *)view;

/**
 将Point从Point所在View或Window中转换到当前视图中

 @param point Point
 @param view Point所在View或Window
 @return 在当前视图中的Point值
 */
- (CGPoint)jx_convertPoint:(CGPoint)point fromViewOrWindow:(UIView *)view;

/**
 将Rect由Rect所在视图转换到目标视图View或Window中

 @param rect Rect
 @param view 目标视图View或Window
 @return 在目标视图View中或Window的Rect
 */
- (CGRect)jx_convertRect:(CGRect)rect toViewOrWindow:(UIView *)view;

/**
 将Rect从Rect所在View或Window中转换到当前视图中

 @param rect Rect
 @param view Rect所在View或Window
 @return 在当前视图中的Rect值
 */
- (CGRect)jx_convertRect:(CGRect)rect fromViewOrWindow:(UIView *)view;

#pragma mark - Count Down
/**
 根据倒计时间及倒计回调处理, 改变View
 
 @param seconds 倒计时间
 @param countDownHandler 倒计回调处理(每秒执行一次回调, 当且仅当倒计结束, finished为YES)
 */
- (void)jx_changeWithCountDown:(NSInteger)seconds countDownHandler:(void (^)(id sender, NSInteger second, BOOL finished))countDownHandler;

#pragma mark - Nib
/**
 根据Nib文件名(当前View类名)，创建对应的View(MainBundle, 或为nil)
 
 @return View
 */
+ (instancetype)jx_viewFromNib;

/**
 根据Nib文件名，创建对应的View(MainBundle, 或为nil)
 
 @param name Nib文件名
 @return View
 */
+ (instancetype)jx_viewFromNib:(NSString *)name;

#pragma mark - Shadow
/**
 设置View的阴影
 
 @param color  阴影颜色
 @param offset 阴影偏移量
 @param radius 阴影角度
 */
- (void)jx_setLayerShadow:(UIColor*)color offset:(CGSize)offset radius:(CGFloat)radius;

#pragma mark - Snapshot
/**
 获取当前View截图
 
 @return 当前View截图
 */
- (UIImage *)jx_snapshotImage;

/**
 获取当前View截图(当前屏幕所有重绘操作完成后截图)
 
 @param afterUpdates 是否在屏幕完成所有重绘操作后截屏
 @return 当前view截图
 */
- (UIImage *)jx_snapshotImageAfterScreenUpdates:(BOOL)afterUpdates;

@end

NS_ASSUME_NONNULL_END
