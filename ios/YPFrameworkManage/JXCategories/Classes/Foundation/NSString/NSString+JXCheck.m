...
@implementation NSString (JXCheck)

- (BOOL)jx_isDecimalNumber {
    if (!self.length) return NO;
    
    return self.jx_isPureInt;
}
...
- (BOOL)jx_isAlphanumericCharacter {
    if (!self.length) return NO;
...
- (BOOL)_jx_isAreaCodeFromCardIDOfPRC {
    ...
    return [dict jx_containsObjectForKey:self];
}

- (BOOL)jx_isValidCardIDOfPRC {
    if (!self.length) return NO;
    ...
    if (![sProvince _jx_isAreaCodeFromCardIDOfPRC]) return NO;
...
- (BOOL)jx_isValidCardIDOfHK {
    ...
    BOOL validate = [self jx_matchesRegex:regex options:0];
    ...
    if (validate) {
        ...
        if (remainder == 10) {
            validate = [lastCode jx_matchesRegex:@"^[Aa]" options:0];
        } else {
            if ([lastCode jx_matchesRegex:@"^[Aa]" options:0]) {
                validate = NO;
            } else {
