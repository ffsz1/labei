...
NSString *const kJXKeychainErrorDomain = @"com.samsoffes.sskeychain";

NSString *const kJXKeychainAccountKey = @"acct";
NSString *const kJXKeychainCreatedAtKey = @"cdat";
NSString *const kJXKeychainClassKey = @"labl";
NSString *const kJXKeychainDescriptionKey = @"desc";
NSString *const kJXKeychainLabelKey = @"labl";
NSString *const kJXKeychainLastModifiedKey = @"mdat";
NSString *const kJXKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef JXKeychainAccessibilityType = NULL;
#endif

@interface JXKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation JXKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kJXKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (JXKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return JXKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (JXKeychainAccessibilityType) {
		CFRelease(JXKeychainAccessibilityType);
	}
	JXKeychainAccessibilityType = accessibilityType;
}
#endif
