 ...
 */
package com.xchat.oauth2.service.client.http;
...
public class OAuth2ErrorHandler implements ResponseErrorHandler {
	...
	private void maybeThrowExceptionFromHeader(String authenticateHeader, String headerType) {
		...
		if (authenticateHeader.toLowerCase().startsWith(headerType)) {
			Map<String, String> headerEntries = com.xchat.oauth2.service.client.http.StringSplitUtils.splitEachArrayElementAndCreateMap(
					com.xchat.oauth2.service.client.http.StringSplitUtils.splitIgnoringQuotes(authenticateHeader.substring(headerType.length()), ','), "=",
					"\"");
