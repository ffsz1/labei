...
package com.xchat.oauth2.service.provider.token.store;
...
import com.xchat.oauth2.service.provider.approval.Approval;
...
/**
 * A {@link com.xchat.oauth2.service.provider.token.TokenStore} implementation that just reads data from the tokens themselves. Not really a store since it
 * never persists anything, and methods like {@link #getAccessToken(com.xchat.oauth2.service.provider.OAuth2Authentication)} always return null. But
 * nevertheless a useful tool since it translates access tokens to and from authentications. Use this wherever a
 * {@link com.xchat.oauth2.service.provider.token.TokenStore} is needed, but remember to use the same {@link com.xchat.oauth2.service.provider.token.store.JwtAccessTokenConverter} instance (or one with the same
