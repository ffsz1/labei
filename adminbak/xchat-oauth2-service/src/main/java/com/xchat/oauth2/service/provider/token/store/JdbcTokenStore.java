...
import com.xchat.oauth2.service.provider.token.AuthenticationKeyGenerator;
import com.xchat.oauth2.service.provider.token.DefaultAuthenticationKeyGenerator;
...
public class JdbcTokenStore implements TokenStore {
	...
	private static final String DEFAULT_REFRESH_TOKEN_INSERT_STATEMENT = "insert into oauth_refresh_token (token_id, token, authentication, expiration) values (?, ?, ?, ?)";
	...
	public void storeRefreshToken(OAuth2RefreshToken refreshToken, OAuth2Authentication authentication) {
		...
		if (refreshToken instanceof ExpiringOAuth2RefreshToken) {
			...
			expiration = expiringToken.getExpiration();
		}
		...
		jdbcTemplate.update(insertRefreshTokenSql, new Object[] { extractTokenKey(refreshToken.getValue()),
				...
				new SqlLobValue(serializeAuthentication(authentication)), expiration}, new int[] { Types.VARCHAR, Types.BLOB,
