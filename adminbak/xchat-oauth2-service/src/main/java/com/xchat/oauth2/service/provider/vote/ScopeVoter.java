...
package com.xchat.oauth2.service.provider.vote;
...
/**
 * <p>
 * Votes if any {@link org.springframework.security.access.ConfigAttribute#getAttribute()} starts with a prefix indicating that it is an OAuth2 scope. The
 ...
 * <code>Authentication</code> is not a {@link com.xchat.oauth2.service.provider.OAuth2Authentication} or the current client authentication is not a
 * {@link com.xchat.oauth2.service.provider.AuthorizationRequest} (which contains the scope data). Votes to grant access if there is an exact matching
 * {@link com.xchat.oauth2.service.provider.AuthorizationRequest#getScope() authorized scope} to a <code>ConfigAttribute</code> starting with the scope
...
public class ScopeVoter implements AccessDecisionVoter<Object> {
	...
	/**
	 * Flag to determine the behaviour on access denied. If set then we throw an {@link com.xchat.oauth2.service.common.exceptions.InsufficientScopeException}
	 * instead of returning {@link org.springframework.security.access.AccessDecisionVoter#ACCESS_DENIED}. This is unconventional for an access decision
