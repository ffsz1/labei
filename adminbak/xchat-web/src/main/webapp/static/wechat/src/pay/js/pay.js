...
var num = $('#num'),
  ...
  // pay_url = 'http://beta.erbanyy.com/wx/submitPay?',
  payBase_url = '/wx/submitPay?',
  list_url = '/chargeprod/list?channelType=1',
  locateObj = getQueryString()
  // betaBol = betaEnvCheck();
  // base_url = (betaBol)?'http://beta.erbanyy.com/wechat/src/pay/pay.html':'http://www.erbanyy.com/wechat/src/pay/pay.html'
  // fail_url = 'http://beta.erbanyy.com/wechat/src/pay/failed.html'
  ;
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res.code == 4002) {
          ...
          descConfirm.html('向<span class="nick">'
            + nick 
            + '</span>('
            + '拉贝号：'
            + erbanNo
            +')充值<span class="desc_price">'
            + price 
            + '</span>元？');
