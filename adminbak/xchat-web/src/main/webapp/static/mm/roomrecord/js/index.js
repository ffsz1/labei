$(function(){
        ...
        success:function(data){
            ...
            $.each(riliushui,function(i) {
                ...
                if(i==(riliushui.length-1)){
                    $child = $('<dl><a href=https://www.47huyu.cn/mm/roomrecord/list.html?date='+riliushui[i].date+'&roomUid='+urlData[1]+'=><dt><div class="date2">' + (date.getMonth()+1)+'-'+date.getDate() + '<br /><span class="notice">' + (2000+date.getYear()-100) + '</span></div></dt><dd><span class="money">+' + riliushui[i].totalGoldNum + '</span><br/><span class="notice">房间流水</span></dd></a></dl>');
                }else {
                    str1 = riliushui[i+1].date.replace(/-/g,"/");
                    var lastDate = new Date(str1 );
                    if (i!=(riliushui.length-1)&&date.getYear()==lastDate.getYear()){
                        $child = $('<dl><a href=https://www.47huyu.cn/mm/roomrecord/list.html?date='+riliushui[i].date+'&roomUid='+urlData[1]+'=><dt><div class="date">' + (date.getMonth()+1)+'-'+date.getDate() + '</div></dt><dd><span class="money">+' + riliushui[i].totalGoldNum + '</span><br/><span class="notice">房间流水</span></dd></a></dl>');
                    }else{
                        $child = $('<dl><a href=https://www.47huyu.cn/mm/roomrecord/list.html?date='+riliushui[i].date+'&roomUid='+urlData[1]+'=><dt><div class="date2">' + (date.getMonth()+1)+'-'+date.getDate() + '<br /><span class="notice">' + (2000+date.getYear()-100) + '</span></div></dt><dd><span class="money">+' + riliushui[i].totalGoldNum + '</span><br/><span class="notice">房间流水</span></dd></a></dl>')
                    }
                }
    ...
    $('.ad').click(function(){
        location.href = 'https://www.47huyu.cn/mm/literank/index.html'
    })
})
