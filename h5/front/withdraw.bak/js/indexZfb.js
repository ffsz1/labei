...
var info = {};
var initData = parseQueryString(window.location.href)
...
$('.verify-wx input').on('input', function() { //输入时 控制提交按钮的活动
    if (verifyTel($('#input1').val())) { //验证正确 
        if ($('#input0').val().length == 0 || $('#input0').val().trim() == '' || $('#input3').val().length == 0 || $('#input3').val().trim() == '') {
            return;
        }
    ...
    if ($(this).hasClass('active')) {
        ...
        $.ajax({
                ...
                success: function success(res) {
                    ...
                    if (res.code === 200) {
                        var data = res.data;
                        data.zfbAcc = $('#input0').val();
                        data.zfbName = $('#input3').val();
                        data.zfbSele = 1;
    ...
    if ($('.get-btn').hasClass('active')) { //60秒之内不能重复请求
        ...
        if (verifyTel(val)) { //验证正确 发送请求
            ...
            $.ajax({
                ...
                success: function success(res) {
                    console.log(res, allUrl() + '/withDraw/v2/getAccountSms')
                    if (res.code === 200) {
                        countdown() //开启倒计时
                        $toast.show({
                            text: '发送请求',
