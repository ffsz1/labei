...
Dialog.prototype = {
   ...
   * 创建按钮
   */
  creatButton: function creatButton(footer, buttons) {
    var _this_ = this; // if (buttons.length) {
    // }
    //遍历出数组
    ...
    $(buttons).each(function (index, element, item) {
      ...
      singleButton.addClass(className).text(text); //如果有回调函数，按钮绑定回调函数
...
window.loading = function loading(time) {
  ...
  $('.loading .loading-img').attr('src', 'https://www.tiannb.com/front/lib/toolKit/images/loading/loading.gif'); // lo_time = setInterval(function() {
  //     if (i < 1) { i = 1 };
  //     if (i == 12) { i = 1 };
  //     i++;
  //     //线下起服务
