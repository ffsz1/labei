...
public class CarAdapter extends BaseQuickAdapter<DressUpBean, CarAdapter.ViewHolder> {
    ...
    public CarAdapter(int type, boolean isMySelf, Context context, boolean isBag) {
    ...
    protected void convert(ViewHolder helper, DressUpBean item) {
        if (isBag && item.getDaysRemaining() != null) {
            helper.remainDayTv.setText("剩余" + item.getDaysRemaining() + "天");
            helper.remainDayTv.setVisibility(View.VISIBLE);
        }
        ...
        int isSelect = item.getIsSelect();
        //设置天数
        helper.tvCarTime.setText(item.getEffectiveTime() + "天");
        if (isPurse == 0) {
            helper.ivCarTag.setBackgroundResource(R.drawable.car_lock);
            helper.tvCarTime.setTextColor(mContext.getResources().getColor(R.color.color_ff9b9a9a));
        } else {
            helper.ivCarTag.setBackgroundResource(R.drawable.car_time);
...
//        }
        ...
        if (isSelect == 0 || isSelect == 1) {
            ...
            helper.tvCarName.setTextColor(blackColor);
            helper.tvCarPrice.setTextColor(blackColor);
            helper.tvCarTime.setTextColor(Color.parseColor("#999999"));
        } else if (isSelect == 2) {
            helper.rootView.setBackgroundResource(R.drawable.ic_dress_select_bg);

            int whitColor = mContext.getResources().getColor(R.color.white);
            helper.tvCarName.setTextColor(whitColor);
            helper.tvCarPrice.setTextColor(whitColor);
            helper.tvCarTime.setTextColor(whitColor);
        }
